...
function radar_counts(): array
{
    ...
    return [
        ...
        'running_tasks' => (int)$scalar("SELECT COUNT(*) FROM crm_radar_search_tasks WHERE task_status IN ('pending','generating_keywords','searching','fetching_pages','identifying_company')"),
...
function radar_task_start(int $id): array
{
    ...
    $task = $detail['task'];
    if (in_array((string)$task['task_status'], ['completed','searching','generating_keywords','fetching_pages','identifying_company','waiting_analysis'], true)) {
    ...
    db()->prepare("DELETE FROM crm_radar_job_queue WHERE task_id=? AND job_status IN ('pending','failed','cancelled')")->execute([$id]);
