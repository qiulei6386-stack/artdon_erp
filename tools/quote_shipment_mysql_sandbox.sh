...
for test in quote_conversion_mysql.php quote_shipment_batch_mysql.php; do
...
money_schema="quote_money_$suffix"
"$mysql_bin" --no-defaults --protocol=SOCKET --socket="$socket_path" --user=root -e "CREATE DATABASE $money_schema CHARACTER SET utf8mb4;"
export CRM_PHASE1_MYSQL_SCHEMA="$money_schema"
timeout 60 "$php_bin" "${php_args[@]}" "$repo_root/tests/quote_shipment_batch_mysql.php"
echo 'Shipment integration and conversion isolated MySQL tests passed. No production database connection was used.'
