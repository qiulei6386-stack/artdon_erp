...
function qoe_build_pl_sheet($order,$ship,$items,$cartons,$settings,$customer,$sellerName,$sellerText,$docTitle,&$images,&$rowHeights,&$colWidths,&$merges){
  ...
  foreach($plItems as $i=>$it){ $desc=function_exists('qd_desc')?qd_desc($it):($it['specification']??''); $height=max(44,min(140,36+qoe_line_count($desc)*11)); $rowHeights[$r]=$height; $img=function_exists('qd_img_src')?qd_img_src($it):($it['image']??''); if($img!==''){ $height=max($height,88); $rowHeights[$r]=$height; $im=qoe_image_source_to_jpeg($img); if($im){ $im['row']=$r; $im['col']=1; $images[]=$im; } } $dims=function_exists('qd_carton_dims')?qd_carton_dims($it['carton_size']??''):['','',''];
    $row=[qoe_cell(1,$r,'',7),qoe_cell(2,$r,$it['customer_code']??'',7),qoe_cell(3,$r,$it['product_code']??'',7),qoe_cell(4,$r,$desc,9),qoe_cell(5,$r,$it['color']??'',7),qoe_cell(6,$r,qoe_num($it['qty']??0),8,true)];
    if(empty($it['_carton_skip_pack'])){
      $span=max(1,(int)($it['_carton_rowspan']??1));
      $row=array_merge($row,[qoe_cell(7,$r,qoe_num($it['pcs_per_ctn']??0),8,true),qoe_cell(8,$r,$dims[0]??'',7),qoe_cell(9,$r,$dims[1]??'',7),qoe_cell(10,$r,$dims[2]??'',7),qoe_cell(11,$r,qoe_num($it['cartons']??0),8,true),qoe_cell(12,$r,qoe_num($it['nw']??0),8,true),qoe_cell(13,$r,qoe_num($it['gw']??0),8,true),qoe_cell(14,$r,qoe_num($it['cbm']??0),8,true)]);
      if($span>1){ for($mc=7;$mc<=14;$mc++) qoe_merge($merges,$mc,$r,$mc,$r+$span-1); }
    }
    $rows[]=qoe_row($r,$row,$height); $r++; }
  $end=max($start,$r-1); $rowHeights[$r]=24; $rows[]=qoe_row($r,array_merge(qoe_style_cells($r,1,5,7,''),[qoe_cell(6,$r,$totalQty,10,true),qoe_cell(7,$r,'',7),qoe_cell(8,$r,'Total:',12),qoe_cell(9,$r,'',12),qoe_cell(10,$r,'',12),qoe_cell(11,$r,$totalCartons,10,true),qoe_cell(12,$r,$totalNw,10,true),qoe_cell(13,$r,$totalGw,10,true),qoe_cell(14,$r,$totalCbm,10,true)]),24); qoe_merge($merges,8,$r,10,$r); $r+=3;
