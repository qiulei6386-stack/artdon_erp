...
function qoe_build_ci_sheet($order,$ship,$items,$settings,$customer,$sellerName,$sellerText,$docTitle,&$images,&$rowHeights,&$colWidths,&$merges){
  ...
  foreach($terms as $row){ $label=(string)($row[0]??''); $val=(string)($row[1]??''); $cells=[]; for($c=8;$c<=9;$c++)$cells[]=qoe_cell($c,$tr,$c===8?$label:'',4); for($c=10;$c<=11;$c++)$cells[]=qoe_cell($c,$tr,$c===10?$val:'',5); $rows[]=qoe_row($tr,$cells,26); qoe_merge($merges,8,$tr,9,$tr); qoe_merge($merges,10,$tr,11,$tr); $rowHeights[$tr]=26; $tr++; }
  ...
  foreach($items as $i=>$it){ $desc=function_exists('qd_desc')?qd_desc($it):($it['specification']??''); $height=max(88,min(140,36+qoe_line_count($desc)*11)); $rowHeights[$r]=$height; $img=function_exists('qd_img_src')?qd_img_src($it):($it['image']??''); if($img!==''){ $im=qoe_image_source_to_jpeg($img); if($im){ $im['row']=$r; $im['col']=1; $images[]=$im; } }
    $qty=qoe_num($it['qty']??0); $price=qoe_num($it['unit_price']??0); $row=[qoe_cell(1,$r,'',7),qoe_cell(2,$r,$it['order_no']??'',7),qoe_cell(3,$r,$it['size']??'',7),qoe_cell(4,$r,$it['customer_code']??'',7),qoe_cell(5,$r,$it['product_code']??'',7),qoe_cell(6,$r,$desc,9),qoe_cell(7,$r,$it['color']??'',7),qoe_cell(8,$r,$qty,8,true),qoe_cell(9,$r,$price,8,true),qoe_formula(10,$r,'H'.$r.'*I'.$r,8),qoe_cell(11,$r,$it['hs_code']??'',15)]; $rows[]=qoe_row($r,$row,$height); $r++; }
  $end=max($start,$r-1); $rowHeights[$r]=24; $rows[]=qoe_row($r,array_merge(qoe_style_cells($r,1,7,7,''),[qoe_formula(8,$r,'SUM(H'.$start.':H'.$end.')',10),qoe_cell(9,$r,'Total:',12),qoe_formula(10,$r,'SUM(J'.$start.':J'.$end.')',10),qoe_cell(11,$r,'',7)]),24); $r+=3;
...
function qoe_build_pl_sheet($order,$ship,$items,$cartons,$settings,$customer,$sellerName,$sellerText,$docTitle,&$images,&$rowHeights,&$colWidths,&$merges){
  ...
  $tr=5; $terms=function_exists('qd_doc_terms')?qd_doc_terms('pl',$order,$ship,$settings):[]; foreach($terms as $row){$label=(string)($row[0]??'');$val=(string)($row[1]??'');$cells=[];for($c=11;$c<=12;$c++)$cells[]=qoe_cell($c,$tr,$c===11?$label:'',4);for($c=13;$c<=15;$c++)$cells[]=qoe_cell($c,$tr,$c===13?$val:'',5);$rows[]=qoe_row($tr,$cells,26);qoe_merge($merges,11,$tr,12,$tr);qoe_merge($merges,13,$tr,15,$tr);$rowHeights[$tr]=26;$tr++;}
  ...
  foreach($plItems as $i=>$it){ $desc=function_exists('qd_desc')?qd_desc($it):($it['specification']??''); $height=max(44,min(140,36+qoe_line_count($desc)*11)); $rowHeights[$r]=$height; $img=function_exists('qd_img_src')?qd_img_src($it):($it['image']??''); if($img!==''){ $height=max($height,88); $rowHeights[$r]=$height; $im=qoe_image_source_to_jpeg($img); if($im){ $im['row']=$r; $im['col']=1; $images[]=$im; } } $dims=function_exists('qd_carton_dims')?qd_carton_dims($it['carton_size']??''):['','',''];
    ...
    if(empty($it['_carton_skip_pack'])){
      ...
      $row=array_merge($row,[qoe_cell(8,$r,qoe_num($it['pcs_per_ctn']??0),8,true),qoe_cell(9,$r,$dims[0]??'',7),qoe_cell(10,$r,$dims[1]??'',7),qoe_cell(11,$r,$dims[2]??'',7),qoe_cell(12,$r,qoe_num($it['cartons']??0),8,true),qoe_cell(13,$r,qoe_num($it['nw']??0),8,true),qoe_cell(14,$r,qoe_num($it['gw']??0),8,true),qoe_cell(15,$r,qoe_num($it['cbm']??0),8,true)]);
      if($span>1){ for($mc=8;$mc<=15;$mc++) qoe_merge($merges,$mc,$r,$mc,$r+$span-1); }
    }
    $rows[]=qoe_row($r,$row,$height); $r++; }
  $end=max($start,$r-1); $rowHeights[$r]=24; $rows[]=qoe_row($r,array_merge(qoe_style_cells($r,1,6,7,''),[qoe_cell(7,$r,$totalQty,10,true),qoe_cell(8,$r,'',7),qoe_cell(9,$r,'Total:',12),qoe_cell(10,$r,'',12),qoe_cell(11,$r,'',12),qoe_cell(12,$r,$totalCartons,10,true),qoe_cell(13,$r,$totalNw,10,true),qoe_cell(14,$r,$totalGw,10,true),qoe_cell(15,$r,$totalCbm,10,true)]),24); qoe_merge($merges,9,$r,11,$r); $r+=3;
