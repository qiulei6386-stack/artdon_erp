...
<div class="app-shell" data-shell>
  ...
  <section class="workspace">
    ...
    <main class="content">
      ...
      <?php elseif ($activePage && $requestedPage === 'commercial_product_library'): $catalog=(new Artdon\CommercialCenter\Services\CatalogCenterService())->products($view['auth'], (string)($_GET['q']??''), (string)($_GET['category']??'')); ?>
        ...
        <form class="catalog-filters" method="get"><input type="hidden" name="page" value="commercial_product_library"><input type="search" name="q" value="<?= cc_h((string)($_GET['q']??'')) ?>" placeholder="搜索型号、产品名或系列"><select name="category"><option value="">全部分类</option><?php foreach($catalog['categories'] as $category): ?><option value="<?= cc_h($category['category']) ?>"><?= cc_h($category['category']) ?>（<?= (int)$category['total'] ?>）</option><?php endforeach; ?></select><button type="submit">搜索</button><span class="view-switch">▦ 卡片　☷ 列表</span></form>
