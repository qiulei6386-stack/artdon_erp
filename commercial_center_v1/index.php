      ...
      <?php elseif (in_array($activeView, ['products', 'materials'], true)): $catalog = $view[$activeView]; ?>
        ...
        <?php else: ?>
          <form class="catalog-filters" method="get">
            ...
            <?php if($activeView==='products'): ?><select name="size"><option value="12" <?= (int)($catalog['page_size']??24)===12?'selected':'' ?>>12 个/页</option><option value="24" <?= (int)($catalog['page_size']??24)===24?'selected':'' ?>>24 个/页</option><option value="48" <?= (int)($catalog['page_size']??24)===48?'selected':'' ?>>48 个/页</option></select><?php endif; ?>
            <button type="submit">筛选</button><a href="?view=<?= cc_h($activeView) ?>">清除</a>
            <span><?= $activeView==='products'?'当前页 '.count($catalog['rows']).' / 共 '.(int)($catalog['total']??0).' 个':'当前返回 '.count($catalog['rows']).' 条' ?></span>
          </form>
          ...
          <?php if($activeView==='products'): $productQuery='view=products&q='.rawurlencode((string)$view['filters']['q']).'&category='.rawurlencode((string)$view['filters']['category']).'&size='.(int)$catalog['page_size']; ?><nav class="product-pagination" aria-label="产品分页"><a href="?<?= $productQuery ?>&p=<?= max(1,(int)$catalog['page']-1) ?>">上一页</a><span>第 <?= (int)$catalog['page'] ?> / <?= (int)$catalog['pages'] ?> 页</span><select onchange="location.href='?<?= $productQuery ?>&p='+this.value"><?php for($i=1;$i<=(int)$catalog['pages'];$i++): ?><option value="<?= $i ?>" <?= $i===(int)$catalog['page']?'selected':'' ?>><?= $i ?></option><?php endfor; ?></select><a href="?<?= $productQuery ?>&p=<?= min((int)$catalog['pages'],(int)$catalog['page']+1) ?>">下一页</a></nav><?php endif; ?>
