...
<div class="app-shell" data-shell>
  ...
  <section class="workspace">
    ...
    <main class="content">
      ...
      <?php elseif ($activePage && $requestedPage === 'commercial_product_library'): $pageNo=max(1,(int)($_GET['p']??1)); $requestedSize=(int)($_GET['size']??12); $pageSize=in_array($requestedSize,[6,12,24],true)?$requestedSize:12; $catalog=(new Artdon\CommercialCenter\Services\CatalogCenterService())->products($view['auth'], (string)($_GET['q']??''), (string)($_GET['category']??'')); $allProducts=$catalog['rows']; $totalProducts=count($allProducts); $totalPages=max(1,(int)ceil($totalProducts/$pageSize)); $pageNo=min($pageNo,$totalPages); $catalog['rows']=array_slice($allProducts,($pageNo-1)*$pageSize,$pageSize); ?>
        ...
        <form class="catalog-filters product-library-filters" method="get"><input type="hidden" name="page" value="commercial_product_library"><input type="search" name="q" value="<?= cc_h((string)($_GET['q']??'')) ?>" placeholder="搜索型号 / 产品"><select name="category"><option value="">全部分类</option><?php foreach($catalog['categories'] as $category): ?><option value="<?= cc_h($category['category']) ?>" <?= (string)($_GET['category']??'')===$category['category']?'selected':'' ?>><?= cc_h($category['category']) ?>（<?= (int)$category['total'] ?>）</option><?php endforeach; ?></select><select name="size"><option value="6" <?= $pageSize===6?'selected':'' ?>>6 个/页</option><option value="12" <?= $pageSize===12?'selected':'' ?>>12 个/页</option><option value="24" <?= $pageSize===24?'selected':'' ?>>24 个/页</option></select><button type="submit">搜索</button><span class="view-switch">共 <?= $totalProducts ?> 个　▦ 卡片</span></form>
