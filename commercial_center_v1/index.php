...
<div class="app-shell" data-shell>
  ...
  <section class="workspace">
    ...
    <main class="content">
      <?php if ($activePage && $requestedPage === 'commercial_product_library'): require __DIR__ . '/views/product_library_v2.php'; elseif ($activePage && $requestedPage === 'product_sync_center'): $syncStatus=(new Artdon\CommercialCenter\Services\ProductSyncService())->status($view['auth']); ?>
      ...
      <?php elseif ($activePage && $requestedPage === 'commercial_product_library'): $pageNo=max(1,(int)($_GET['p']??1)); $requestedSize=(int)($_GET['size']??12); $pageSize=in_array($requestedSize,[6,12,24],true)?$requestedSize:12; $viewMode=(($_GET['mode']??'cards')==='list')?'list':'cards'; $catalog=(new Artdon\CommercialCenter\Services\CatalogCenterService())->products($view['auth'], (string)($_GET['q']??''), (string)($_GET['category']??'')); $allProducts=$catalog['rows']; $bomCosts=[]; try{$bomCosts=(new Artdon\CommercialCenter\Repositories\LegacyCatalogReadRepository())->bomCostsForModels(array_column($allProducts,'model_no'));}catch(\Throwable $bomError){$bomCosts=[];} foreach($allProducts as &$product){$product['bom_cost']=$bomCosts[(string)$product['model_no']]??null;} unset($product); $totalProducts=count($allProducts); $totalPages=max(1,(int)ceil($totalProducts/$pageSize)); $pageNo=min($pageNo,$totalPages); $catalog['rows']=array_slice($allProducts,($pageNo-1)*$pageSize,$pageSize); $statusCounts=['可报价'=>0,'开发中'=>0,'停售'=>0]; foreach($allProducts as $product){$s=(string)($product['status']??''); if(isset($statusCounts[$s])) $statusCounts[$s]++;} ?>
        <section class="page-head"><div><span class="eyebrow">COMMERCIAL PRODUCT LIBRARY</span><h1>报价产品库</h1><p>读取命名中心产品引用；只有状态为“可报价”的产品才能进入后续报价。</p></div><div class="page-meta"><span>来源</span><b>命名中心只读</b></div></section>
        <section class="product-stats"><div><span>产品总数</span><strong><?= $totalProducts ?></strong></div><div><span>可报价数量</span><strong><?= $statusCounts['可报价'] ?></strong></div><div><span>开发中数量</span><strong><?= $statusCounts['开发中'] ?></strong></div><div><span>停售数量</span><strong><?= $statusCounts['停售'] ?></strong></div></section><form class="catalog-filters product-library-filters" method="get"><input type="hidden" name="page" value="commercial_product_library"><input type="hidden" name="mode" value="<?= $viewMode ?>"><input type="search" name="q" value="<?= cc_h((string)($_GET['q']??'')) ?>" placeholder="型号 / 系列 / 类别 / 功率 / 参数"><select name="category"><option value="">全部类别</option><?php foreach($catalog['categories'] as $category): ?><option value="<?= cc_h($category['category']) ?>" <?= (string)($_GET['category']??'')===$category['category']?'selected':'' ?>><?= cc_h($category['category']) ?>（<?= (int)$category['total'] ?>）</option><?php endforeach; ?></select><select name="status"><option value="">全部状态</option><option>可报价</option><option>开发中</option><option>停售</option></select><select name="power"><option value="">全部功率</option><option>20W</option><option>30W</option><option>40W</option></select><select name="size"><option value="6" <?= $pageSize===6?'selected':'' ?>>6 个/页</option><option value="12" <?= $pageSize===12?'selected':'' ?>>12 个/页</option><option value="24" <?= $pageSize===24?'selected':'' ?>>24 个/页</option></select><button type="submit">搜索</button><a class="view-switch" href="?page=commercial_product_library&mode=<?= $viewMode==='cards'?'list':'cards' ?>">▦ 卡片 / ☷ 列表</a><span class="view-switch">共 <?= $totalProducts ?> 个</span></form>
        <?php if ($catalog['status'] !== 'available'): ?><section class="login-notice"><strong><?= $catalog['status']==='unauthenticated'?'需要统一登录':'当前账号无命名中心查看权限' ?></strong></section><?php else: ?><section class="product-library-grid"><?php foreach($catalog['rows'] as $row): ?><article class="library-card"><div class="library-thumb"><?php $image=cc_legacy_asset_url($row['image_path']); if($image): ?><img src="<?= cc_h($image) ?>" alt="" loading="lazy"><?php else: ?><span>NO IMAGE</span><?php endif; ?></div><div><small><?= cc_h($row['category']) ?> · <?= cc_h($row['series_name']) ?></small><h3><?= cc_h($row['model_no']) ?></h3><p><?= cc_h($row['product_name']) ?></p><footer><span>状态：<?= cc_h($row['status']) ?></span><a href="?view=products">查看详情</a></footer></div></article><?php endforeach; ?><?php if($catalog['rows']===[]): ?><div class="empty">当前没有可显示的命名中心产品。</div><?php endif; ?></section><nav class="product-pagination" aria-label="产品分页"><?php $queryBase='page=commercial_product_library&q='.rawurlencode((string)($_GET['q']??'' )).'&category='.rawurlencode((string)($_GET['category']??'' )).'&size='.$pageSize; ?><a href="?<?= $queryBase ?>&p=<?= max(1,$pageNo-1) ?>">上一页</a><span>第 <?= $pageNo ?> / <?= $totalPages ?> 页</span><select onchange="location.href='?<?= $queryBase ?>&p='+this.value"><?php for($i=1;$i<=$totalPages;$i++): ?><option value="<?= $i ?>" <?= $i===$pageNo?'selected':'' ?>><?= $i ?></option><?php endfor; ?></select><a href="?<?= $queryBase ?>&p=<?= min($totalPages,$pageNo+1) ?>">下一页</a></nav><?php endif; ?>
      <?php elseif ($activePage && $requestedPage === 'permission_center'): ?>
