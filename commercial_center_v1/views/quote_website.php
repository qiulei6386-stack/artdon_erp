<?php declare(strict_types=1); $rows=array_slice($catalogRows,0,6); $webQty=[12,24,40,20,16,30]; ?>
  ...
  <div class="ref-body web-body">
    <main>
      <section class="ref-panel web-order-info">
        <div><label>网站订单号</label><strong>SG202505120789</strong><small>♙</small><label>网站下单时间</label><strong>2025-05-12 14:32:18</strong><small>♙</small><label>预计交期</label><strong>2025-06-10</strong><small>♙</small></div>
        <div><label>内部报价号</label><strong>Q250512-0879</strong><small>♙</small><label>业务员</label><strong>James Tan</strong><small>♙</small><label>付款方式</label><strong>T/T 30% 预付，70% 见提单副本</strong><small>♙</small></div>
        <div><label>客户</label><strong>BrightLite Pte Ltd</strong><small>♙</small><label>国家/地区</label><strong>Singapore</strong><small>♙</small></div>
        <div><label>联系人</label><strong>Mr. Alan Lee</strong><small>♙</small><label>币种</label><strong>USD</strong><small>♙</small></div>
      </section>
      <section class="ref-panel web-products">
        ...
        <div class="ref-table-scroll">
          ...
          <tbody data-quote-lines><?php foreach($rows as $i=>$row): $price=isset($row['bom_cost'])&&$row['bom_cost']!==null?round((float)$row['bom_cost']*1.65,2):0; $img=cc_legacy_asset_url($row['image_path']??''); ?><tr><td><?= $i+1 ?></td><td><div class="ref-thumb"><?php if($img):?><img src="<?=cc_h($img)?>" alt=""><?php else:?>图<?php endif;?></div></td><td><b><?=cc_h($row['model_no'])?></b> ♙</td><td><?=cc_h(preg_replace('/ · BOM成本.*$/u','',(string)$row['product_name']))?> ♙</td><td><span><?=cc_h($row['category'])?></span><small><?=['黑色 / 30° / 3000K','白色 / 20W / 4000K','白色 / 2W / 3000K','黑色 / 可调 / 6W','白色 / 15W / 3000K','白色 / 3W / 4000K'][$i]?></small></td><td><input type="number" value="<?=$webQty[$i]?>" data-qty readonly></td><td><b><?=number_format($price,2)?></b> ♙</td><td><input type="number" value="<?=$price?>" data-price></td><td><input type="number" value="0" data-discount></td><td data-line-total>0.00</td><td>2025-06-<?=str_pad((string)(10+($i>1?5:0)+($i>3?3:0)),2,'0',STR_PAD_LEFT)?> ♙</td><td>— ♙</td></tr><?php endforeach;?></tbody></table>
