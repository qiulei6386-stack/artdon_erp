...
function shipmentCardActions(){
  document.querySelectorAll('#orderDetail .ship-card').forEach(card=>{
    ...
    buttons.insertAdjacentHTML('beforeend',`<button class="gray" onclick="openOrderDocManage('pl',${id})">管理 PL</button><button class="gray" onclick="openOrderDocManage('ci',${id})">管理 CI</button>${status==='草稿'?`<button class="gray" onclick="editShipment(${id})">修改批次</button><button class="red" onclick="deleteShipment(${id})">删除批次</button>`:''}`);
  });
}
