...
function commissionItemEstimate(it){let mode=commissionItemValue(it,'commission_mode')||'percent',v=Number(commissionItemValue(it,'commission_value')||0);if(mode==='percent')return Number(it.product_amount||0)*v/100;if(mode==='fixed_order')return v;if(mode==='fixed_unit')return Number(it.qty||0)*v;return null}
...
function renderCommissionItemRows(r){if(!COMMISSION_ORDER_EXPANDED.has(Number(r.order_id)))return '';return (r.items||[]).map(it=>{let d=COMMISSION_ITEM_DRAFTS[it.order_item_id],estimate=commissionItemEstimate(it),state=commissionItemState(r,it),lock=state.inherited;return `<tr class="commission-product-row ${d?'changed':''}" style="background:#f3f7fc"><td><input type="checkbox" ${state.enabled?'checked':''} ${lock?'disabled title="整单佣金：产品随整单参与"':''} onchange="stageCommissionItem(${it.order_item_id},'is_commission_enabled',this.checked?1:0)">${d?'<b style="color:#d97706">●</b>':''}</td><td style="padding-left:22px;color:#2563eb">${it.image?`<img src="${esc(it.image)}" style="width:28px;height:28px;object-fit:contain;vertical-align:middle">`:''} ↳ ${Number(it.item_index||0)}</td><td><b>${esc(it.product_model||'-')}</b><br><small>${esc(it.customer_model||'-')}</small></td><td title="${esc(it.product_name||'')}">${esc(it.product_name||'-')}</td><td>${esc(it.color||'-')}</td><td>${esc(r.order_currency)} ${money(it.product_amount)}</td><td>${fmtNum(it.qty)}</td><td>${money(it.unit_price)}</td><td>${state.label}</td><td>${commissionItemInput(it,'target_name','','text',lock)}</td><td>${commissionItemInput(it,'target_type','target_type','text',lock)}</td><td>${commissionItemInput(it,'commission_mode','commission_mode','text',lock)}</td><td>${commissionItemInput(it,'commission_value','','number',lock)}</td><td>${commissionItemInput(it,'calc_base','calc_base','text',lock)}</td><td>${commissionItemInput(it,'currency','currency','text',lock)}</td><td>${lock?'随整单计算':(state.enabled&&estimate!==null?money(estimate):'0.00')}</td><td>${commissionEffectSelect(it.order_item_id,commissionItemValue(it,'receivable_effect')||'none',true,lock)}</td><td>-</td><td>${commissionItemInput(it,'settle_status','settle_status','text',lock)}</td><td>${commissionItemInput(it,'note','','text',lock)}</td></tr>`}).join('')}
