...
let CURRENT_ORDER=null, CURRENT_ORDER_ITEMS=[], CURRENT_ORDER_SHIPMENTS=[], PACKAGING=[], SHIPMENT_PREP=null, SHIPMENT_EDIT_ID=0, packTimer=null;
...
function renderShipmentRows(items){let box=$('shipItemRows');if(!box)return;box.innerHTML=(items||[]).map((it,i)=>{let editing=SHIPMENT_EDIT_ID>0,existing=it.shipment_item||{},remain=num(it.remain_qty),shipped=editing?num(it.other_shipped_qty):num(it.shipped_qty),p=it.packaging_profile||{};let qty=editing?num(existing.qty):remain;let pcs=editing?(num(existing.pcs_per_ctn)||''):(num(p.pcs_per_ctn)||'');let ctns=editing?(num(existing.cartons)||''):(pcs?Math.ceil(remain/num(pcs)):''),size=editing?(existing.carton_size||''):(p.carton_size||'');let nw=editing?num(existing.nw):(num(p.carton_nw)?num(p.carton_nw)*num(ctns):(num(p.unit_nw)*remain));let gw=editing?num(existing.gw):(num(p.carton_gw)?num(p.carton_gw)*num(ctns):(num(p.unit_gw)*remain));let cbm=editing?num(existing.cbm):(num(p.carton_cbm)?num(p.carton_cbm)*num(ctns):dimCbm(size,ctns));return `<tr data-order-item="${Number(it.id)}"><td>${esc(it.product_code||'')}</td><td>${esc(it.customer_code||'')}</td><td>${esc(it.product_name||'')}</td><td>${fmtNum(it.qty)}</td><td>${fmtNum(shipped)}</td><td>${fmtNum(remain)}</td><td><input class="ship-qty" type="number" step="0.01" max="${remain}" value="${qty>0?fmtNum(qty):''}" oninput="recalcShipRow(this)"></td><td><input class="ship-pcs" type="number" step="0.01" value="${esc(pcs)}" oninput="recalcShipRow(this)"></td><td><input class="ship-ctns" type="number" step="0.01" value="${esc(ctns)}" oninput="recalcShipmentTotals()"></td><td><input class="ship-size" value="${esc(size)}" placeholder="45*35*28cm" oninput="recalcShipRow(this)"></td><td><input class="ship-nw" type="number" step="0.001" value="${nw?fmtNum(nw,3):''}" oninput="recalcShipmentTotals()"></td><td><input class="ship-gw" type="number" step="0.001" value="${gw?fmtNum(gw,3):''}" oninput="recalcShipmentTotals()"></td><td><input class="ship-cbm" type="number" step="0.0001" value="${cbm?fmtNum(cbm,4):''}" oninput="recalcShipmentTotals()"></td><td><input class="ship-note" placeholder="备注" value="${esc(existing.note||'')}"></td></tr>`}).join('')||'<tr><td colspan="14">没有未出货产品</td></tr>';}
