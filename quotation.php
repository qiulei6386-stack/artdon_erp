...
function stageCommissionItem(id,f,v){id=Number(id);if(['commission_value','settled_amount','is_commission_enabled'].includes(f))v=Number(v||0);if(!COMMISSION_ITEM_DRAFTS[id])COMMISSION_ITEM_DRAFTS[id]={order_item_id:id};COMMISSION_ITEM_DRAFTS[id][f]=v;renderCommissionOrderRows()}
function commissionItemInput(it,f,group='',type='text'){let v=commissionItemValue(it,f);if(group)return `<select class="commission-edit" onchange="stageCommissionItem(${it.order_item_id},'${f}',this.value)">${commissionSelect(group,v)}</select>`;return `<input class="commission-edit" type="${type}" ${type==='number'?'min="0" step="0.01"':''} value="${esc(v)}" oninput="stageCommissionItem(${it.order_item_id},'${f}',this.value)">`}
...
function renderCommissionItemRows(r){if(!COMMISSION_ORDER_EXPANDED.has(Number(r.order_id)))return '';return (r.items||[]).map(it=>{let d=COMMISSION_ITEM_DRAFTS[it.order_item_id],estimate=commissionItemEstimate(it),enabled=Number(commissionItemValue(it,'is_commission_enabled')??1);return `<tr class="commission-product-row ${d?'changed':''}" style="background:#f3f7fc"><td><input type="checkbox" ${enabled?'checked':''} onchange="stageCommissionItem(${it.order_item_id},'is_commission_enabled',this.checked?1:0)">${d?'<b style="color:#d97706">●</b>':''}</td><td style="padding-left:22px;color:#2563eb">${it.image?`<img src="${esc(it.image)}" style="width:28px;height:28px;object-fit:contain;vertical-align:middle">`:''} ↳ ${Number(it.item_index||0)}</td><td><b>${esc(it.product_model||'-')}</b><br><small>${esc(it.customer_model||'-')}</small></td><td title="${esc(it.product_name||'')}">${esc(it.product_name||'-')}</td><td>${esc(it.color||'-')}</td><td>${esc(r.order_currency)} ${money(it.product_amount)}</td><td>${fmtNum(it.qty)}</td><td>${money(it.unit_price)}</td><td>${enabled?'参与佣金':'不参与'}</td><td>${commissionItemInput(it,'target_name')}</td><td>${commissionItemInput(it,'target_type','target_type')}</td><td>${commissionItemInput(it,'commission_mode','commission_mode')}</td><td>${commissionItemInput(it,'commission_value','','number')}</td><td>${commissionItemInput(it,'calc_base','calc_base')}</td><td>${commissionItemInput(it,'currency','currency')}</td><td>${enabled&&estimate!==null?money(estimate):'0.00'}</td><td>${commissionEffectSelect(it.order_item_id,commissionItemValue(it,'receivable_effect')||'none',true)}</td><td>-</td><td>${commissionItemInput(it,'settle_status','settle_status')}</td><td>${commissionItemInput(it,'note')}</td></tr>`}).join('')}
