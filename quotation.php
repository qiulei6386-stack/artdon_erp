...
function renderCommissionItemRows(r){if(!COMMISSION_ORDER_EXPANDED.has(Number(r.order_id)))return '';return (r.items||[]).map(it=>{let d=COMMISSION_ITEM_DRAFTS[it.order_item_id],estimate=commissionItemEstimate(it),state=commissionItemState(r,it),lock=state.inherited,edited=!!d;return `<tr class="commission-product-row ${d?'changed':''}" style="background:#f3f7fc"><td><input type="checkbox" ${state.enabled?'checked':''} ${lock?'disabled title="整单佣金：产品随整单参与"':''} onchange="stageCommissionItem(${it.order_item_id},'is_commission_enabled',this.checked?1:0)">${d?'<b style="color:#d97706">●</b>':''}</td><td style="padding-left:22px;color:#2563eb">${it.image?`<img src="${esc(it.image)}" style="width:28px;height:28px;object-fit:contain;vertical-align:middle">`:''} ↳ ${Number(it.item_index||0)}</td><td><b>${esc(it.product_model||'-')}</b><br><small>${esc(it.customer_model||'-')}</small></td><td title="${esc(it.product_name||'')}">${esc(it.product_name||'-')}</td><td>${esc(it.color||'-')}</td><td>${esc(r.order_currency)} ${money(it.product_amount)}</td><td>${fmtNum(it.qty)}</td><td>${money(it.unit_price)}</td><td>${state.label}</td><td>${commissionItemInput(it,'target_name','','text',lock)}</td><td>${commissionItemInput(it,'target_type','target_type','text',lock)}</td><td>${commissionItemInput(it,'commission_mode','commission_mode','text',lock)}</td><td>${commissionItemInput(it,'commission_value','','number',lock)}</td><td>${commissionItemInput(it,'calc_base','calc_base','text',lock)}</td><td>${commissionItemInput(it,'currency','currency','text',lock)}</td><td>${lock?'随整单计算':((state.enabled||edited)&&estimate!==null?money(estimate):(edited?'毛利数据不足':'0.00'))}</td><td>${commissionEffectSelect(it.order_item_id,commissionItemValue(it,'receivable_effect')||'none',true,lock)}</td><td>-</td><td>${commissionItemInput(it,'settle_status','settle_status','text',lock)}</td><td>${commissionItemInput(it,'note','','text',lock)}</td></tr>`}).join('')}
function renderCommissionOrderRows(){let box=$('commissionOrderRows');if(!box)return;let selected=new Set([...document.querySelectorAll('.commission-order-check:checked')].map(x=>Number(x.value))),label=commissionOrderSourceLabel();box.innerHTML=COMMISSION_ORDER_ROWS.map(r=>{let estimate=commissionOrderEstimate(r),d=COMMISSION_ORDER_DRAFTS[r.order_id],expanded=COMMISSION_ORDER_EXPANDED.has(Number(r.order_id));return `<tr class="${d?'changed':''}"><td><button class="gray" style="padding:3px 7px" onclick="toggleCommissionOrderExpand(${r.order_id})">${expanded?'−':'+'}</button> <input class="commission-order-check" type="checkbox" value="${r.order_id}" ${selected.has(Number(r.order_id))?'checked':''}>${d?'<b style="color:#d97706">●</b>':''}</td><td><b>${esc(r.order_no)}</b><br><button class="gray" style="padding:2px 6px" onclick="openCommissionHistory(${r.order_id})">历史</button></td><td>${esc(r.quote_no)}</td><td>${esc(r.customer_name)}</td><td>${esc(r.user_name||'-')}</td><td>${esc(r.order_currency)} ${money(r.order_amount)}</td><td>${fmtNum(r.total_qty)}</td><td>${esc(r.payment_status)}${r.shipment_status?'<br><small>'+esc(r.shipment_status)+'</small>':''}</td><td>${commissionScopeSelect(r)}</td><td>${commissionOrderInput(r,'target_name')}</td><td>${commissionOrderInput(r,'target_type','target_type')}</td><td>${commissionOrderInput(r,'commission_mode','commission_mode')}</td><td>${commissionOrderInput(r,'commission_value','', 'number')}</td><td>${commissionOrderInput(r,'calc_base','calc_base')}</td><td>${commissionOrderInput(r,'currency','currency')}</td><td>${estimate===null?'毛利数据不足':money(estimate)}</td><td>${commissionOrderInput(r,'settled_amount','', 'number')}</td><td>${commissionEffectSelect(r.order_id,commissionOrderValue(r,'receivable_effect')||'none')}</td><td>${commissionOrderInput(r,'settle_status','settle_status')}</td><td>${commissionOrderInput(r,'note')}</td></tr>${renderCommissionItemRows(r)}`}).join('')||`<tr><td colspan="20" style="text-align:center;padding:40px">没有符合条件的${label}</td></tr>`;$('commissionOrderStats').textContent=`${label}修改 ${Object.keys(COMMISSION_ORDER_DRAFTS).length} 行 ｜ 产品修改 ${Object.keys(COMMISSION_ITEM_DRAFTS).length} 行 ｜ 当前页 ${COMMISSION_ORDER_ROWS.length} 条 ｜ 总计 ${COMMISSION_ORDER_TOTAL} 条`}
