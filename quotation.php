...
let COMMISSION_ROWS=[],COMMISSION_OPTIONS=[],COMMISSION_DRAFTS={},COMMISSION_PAGE=1,COMMISSION_PAGE_SIZE=50,COMMISSION_TOTAL=0,COMMISSION_PAGES=1,COMMISSION_TIMER=null;
...
async function loadCommissionRules(){renderCommissionRulesLoading();try{let d=await api('commission_rule_list&'+commissionQuery());if(!COMMISSION_OPTIONS.length)COMMISSION_OPTIONS=Array.isArray(d.options)?d.options:[];COMMISSION_ROWS=Array.isArray(d.list)?d.list:[];COMMISSION_TOTAL=Number(d.total||0);COMMISSION_PAGE=Number(d.page||1);COMMISSION_PAGES=Number(d.total_pages||1);refreshCommissionFilters();renderCommissionHead();renderCommissionRows();renderCommissionPager()}catch(e){renderCommissionRulesLoading('读取佣金规则失败：'+(e?.message||'请刷新后重试'));console.error('loadCommissionRules failed',e)}}
...
async function loadCommissionOrders(){
  ...
  const token=++COMMISSION_ORDER_LOAD_TOKEN,rows=$('commissionOrderRows'),stats=$('commissionOrderStats'),wrap=rows?.closest('.commission-table-wrap'),requestedPage=Math.max(1,Number(COMMISSION_ORDER_PAGE||1));
  ...
  const optionsRequest=COMMISSION_OPTIONS.length?Promise.resolve(null):api('commission_options_list');
  ...
  try{
    let [options,data]=await Promise.all([optionsRequest,dataRequest]);if(token!==COMMISSION_ORDER_LOAD_TOKEN)return;
    ...
    renderCommissionOrderRows();renderCommissionOrderPager();if(stats)stats.textContent=`已显示 ${commissionOrderSourceLabel()}；当前页 ${COMMISSION_ORDER_ROWS.length} 条，共 ${COMMISSION_ORDER_TOTAL} 条`;
  }catch(e){
