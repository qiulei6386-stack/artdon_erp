...
let QUOTE_DETAILS_PROMISE=null;
...
loadQuote=function(id){
  let q=(DB.quotes||[]).find(x=>String(x.id)===String(id));
  if(q&&Number(q._detail_loaded||0)===1){rememberOpenedContext('quote',id);return quoteLoadImmediate(id);}
  // 先锁定用户刚点击的报价ID，避免异步明细加载期间沿用上一张报价导出。
  if(q){rememberOpenedContext('quote',id);S.currentQuoteId=Number(q.id||0);S.currentApprovalStatus=quoteApprovalStatus(q);if($('quoteNo'))$('quoteNo').value=q.quote_no||'';updateQuoteApprovalStrip(q);}
  return ensureQuoteDetail(id).then(()=>quoteLoadImmediate(id));
};
