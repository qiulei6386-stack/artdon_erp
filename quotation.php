...
<div class="commission-actions" style="flex-basis:40px;border-bottom:1px solid #e5e7eb"><button id="commissionRuleTab" class="blue" onclick="setCommissionView('rules')">佣金规则</button><button id="commissionOrderTab" class="gray" onclick="setCommissionView('orders')">报价 / 订单佣金</button><button id="commissionReminderTab" class="gray" onclick="setCommissionView('reminders')">提醒设置</button><span class="hint">内部核算，不进入任何对外单证</span></div>
...
function setCommissionView(view){COMMISSION_VIEW=view;let body=$('commissionBody'),orders=view==='orders',reminders=view==='reminders',rules=view==='rules',ruleFilters=body.querySelector(':scope > .commission-filters'),ruleActions=body.querySelectorAll(':scope > .commission-actions')[1],ruleTable=body.querySelector(':scope > .commission-table-wrap'),rulePager=body.querySelector(':scope > .commission-pager');if(ruleFilters)ruleFilters.style.display=rules?'':'none';if(ruleActions)ruleActions.style.display=rules?'':'none';if(ruleTable)ruleTable.style.display=rules?'':'none';if(rulePager)rulePager.style.display=rules?'':'none';$('commissionOrderArea').style.display=orders?'flex':'none';$('commissionOrderArea').style.flexDirection='column';$('commissionReminderArea').style.display=reminders?'flex':'none';$('commissionReminderArea').style.flexDirection='column';$('commissionRuleTab').className=rules?'blue':'gray';$('commissionOrderTab').className=orders?'blue':'gray';$('commissionReminderTab').className=reminders?'blue':'gray';if(orders){ensureCommissionOrderArea();loadCommissionOrders()}else if(reminders){ensureCommissionReminderArea();loadCommissionReminders()}else loadCommissionRules()}
