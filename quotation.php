...
async function loadCommissionRules(){let [d,o]=await Promise.all([api('commission_rule_list&'+commissionQuery()),COMMISSION_OPTIONS.length?Promise.resolve(null):api('commission_options_list')]);if(o)COMMISSION_OPTIONS=o.options||[];COMMISSION_ROWS=d.list||[];COMMISSION_TOTAL=Number(d.total||0);COMMISSION_PAGE=Number(d.page||1);COMMISSION_PAGES=Number(d.total_pages||1);refreshCommissionFilters();renderCommissionHead();renderCommissionRows();renderCommissionPager()}
