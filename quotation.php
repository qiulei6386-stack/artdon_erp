...
<div class="system-nav no-print"><a href="index.php">首页</a><a href="crm.php">CRM</a><a href="mail.php">邮箱</a><a href="promotion.php">推广</a><a class="active" href="quotation.php">报价</a><a href="datasheet.php">资料</a><a href="bom.php">BOM</a><a href="dispatch_next.php">派工</a><a href="naming.php">命名系统</a><a href="crm.php#linkage">PLM</a><a href="crm.php#linkage">材料重量</a></div>
...
function partDefsByType(t,p){let need=p?(p.need_connector==='yes'||(p.need_connector==='auto'&&(p.type==='track'||p.type==='magnetic'))):(t==='track'||t==='magnetic');let a=[['led','芯片','芯片,COB,LED'],['driver','电源','电源,Driver,驱动'],['optic','光学','光学,透镜,反光杯,Reflector']];if(need)a.splice(1,0,['connector','接头','接头,连接器']);a.push(['extra','附件','附件,附加,配件,蜂窝,线材']);return a}
...
function matText(m){return [m.category,m.brand,m.name,m.model,m.spec,m.supplier,m.unit].join(' ').toLowerCase()}function materialBase(cats){let arr=String(cats||'').toLowerCase().split(',').map(x=>x.trim()).filter(Boolean);return DB.materials.filter(m=>arr.some(c=>matText(m).includes(c)))}function filterMat(cats,kw=''){kw=String(kw||'').toLowerCase();return materialBase(cats).filter(m=>!kw||matText(m).includes(kw))}function matName(m){return quoteBrandModelOnly(m)||'物料'}
...
function partModalBase(){return materialBase(partModalState.cats)}
