...
function paymentSummaryHtml(o,summary,payments){summary=summary||{};payments=payments||[];let cur=summary.currency||o.currency||'USD';let paid=num(summary.paid_amount),deduct=num(summary.commission_deduct_amount),bal=num(summary.balance_amount),amount=num(summary.order_amount||o.amount);let status=summary.payment_status||o.payment_status||'未收款';let rows=payments.map(p=>`<tr><td>${esc(p.payment_date||'')}</td><td>${esc(p.payment_type||'')}</td><td>${esc(p.currency||cur)} ${money(p.amount)}</td><td>${num(p.commission_deduct_amount)?esc(cur)+' '+money(p.commission_deduct_amount):'-'}</td><td>${esc(p.method||'')}</td><td>${esc(p.bank_ref||'')}</td><td>${esc(p.commission_deduct_note||p.note||'')}</td><td><button class="red" onclick="deleteOrderPayment(${Number(p.id)},${Number(o.id)})">删除</button></td></tr>`).join('');return `<div class="payment-panel"><div class="payment-panel-head"><div><b>收款记录 / Payment</b><div class="hint">实际到账与佣金抵扣分开核算；佣金页设置“货款扣佣”或收款时确认抵扣，都会减少应收。</div></div><button class="blue" onclick="openPaymentModal(${Number(o.id)})">新增收款</button></div><div class="payment-mini-grid"><div><b>订单金额</b><span>${esc(cur)} ${money(amount)}</span></div><div><b>实际已收</b><span class="payment-status-paid">${esc(cur)} ${money(paid)}</span></div><div><b>佣金抵扣</b><span>${esc(cur)} ${money(deduct)}</span></div><div><b>未收款</b><span class="${bal>0?'payment-status-warn':'payment-status-paid'}">${esc(cur)} ${money(bal)}</span></div><div><b>付款状态</b><span class="${paymentCls(status)}">${esc(status)}</span></div></div>${bal>0?'<div class="payment-alert">系统自动抵扣会显示为“佣金抵扣”流水；取消货款扣佣后会自动撤回系统抵扣。</div>':''}<div class="payment-list"><table><tr><th>日期</th><th>类型</th><th>实际到账</th><th>佣金抵扣</th><th>方式</th><th>水单/参考号</th><th>备注</th><th>操作</th></tr>${rows||'<tr><td colspan="8">暂无收款记录</td></tr>'}</table></div></div>`}
...
async function openPaymentModal(orderId){let o=CURRENT_ORDER&&Number(CURRENT_ORDER.id)===Number(orderId)?CURRENT_ORDER:(DB.orders||[]).find(x=>Number(x.id)===Number(orderId));if(!o){alert('请先选择订单');return;}$('payOrderId').value=orderId;$('payOrderInfo').textContent='订单：'+(quoteOrderNoAtV68522(o.order_no,o.quote_no)||'')+' ｜ 客户：'+(o.customer_name||'')+' ｜ 金额：'+(o.currency||'USD')+' '+money(o.amount);$('payType').value='订金';$('payDate').value=today();$('payAmount').value='';$('payCurrency').value=o.currency||'USD';$('payMethod').value='';$('payBankRef').value='';$('payNote').value='';$('payCommissionDeductEnabled').checked=false;$('payCommissionDeductAmount').value='';$('payCommissionDeductReason').value='';$('payCommissionDeductNote').value='';$('payCommissionSnapshotId').value='';$('payCommissionDeductFields').style.display='none';$('payCommissionDeductBox').style.display='none';$('payCommissionNotice').style.display='none';$('paymentModal').classList.add('show');try{let c=await orderApi('commission_payment_info',{order_id:orderId}),s=(c.snapshots||[])[0];if(c.has_commission){let effect=s?.receivable_effect||'none',effectText={none:'不影响应收',deduct_from_payment:'从客户货款中扣佣',pending_confirm:'待确认'}[effect]||effect;$('payCommissionNotice').style.display='block';$('payCommissionNotice').innerHTML=`<b>此订单有关联佣金</b><br>佣金对象：${esc(s?.target_name||'-')} ｜ 预计佣金：${esc(s?.currency||o.currency||'USD')} ${money(c.estimated_commission)} ｜ 佣金方式：${esc(effectText)} ｜ 结算节点：${esc(commissionOptName('settle_node',s?.settle_node||''))}`;if(effect==='deduct_from_payment'&&s){$('payCommissionDeductBox').style.display='block';$('payCommissionSnapshotId').value=s.id;$('payCommissionDeductAmount').value=Math.max(0,num(s.commission_amount)-num(s.deduct_amount)).toFixed(2)}}}catch(e){$('payCommissionNotice').style.display='block';$('payCommissionNotice').textContent='佣金提醒读取失败：'+e.message}}
...
async function saveOrderPayment(){let orderId=Number($('payOrderId')?.value||0);let amount=num($('payAmount')?.value),deductOn=!!$('payCommissionDeductEnabled')?.checked,deduct=deductOn?num($('payCommissionDeductAmount')?.value):0;if(!orderId){alert('缺少订单ID');return;}if(amount<=0){alert('请填写实际到账金额');return;}if(deductOn&&deduct<=0){alert('请填写佣金抵扣金额');return;}try{let r=await orderApi('save_payment',{order_id:orderId,payment_type:$('payType').value,payment_date:$('payDate').value,amount:amount,currency:$('payCurrency').value,method:$('payMethod').value,bank_ref:$('payBankRef').value,note:$('payNote').value,commission_deduct_amount:deduct,commission_deduct_snapshot_id:deductOn?Number($('payCommissionSnapshotId').value||0):0,commission_deduct_reason:deductOn?$('payCommissionDeductReason').value:'',commission_deduct_note:deductOn?$('payCommissionDeductNote').value:''});closePaymentModal();await refreshQuoteRuntime('save_order_payment',{orders:true,documents:true});await viewOrder(orderId);alert('收款已保存：实际到账 '+money(r.paid_amount||0)+'，佣金抵扣 '+money(r.commission_deduct_amount||0)+'，剩余应收 '+money(r.balance_amount||0));}catch(e){alert('保存收款失败：'+e.message)}}
