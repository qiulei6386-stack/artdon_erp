...
async function addCommissionRule(){let name=prompt('佣金对象名称：','');if(name===null||!name.trim())return;await api('commission_rule_save',{rule_name:name.trim(),target_name:name.trim(),target_type:'sales',commission_mode:'percent',commission_value:0,currency:'USD',calc_base:'order_amount',settle_node:'payment_received',settle_status:'unsettled',apply_scope:'all',is_active:1});loadCommissionRules()}
