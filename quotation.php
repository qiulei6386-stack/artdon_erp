...
renderOrderDetail=function(...args){renderOrderDetailBase(...args);let o=args[0]||{};let bar=$('orderDetail')?.querySelector('.card-head .btns');if(bar&&!bar.querySelector('[data-combined-ship]')){let b=document.createElement('button');b.className='blue';b.dataset.combinedShip='1';b.textContent='合并出货';b.onclick=function(){openCombinedShipmentModal(Number(o.id||0));};let pay=[...bar.querySelectorAll('button')].find(x=>x.textContent==='新增收款');bar.insertBefore(b,pay||bar.children[2]||null);}shipmentCardActions();};
