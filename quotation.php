...
function setCommissionScope(id,scope){id=Number(id);if(scope==='line'||scope==='mixed'){COMMISSION_ORDER_EXPANDED.add(id);sessionStorage.setItem('artdon_commission_expanded_orders',JSON.stringify([...COMMISSION_ORDER_EXPANDED]))}stageCommissionOrder(id,'commission_scope',scope)}
...
function commissionItemState(r,it){let scope=commissionOrderValue(r,'commission_scope')||r.commission_scope||'order';if(scope==='order')return {inherited:true,enabled:true,label:'随订单参与'};let configured=Number(commissionItemValue(it,'commission_configured')??it.commission_configured??0)===1||['target_name','target_type','commission_mode','commission_value','calc_base','currency','receivable_effect','settle_status'].some(f=>String(commissionItemValue(it,f)??'').trim()!=='');let enabledValue=commissionItemValue(it,'is_commission_enabled');let enabled=configured&&(enabledValue===''||enabledValue===null||enabledValue===undefined)?true:Number(enabledValue??0)===1;if(!configured)return {inherited:false,enabled:false,label:'填写即参与'};return {inherited:false,enabled,label:enabled?'参与佣金':'不参与'}}
