...
function commissionSummaryRow(c){return `<tr><td>${esc(c.currency)}</td><td>${c.orders}</td><td>${commissionSummaryMoney(c.currency,c.amount)}</td><td style="color:#059669">${commissionSummaryMoney(c.currency,c.paid)}</td><td style="color:#dc2626">${commissionSummaryMoney(c.currency,c.balance)}</td><td>${commissionSummaryMoney(c.currency,c.expected)}</td><td>${commissionSummaryMoney(c.currency,c.settled)}</td><td>${commissionSummaryMoney(c.currency,Math.max(0,c.expected-c.settled))}</td></tr>`}
function renderCommissionSummaryPage(list,total){let rows=(Array.isArray(list)?list:[]).filter(r=>{let t=commissionSummaryTotals(r);return t.expected>0||t.settled>0}),groups={},currencies={};rows.forEach(r=>{let t=commissionSummaryTotals(r),code=commissionCustomerCode(r),name=String(r.customer_name||'未命名客户'),key=[name,code,t.currency].join('|'),g=groups[key]||(groups[key]={name,code,...t,orders:0});g.orders++;g.amount+=t.amount;g.paid+=t.paid;g.balance+=t.balance;g.expected+=t.expected;g.settled+=t.settled;let c=currencies[t.currency]||(currencies[t.currency]={...t,orders:0});c.orders++;c.amount+=t.amount;c.paid+=t.paid;c.balance+=t.balance;c.expected+=t.expected;c.settled+=t.settled;});$('commissionSummaryStats').textContent=`显示 ${rows.length} 条有佣金订单${Number(total)>rows.length?` / 共 ${Number(total)} 条`:''}`;let currencyRows=Object.values(currencies).sort((a,b)=>a.currency.localeCompare(b.currency)).map(commissionSummaryRow).join('');let currencyBox=$('commissionSummaryCurrencyRows');if(currencyBox)currencyBox.innerHTML=currencyRows||'<tr><td colspan="8" style="text-align:center;padding:24px">当前筛选没有佣金订单</td></tr>';$('commissionSummaryCustomerRows').innerHTML=Object.values(groups).sort((a,b)=>b.expected-a.expected).map(g=>`<tr><td>${esc(g.name)}</td><td>${esc(g.code||'-')}</td><td>${esc(g.currency)}</td><td>${g.orders}</td><td>${commissionSummaryMoney(g.currency,g.amount)}</td><td style="color:#059669">${commissionSummaryMoney(g.currency,g.paid)}</td><td style="color:#dc2626">${commissionSummaryMoney(g.currency,g.balance)}</td><td>${commissionSummaryMoney(g.currency,g.expected)}</td><td>${commissionSummaryMoney(g.currency,g.settled)}</td><td>${commissionSummaryMoney(g.currency,Math.max(0,g.expected-g.settled))}</td></tr>`).join('')||'<tr><td colspan="10" style="text-align:center;padding:24px">当前筛选没有佣金订单</td></tr>';$('commissionSummaryOrderRows').innerHTML=rows.map(r=>{let t=commissionSummaryTotals(r);return `<tr><td><button class="blue" onclick="showPage('orders');setTimeout(()=>viewOrder(${Number(r.order_id)}),0)">${esc(r.order_no||'-')}</button></td><td>${esc(r.quote_no||'-')}</td><td>${esc(r.customer_name||'-')}</td><td>${esc(commissionCustomerCode(r)||'-')}</td><td>${esc(r.user_name||'-')}</td><td>${esc(String(r.order_date||r.order_created_at||'-').slice(0,10))}</td><td>${esc(t.currency)}</td><td>${commissionSummaryMoney(t.currency,t.amount)}</td><td style="color:#059669">${commissionSummaryMoney(t.currency,t.paid)}</td><td style="color:#dc2626">${commissionSummaryMoney(t.currency,t.balance)}</td><td>${commissionSummaryMoney(t.currency,t.expected)}</td><td>${esc(commissionSummarySettlementText(r.settle_status))}</td></tr>`}).join('')||'<tr><td colspan="12" style="text-align:center;padding:24px">当前筛选没有佣金订单</td></tr>'}
