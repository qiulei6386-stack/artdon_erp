...
let COMMISSION_VIEW='rules',COMMISSION_ORDER_ROWS=[],COMMISSION_ORDER_DRAFTS={},COMMISSION_ITEM_DRAFTS={},COMMISSION_ORDER_PAGE=1,COMMISSION_ORDER_SIZE=50,COMMISSION_ORDER_TOTAL=0,COMMISSION_ORDER_PAGES=1,COMMISSION_ORDER_TIMER=null;
...
async function loadCommissionOrders(){ensureCommissionOrderArea();let rows=$('commissionOrderRows'),stats=$('commissionOrderStats');if(rows)rows.innerHTML='<tr><td colspan="20" style="text-align:center;padding:40px;color:#64748b">正在读取报价与订单...</td></tr>';if(stats)stats.textContent='正在读取报价与订单...';try{if(!COMMISSION_OPTIONS.length){let options=await api('commission_options_list');COMMISSION_OPTIONS=Array.isArray(options.options)?options.options:[];refreshCommissionOrderOptions()}let payload={page:COMMISSION_ORDER_PAGE,page_size:COMMISSION_ORDER_SIZE,keyword:$('commissionOrderKeyword')?.value||'',settle_status:$('commissionOrderSettle')?.value||'',currency:$('commissionOrderCurrency')?.value||'',missing_only:Number($('commissionOrderMissing')?.value||0)},result=await Promise.all([orderApi('commission_order_list',payload),COMMISSION_ORDER_PAGE===1?api('commission_quote_list',{keyword:payload.keyword}):Promise.resolve({list:[],total:0})]),d=result[0],qd=result[1];COMMISSION_ORDER_ROWS=[...(qd.list||[]).slice(0,COMMISSION_ORDER_SIZE),...(Array.isArray(d.list)?d.list:[])];COMMISSION_ORDER_TOTAL=Number(d.total||0)+Number(qd.total||0);COMMISSION_ORDER_PAGE=Number(d.page||COMMISSION_ORDER_PAGE);COMMISSION_ORDER_PAGES=Math.max(1,Math.ceil(COMMISSION_ORDER_TOTAL/COMMISSION_ORDER_SIZE))}catch(e){COMMISSION_ORDER_ROWS=[];COMMISSION_ORDER_TOTAL=0;if(rows)rows.innerHTML=`<tr><td colspan="20" style="text-align:center;padding:40px;color:#dc2626">报价/订单读取失败：${esc(e.message||'未知错误')}　<button class="gray" onclick="loadCommissionOrders()">重新加载</button></td></tr>`;if(stats)stats.textContent='报价/订单读取失败';if($('commissionOrderPager'))$('commissionOrderPager').innerHTML='';return}renderCommissionOrderRows();$('commissionOrderPager').innerHTML=`<span>共 ${COMMISSION_ORDER_TOTAL} 条（含报价与订单）</span><button ${COMMISSION_ORDER_PAGE<=1?'disabled':''} onclick="commissionOrderGo(${COMMISSION_ORDER_PAGE-1})">上一页</button><span>第 ${COMMISSION_ORDER_PAGE} / ${COMMISSION_ORDER_PAGES} 页</span><button ${COMMISSION_ORDER_PAGE>=COMMISSION_ORDER_PAGES?'disabled':''} onclick="commissionOrderGo(${COMMISSION_ORDER_PAGE+1})">下一页</button><span class="spacer"></span><select style="width:80px" onchange="COMMISSION_ORDER_SIZE=Number(this.value);COMMISSION_ORDER_PAGE=1;loadCommissionOrders()"><option ${COMMISSION_ORDER_SIZE===50?'selected':''}>50</option><option ${COMMISSION_ORDER_SIZE===100?'selected':''}>100</option><option ${COMMISSION_ORDER_SIZE===200?'selected':''}>200</option></select>`}
