...
function commissionScopeSelect(r){let v=commissionOrderValue(r,'commission_scope')||'order';return `<select class="commission-edit" onchange="stageCommissionOrder(${r.order_id},'commission_scope',this.value)"><option value="order" ${v==='order'?'selected':''}>整单</option><option value="line" ${v==='line'?'selected':''}>明细</option><option value="mixed" ${v==='mixed'?'selected':''}>混合</option></select>`}
...
function commissionOrderInput(r,f,group='',type='text'){let v=commissionOrderValue(r,f);if(group)return `<select class="commission-edit" onchange="stageCommissionOrder(${r.order_id},'${f}',this.value)">${commissionSelect(group,v)}</select>`;return `<input class="commission-edit" type="${type}" ${type==='number'?'min="0" step="0.01"':''} value="${esc(v)}" oninput="stageCommissionOrder(${r.order_id},'${f}',this.value)">`}
...
function stageCommissionItem(id,f,v){id=Number(id);if(['commission_value','settled_amount','is_commission_enabled'].includes(f))v=Number(v||0);if(!COMMISSION_ITEM_DRAFTS[id])COMMISSION_ITEM_DRAFTS[id]={order_item_id:id};COMMISSION_ITEM_DRAFTS[id][f]=v;if(f==='is_commission_enabled')COMMISSION_ITEM_DRAFTS[id].commission_configured=1;renderCommissionOrderRows()}
function commissionItemInput(it,f,group='',type='text',disabled=false){let v=commissionItemValue(it,f),lock=disabled?' disabled title="整单佣金无需逐项设置"':'';if(group)return `<select class="commission-edit"${lock} onchange="stageCommissionItem(${it.order_item_id},'${f}',this.value)">${commissionSelect(group,v)}</select>`;return `<input class="commission-edit"${lock} type="${type}" ${type==='number'?'min="0" step="0.01"':''} value="${esc(v)}" oninput="stageCommissionItem(${it.order_item_id},'${f}',this.value)">`}
...
function commissionItemState(r,it){let scope=commissionOrderValue(r,'commission_scope')||r.commission_scope||'order';if(scope==='order')return {inherited:true,enabled:true,label:'随整单参与'};let configured=Number(commissionItemValue(it,'commission_configured')??it.commission_configured??0)===1,enabled=Number(commissionItemValue(it,'is_commission_enabled')??0)===1;if(!configured)return {inherited:false,enabled:false,label:'未设置'};return {inherited:false,enabled,label:enabled?'参与佣金':'不参与'}}
