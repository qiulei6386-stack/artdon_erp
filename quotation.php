...
function stageCommissionItem(id,f,v){id=Number(id);if(['commission_value','settled_amount','is_commission_enabled'].includes(f))v=Number(v||0);let base=commissionItemBaseValue(id,f);if(commissionValueEqual(v,base)){if(COMMISSION_ITEM_DRAFTS[id])delete COMMISSION_ITEM_DRAFTS[id][f]}else{if(!COMMISSION_ITEM_DRAFTS[id])COMMISSION_ITEM_DRAFTS[id]={order_item_id:id};COMMISSION_ITEM_DRAFTS[id][f]=v;if(['is_commission_enabled','target_name','target_type','commission_mode','commission_value','calc_base','currency','receivable_effect','settle_status'].includes(f))COMMISSION_ITEM_DRAFTS[id].commission_configured=1}pruneCommissionItemDraft(id);renderCommissionOrderRows()}
