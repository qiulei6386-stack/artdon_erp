...
.commission-table td[data-commission-col="target_name"],.commission-table td[data-commission-col="note"]{padding:3px 5px}.commission-table .commission-target-edit{display:block;width:100%;height:30px;padding:5px 7px;border:1px solid #cbd5e1!important;border-radius:6px;background:#fff!important;color:#172033;font-weight:800}.commission-table .commission-note-edit{display:block;width:100%;height:30px;min-height:30px;max-height:82px;padding:5px 7px;border:1px solid #cbd5e1!important;border-radius:6px;background:#fff!important;color:#172033;line-height:18px;white-space:normal;resize:vertical;overflow:auto}.commission-table .commission-target-edit::placeholder,.commission-table .commission-note-edit::placeholder{color:#94a3b8}.commission-table .commission-target-edit:focus,.commission-table .commission-note-edit:focus{border-color:#2563eb!important;box-shadow:0 0 0 2px rgba(37,99,235,.12);outline:none}
...
function commissionEdit(r,f,group='',type='text'){let v=commissionValue(r,f);if(group)return `<select class="commission-edit" onchange="stageCommission(${r.id},'${f}',this.value)">${commissionSelect(group,v)}</select>`;if(f==='target_name')return `<input class="commission-edit commission-target-edit" type="text" placeholder="输入佣金对象" value="${esc(v??'')}" oninput="stageCommissionSoft(${r.id},'${f}',this.value,this)">`;if(f==='note')return `<textarea class="commission-edit commission-note-edit" rows="1" placeholder="输入备注" oninput="stageCommissionSoft(${r.id},'${f}',this.value,this)">${esc(v??'')}</textarea>`;let handler=type==='text'?`stageCommissionSoft(${r.id},'${f}',this.value,this)`:`stageCommission(${r.id},'${f}',this.value)`;return `<input class="commission-edit" type="${type}" ${type==='number'?'min="0" step="0.01"':''} value="${esc(v??'')}" oninput="${handler}">`}
