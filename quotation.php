...
let COMMISSION_LOG_ROWS=[],COMMISSION_LOG_TIMER=null;
const COMMISSION_LOG_ACTION_LABELS={commission_order_save:'保存订单佣金',commission_quote_save:'保存报价佣金',commission_quote_lines_save:'保存报价产品佣金',commission_item_save:'保存订单产品佣金',commission_rule_save:'保存佣金规则',commission_rule_delete:'删除佣金规则',commission_rule_import:'导入佣金规则',commission_rule_export:'导出佣金规则',commission_reminder_save:'保存佣金提醒',commission_reminder_toggle:'切换佣金提醒',commission_customer_reminder_triggered:'触发客户佣金提醒',quote_commission_confirmation_saved:'保存报价佣金确认'};
function ensureCommissionLogArea(){
  let box=$('commissionLogArea');if(!box||box.dataset.ready)return;box.dataset.ready='1';
  box.innerHTML=`<div class="commission-filters"><div class="commission-search"><label>搜索</label><input id="commissionLogKw" placeholder="单号 / 客户 / 佣金对象 / 操作人 / 摘要" oninput="commissionLogFilter(true)"></div><div><label>日志类型</label><select id="commissionLogAction" onchange="commissionLogFilter()"><option value="">全部佣金日志</option><option value="commission_order_save">订单佣金</option><option value="commission_quote_save">报价佣金</option><option value="commission_quote_lines_save">报价产品佣金</option><option value="commission_item_save">订单产品佣金</option><option value="commission_rule_save">佣金规则</option><option value="commission_reminder">提醒设置</option></select></div><div><label>开始日期</label><input id="commissionLogDateFrom" type="date" onchange="commissionLogFilter()"></div><div><label>结束日期</label><input id="commissionLogDateTo" type="date" onchange="commissionLogFilter()"></div><div><label>显示</label><select id="commissionLogLimit" onchange="commissionLogFilter()"><option value="100">100条</option><option value="200" selected>200条</option><option value="500">500条</option></select></div></div><div class="commission-actions"><button class="blue" onclick="loadCommissionLogs()">刷新佣金日志</button><span id="commissionLogStats" class="hint">记录佣金是谁添加、什么时候添加、后续修改和提醒配置。</span></div><div class="commission-table-wrap"><table class="table commission-table commission-log-table" style="min-width:1180px"><thead><tr><th>时间</th><th>类型</th><th>单号</th><th>客户 / 产品</th><th>佣金对象</th><th>操作人</th><th>摘要</th><th>明细</th></tr></thead><tbody id="commissionLogRows"></tbody></table></div>`;
}
...
function commissionLogObject(l){
  let after=commissionLogJson(l.after_json)||{},before=commissionLogJson(l.before_json)||{},detail=commissionLogJson(l.detail_json)||{};
  return after.product_model||before.product_model||detail.product_model||l.customer_name||'-';
}
...
async function loadCommissionLogs(){
  ...
  q.set('limit',$('commissionLogLimit')?.value||200);
  let box=$('commissionLogRows');if(box)box.innerHTML='<tr><td colspan="8" style="text-align:center;padding:24px">正在读取佣金日志...</td></tr>';
  try{
    let d=await api('commission_log_list&'+q.toString());
    COMMISSION_LOG_ROWS=d.logs||[];
    if($('commissionLogStats'))$('commissionLogStats').textContent=`共 ${Number(d.total||0)} 条，当前显示 ${COMMISSION_LOG_ROWS.length} 条`;
    box.innerHTML=COMMISSION_LOG_ROWS.map(l=>`<tr><td>${esc(l.created_at||'')}</td><td>${esc(commissionLogActionName(l.action))}</td><td>${esc(commissionLogDocNo(l))}</td><td>${esc(commissionLogObject(l))}</td><td>${esc(commissionLogTarget(l))}</td><td>${esc(l.user_name||'-')}</td><td>${esc(l.summary||'')}</td><td><details><summary>查看</summary><pre>修改前：\n${esc(commissionLogPretty(l.before_json))}\n\n修改后：\n${esc(commissionLogPretty(l.after_json))}\n\n详情：\n${esc(commissionLogPretty(l.detail_json))}</pre></details></td></tr>`).join('')||'<tr><td colspan="8" style="text-align:center;padding:28px;color:#64748b">暂无佣金日志</td></tr>';
  }catch(e){if(box)box.innerHTML=`<tr><td colspan="8" style="color:#dc2626;padding:24px">读取失败：${esc(e.message||e)}</td></tr>`}
}
