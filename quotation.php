...
function stageCommission(id,f,v){id=Number(id);let r=COMMISSION_ROWS.find(x=>Number(x.id)===id);if(!r)return;if(['commission_value','settled_amount','is_active'].includes(f))v=Number(v||0);if(String(v??'')===String(r[f]??'')){if(COMMISSION_DRAFTS[id]){delete COMMISSION_DRAFTS[id][f];if(Object.keys(COMMISSION_DRAFTS[id]).length===1)delete COMMISSION_DRAFTS[id]}}else{if(!COMMISSION_DRAFTS[id])COMMISSION_DRAFTS[id]={id};COMMISSION_DRAFTS[id][f]=v}renderCommissionRows()}
function commissionEdit(r,f,group='',type='text'){let v=commissionValue(r,f);if(group)return `<select class="commission-edit" onchange="stageCommission(${r.id},'${f}',this.value)">${commissionSelect(group,v)}</select>`;return `<input class="commission-edit" type="${type}" ${type==='number'?'min="0" step="0.01"':''} value="${esc(v??'')}" oninput="stageCommission(${r.id},'${f}',this.value)">`}
