...
function recalcShipmentTotals(){let cartons=collectCartons();let items=collectShipmentItems();let qty=0,ctn=0,nw=0,gw=0,cbm=0;items.forEach(x=>{qty+=num(x.qty);ctn+=num(x.cartons);nw+=num(x.nw);gw+=num(x.gw);cbm+=num(x.cbm)});cartons.forEach(x=>{qty+=num(x.qty);ctn+=num(x.carton_count)||1;nw+=num(x.nw);gw+=num(x.gw);cbm+=num(x.cbm)});if($('shipSummary'))$('shipSummary').textContent=`合计：${fmtNum(qty)} PCS ｜ ${fmtNum(ctn)} CTNS ｜ N.W. ${fmtNum(nw,3)} KG ｜ G.W. ${fmtNum(gw,3)} KG ｜ CBM ${fmtNum(cbm,4)}`;}
