...
function commissionEdit(r,f,group='',type='text'){let v=commissionValue(r,f);if(group)return `<select class="commission-edit" onchange="stageCommission(${r.id},'${f}',this.value)">${commissionSelect(group,v)}</select>`;if(f==='note')return `<textarea class="commission-edit commission-note-edit" rows="1" placeholder="输入备注" oninput="stageCommissionSoft(${r.id},'${f}',this.value,this)">${esc(v??'')}</textarea>`;let handler=type==='text'?`stageCommissionSoft(${r.id},'${f}',this.value,this)`:`stageCommission(${r.id},'${f}',this.value)`;return `<input class="commission-edit" type="${type}" ${type==='number'?'min="0" step="0.01"':''} value="${esc(v??'')}" oninput="${handler}">`}
