...
function ensureCommissionLogArea(){
  ...
  box.innerHTML=`<div class="commission-log-shell">
    <div class="commission-log-hero"><div><span class="commission-log-kicker">Audit Trail</span><b>佣金操作日志</b><p>记录佣金是谁添加、什么时候添加、改了哪些关键字段；列表类浏览动作默认不显示。</p></div><div class="commission-log-metrics"><span><b id="commissionLogTotal">0</b><small>业务日志</small></span><span><b id="commissionLogShown">0</b><small>当前显示</small></span><span><b id="commissionLogLastActor">-</b><small>最近操作人</small></span></div></div>
    ...
    <div class="commission-log-grid"><div id="commissionLogRows" class="commission-log-list"></div><aside id="commissionLogDetail" class="commission-log-side"></aside></div>
  </div>`;
}
...
function commissionLogDiffRows(l){
  ...
  return '<div class="commission-log-diff header"><span>字段</span><span>修改前</span><span>修改后</span></div>'+changed.map(k=>`<div class="commission-log-diff"><span>${esc(COMMISSION_LOG_FIELD_LABELS[k]||k)}</span><span>${esc(commissionLogValue(commissionLogRead(before,k)))}</span><span><b>${esc(commissionLogValue(commissionLogRead(after,k)))}</b></span></div>`).join('');
}
...
function commissionLogCard(l,i){
  ...
  return `<button type="button" class="commission-log-card${active}" onclick="selectCommissionLog(${i})">
    <span class="commission-log-time">${esc(parts.date)}<br>${esc(parts.time)}</span>
    <span style="min-width:0"><span class="commission-log-title">${esc(commissionLogActionName(l.action))}</span><span class="commission-log-sub">${esc(commissionLogSummary(l))}</span><span class="commission-log-meta"><span class="commission-log-chip blue">${esc(commissionLogDocNo(l))}</span><span class="commission-log-chip">${esc(commissionLogObject(l))}</span><span class="commission-log-chip green">${esc(l.user_name||'未记录操作人')}</span></span></span>
