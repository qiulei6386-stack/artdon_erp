...
function quote_commission_quote_list($pdo,$d){
  ...
  $rows=rows($pdo,"SELECT id,quote_no,quote_date,customer_id,customer_name,user_name,amount,qty,currency,approval_status,status,items_json,commission_json,created_at FROM quote_orders".$where." ORDER BY COALESCE(quote_date,created_at) DESC,id DESC LIMIT ".$size." OFFSET ".$offset,$args);$list=[];
