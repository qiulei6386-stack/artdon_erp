...
function quote_commission_order_list($pdo,$d){
  ...
  if($orderIds){$marks=implode(',',array_fill(0,count($orderIds),'?'));$itemRows=rows($pdo,"SELECT i.id AS order_item_id,i.order_id,i.item_index,i.product_code AS product_model,i.product_name,i.qty,i.amount AS product_amount,i.unit_price,i.color,c.id AS item_commission_id,c.is_commission_enabled,c.target_name,c.target_type,c.commission_mode,c.commission_value,c.calc_base,c.base_amount,c.commission_amount,c.currency,c.settle_node,c.settle_status,c.settled_amount,c.note FROM quote_sales_order_items i LEFT JOIN quote_commission_item_snapshots c ON c.order_item_id=i.id WHERE i.order_id IN ($marks) ORDER BY i.order_id,i.item_index,i.id",$orderIds);foreach($itemRows as &$it){$it['commission_configured']=$it['item_commission_id']?1:0;$it['is_commission_enabled']=$it['item_commission_id']?(int)$it['is_commission_enabled']:0;}unset($it);foreach($itemRows as $it)$grouped[(int)$it['order_id']][]=$it;}
...
function quote_commission_quote_list($pdo,$d){
  ...
  $size=max(10,min(100,(int)($d['page_size']??50)));$page=max(1,(int)($d['page']??1));$total=(int)(row($pdo,'SELECT COUNT(*) c FROM quote_orders'.$where,$args)['c']??0);$pages=max(1,(int)ceil($total/$size));$page=min($page,$pages);$offset=($page-1)*$size;
  $rows=rows($pdo,"SELECT id,quote_no,quote_date,customer_id,customer_name,user_name,amount,qty,currency,approval_status,status,items_json,commission_json,converted_order_id,converted_order_no,created_at FROM quote_orders".$where." ORDER BY COALESCE(quote_date,created_at) DESC,id DESC LIMIT ".$size." OFFSET ".$offset,$args);$list=[];
  foreach($rows as $r){$c=json_decode((string)($r['commission_json']??''),true);if(!is_array($c))$c=[];$items=json_decode((string)($r['items_json']??'[]'),true);if(!is_array($items))$items=[];$scope=in_array(($c['commission_scope']??''),['order','line','mixed'],true)?$c['commission_scope']:(!empty($c['lines'])?'line':'order');$lineMap=[];foreach(($c['lines']??[]) as $x)$lineMap[(int)($x['item_index']??-1)]=$x;$children=[];
    foreach($items as $i=>$it){$p=$it['product']??[];$hasLine=array_key_exists($i,$lineMap);$line=$lineMap[$i]??[];$children[]=['order_item_id'=>-((int)$r['id']*1000+$i+1),'quote_item_index'=>$i,'order_id'=>-(int)$r['id'],'item_index'=>$i+1,'image'=>$p['image']??'','product_model'=>$p['code']??$p['model']??'','customer_model'=>$it['customer_code']??'','product_name'=>$p['name']??'','color'=>$it['color']??'','qty'=>(float)($it['qty']??0),'unit_price'=>(float)($it['price']??0),'product_amount'=>(float)($it['amount']??0),'commission_configured'=>$hasLine?1:0,'is_commission_enabled'=>$hasLine?(int)($line['is_commission_enabled']??1):0,'target_type'=>$line['target_type']??'other','target_name'=>$line['target_name']??'','commission_mode'=>$line['mode']??'percent','commission_value'=>$line['value']??0,'calc_base'=>'product_amount','base_amount'=>(float)($it['amount']??0),'commission_amount'=>(float)($line['estimated_amount']??0),'currency'=>$line['currency']??$r['currency'],'receivable_effect'=>$c['commission_receivable_effect']??'none','settle_status'=>'unsettled','note'=>$line['note']??''];}
    $convertedNo=trim((string)($r['converted_order_no']??''));$convertedId=(int)($r['converted_order_id']??0);$converted=$convertedNo!==''||$convertedId>0;
    $list[]=['entity_type'=>'quote','quote_id'=>(int)$r['id'],'order_id'=>-(int)$r['id'],'order_no'=>'报价单 · '.$r['quote_no'],'quote_no'=>$r['quote_no'],'customer_name'=>$r['customer_name'],'user_name'=>$r['user_name'],'order_amount'=>$r['amount'],'total_qty'=>$r['qty'],'order_currency'=>$r['currency'],'paid_amount'=>0,'payment_status'=>'报价单','shipment_status'=>$converted?('已转订单'.($convertedNo?' · '.$convertedNo:'')):'未转订单','order_status'=>$r['approval_status']?:$r['status'],'order_created_at'=>$r['created_at'],'converted_order_id'=>$convertedId,'converted_order_no'=>$convertedNo,'snapshot_id'=>0,'rule_id'=>$c['commission_rule_id']??0,'target_name'=>$c['commission_target_name']??'','target_type'=>'other','commission_mode'=>$c['commission_mode']??'percent','commission_value'=>$c['commission_value']??0,'calc_base'=>$c['commission_calc_base']??'order_amount','base_amount'=>$r['amount'],'commission_amount'=>$c['commission_estimated_amount']??0,'currency'=>$c['commission_currency']??$r['currency'],'settle_node'=>$c['commission_settle_node']??'manual','settle_status'=>$c['commission_confirm_status']??'unconfirmed','settled_amount'=>0,'commission_scope'=>$scope,'receivable_effect'=>$c['commission_receivable_effect']??'none','note'=>$c['commission_note']??'','items'=>$children];}
  return ['list'=>$list,'total'=>$total,'page'=>$page,'page_size'=>$size,'total_pages'=>$pages];
}
...
function quote_summary_attach_last_followup(PDO $pdo,array $data): array{
  $quoteIds=[];foreach($data as $r){$id=(int)($r['quote_id']??0);if($id>0)$quoteIds[$id]=$id;}
  if(!$quoteIds || !table_exists($pdo,'crm_quote_followup_activities')){foreach($data as &$r)$r['last_followup_at']='';unset($r);return $data;}
  $ids=array_values($quoteIds);$marks=implode(',',array_fill(0,count($ids),'?'));
  try{
    $activityRows=rows($pdo,"SELECT quote_id,contacted_at FROM crm_quote_followup_activities WHERE quote_source='legacy' AND deleted_at IS NULL AND quote_id IN ($marks) ORDER BY quote_id ASC,contacted_at DESC,id DESC",$ids);
  }catch(Throwable $e){$activityRows=[];}
  $latest=[];foreach($activityRows as $activity){$id=(int)($activity['quote_id']??0);if($id>0 && !array_key_exists($id,$latest))$latest[$id]=(string)($activity['contacted_at']??'');}
  foreach($data as &$r)$r['last_followup_at']=$latest[(int)($r['quote_id']??0)]??'';unset($r);
  return $data;
}
function quote_summary_list_data(PDO $pdo,array $f){
  $params=[];$base=quote_summary_filtered_sql($pdo,$f,$params);$field=['date'=>'quote_date','quote_no'=>'quote_no','order_no'=>'order_no','quote_date'=>'quote_date','order_date'=>'order_date','shipment_date'=>'shipment_date','payment_date'=>'payment_date','quote_amount'=>'quote_amount','order_amount'=>'order_amount','paid_amount'=>'paid_amount','customer'=>'customer_name','owner'=>'owner'][$f['sort_field']]??'quote_date';$order=strtoupper($f['sort_order'])==='ASC'?'ASC':'DESC';$count=(int)(row($pdo,'SELECT COUNT(*) c FROM ('.$base.') z',$params)['c']??0);$offset=($f['page']-1)*$f['page_size'];$data=rows($pdo,$base." ORDER BY `$field` $order, quote_id DESC LIMIT ".(int)$f['page_size'].' OFFSET '.(int)$offset,$params);foreach($data as &$r){$r['status_label']=quote_summary_status($r);$r['receivable_amount']=max(0,(float)$r['order_amount']-(float)$r['paid_amount']);$r['quote_currency']=quote_summary_currency($r['quote_currency']);$r['order_currency']=quote_summary_currency($r['order_currency']);}unset($r);$data=quote_summary_attach_last_followup($pdo,$data);return ['list'=>$data,'total'=>$count,'page'=>$f['page'],'page_size'=>$f['page_size'],'pages'=>max(1,(int)ceil($count/$f['page_size']))];}
function quote_summary_filter_options(PDO $pdo){
  $rows=rows($pdo,'SELECT user_name,customer_name,customer_json FROM quote_orders ORDER BY id DESC LIMIT 5000');$owners=[];$customers=[];$countries=[];foreach($rows as $r){$o=trim((string)($r['user_name']??''));$c=trim((string)($r['customer_name']??''));if($o!=='')$owners[$o]=1;if($c!=='')$customers[$c]=1;$j=json_decode((string)($r['customer_json']??''),true);$co=trim((string)($j['country']??''));if($co!=='')$countries[$co]=1;}ksort($owners);$owners=array_values(array_keys($owners));ksort($customers);$customers=array_values(array_keys($customers));ksort($countries);$countries=array_values(array_keys($countries));return ['owners'=>$owners,'customers'=>$customers,'countries'=>$countries];}
function quote_summary_excel_text($value){$value=str_replace(["\r\n","\r"],"\n",trim((string)$value));return preg_match('/^\s*[=+\-@]/u',$value)?"'".$value:$value;}
function quote_summary_export_excel(PDO $pdo,array $f){
  $params=[];$base=quote_summary_filtered_sql($pdo,$f,$params);$data=rows($pdo,$base.' ORDER BY quote_date DESC, quote_id DESC LIMIT 10000',$params);foreach($data as &$r){$r['status_label']=quote_summary_status($r);$r['receivable_amount']=max(0,(float)$r['order_amount']-(float)$r['paid_amount']);$r['quote_currency']=quote_summary_currency($r['quote_currency']);$r['order_currency']=quote_summary_currency($r['order_currency']);}unset($r);$data=quote_summary_attach_last_followup($pdo,$data);
  $headers=['报价号','订单号','客户','国家/地区','负责人','报价日期','最后一次跟进时间','订单日期','出货 / 收款日期','状态','报价金额','订单金额','已收 / 未收','产品数'];$sheetRows=[];$cells=[];foreach($headers as $i=>$header)$cells[]=quote_price_excel_cell($i+1,1,$header,1);$sheetRows[]='<row r="1" ht="28" customHeight="1">'.implode('',$cells).'</row>';$rowNo=2;
  foreach($data as $r){$quoteMoney=quote_summary_currency($r['quote_currency']).' '.number_format((float)$r['quote_amount'],2,'.',',');$orderMoney=quote_summary_currency($r['order_currency']).' '.number_format((float)$r['order_amount'],2,'.',',');$receivable=quote_summary_currency($r['order_currency']).' '.number_format((float)$r['receivable_amount'],2,'.',',');$paid=quote_summary_currency($r['order_currency']).' '.number_format((float)$r['paid_amount'],2,'.',',');$values=[$r['quote_no'],$r['order_no'],$r['customer_name'],$r['country'],$r['owner'],$r['quote_date'],$r['last_followup_at'],$r['order_date'],'出：'.($r['shipment_date']?: '-') ."\n收：".($r['payment_date']?: '-'),$r['status_label'],$quoteMoney,$orderMoney,'已收 '.$paid."\n未收 ".$receivable,$r['quote_qty']];$cells=[];foreach($values as $i=>$value)$cells[]=quote_price_excel_cell($i+1,$rowNo,quote_summary_excel_text($value),2);$sheetRows[]='<row r="'.$rowNo.'" ht="34" customHeight="1">'.implode('',$cells).'</row>';$rowNo++;}
  $last=max(1,$rowNo-1);$widths=[18,18,34,14,14,14,22,14,20,18,18,18,20,12];$cols='<cols>';foreach($widths as $i=>$width)$cols.='<col min="'.($i+1).'" max="'.($i+1).'" width="'.$width.'" customWidth="1"/>';$cols.='</cols>';
  $sheet='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetViews><sheetView workbookViewId="0" showGridLines="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="21"/>'.$cols.'<sheetData>'.implode('',$sheetRows).'</sheetData><autoFilter ref="A1:N'.$last.'"/><pageMargins left="0.3" right="0.3" top="0.5" bottom="0.5" header="0.2" footer="0.2"/><pageSetup orientation="landscape" fitToWidth="1" fitToHeight="0"/></worksheet>';
  $styles='<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="3"><font><sz val="10"/><name val="Microsoft YaHei"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="10"/><name val="Microsoft YaHei"/></font><font><sz val="10"/><name val="Microsoft YaHei"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF0F766E"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="2"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="thin"><color rgb="FFD8DEE9"/></left><right style="thin"><color rgb="FFD8DEE9"/></right><top style="thin"><color rgb="FFD8DEE9"/></top><bottom style="thin"><color rgb="FFD8DEE9"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="3"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="1" xfId="0" applyFill="1" applyBorder="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="2" fillId="0" borderId="1" xfId="0" applyBorder="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>';
  $files=['[Content_Types].xml'=>'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types"><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Override PartName="/xl/workbook.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml"/><Override PartName="/xl/worksheets/sheet1.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/><Override PartName="/xl/styles.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.styles+xml"/></Types>','_rels/.rels'=>'<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>','xl/workbook.xml'=>'<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="报价明细" sheetId="1" r:id="rId1"/></sheets></workbook>','xl/_rels/workbook.xml.rels'=>'<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/><Relationship Id="rId2" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles" Target="styles.xml"/></Relationships>','xl/worksheets/sheet1.xml'=>$sheet,'xl/styles.xml'=>$styles];
  $bin=quote_price_excel_zip($files);while(ob_get_level()>0)@ob_end_clean();header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');header('Content-Disposition: attachment; filename="quotation-detail-'.date('Ymd-His').'.xlsx"');header('Content-Length: '.strlen($bin));echo $bin;exit;
}
