...
function quote_summary_attach_next_followup(PDO $pdo,array $data): array{
  $quoteIds=[];foreach($data as $r){$id=(int)($r['quote_id']??0);if($id>0)$quoteIds[$id]=$id;}
  if(!$quoteIds || !table_exists($pdo,'crm_quote_followup_activities')){foreach($data as &$r)$r['next_followup_at']='';unset($r);return $data;}
  $ids=array_values($quoteIds);$marks=implode(',',array_fill(0,count($ids),'?'));
  try{
    $activityRows=rows($pdo,"SELECT quote_id,next_followup_at FROM crm_quote_followup_activities WHERE quote_source='legacy' AND deleted_at IS NULL AND quote_id IN ($marks) ORDER BY quote_id ASC,contacted_at DESC,id DESC",$ids);
  }catch(Throwable $e){$activityRows=[];}
  $latest=[];foreach($activityRows as $activity){$id=(int)($activity['quote_id']??0);if($id>0 && !array_key_exists($id,$latest))$latest[$id]=(string)($activity['next_followup_at']??'');}
  foreach($data as &$r)$r['next_followup_at']=$latest[(int)($r['quote_id']??0)]??'';unset($r);
  return $data;
}
function quote_summary_list_data(PDO $pdo,array $f){
  $params=[];$base=quote_summary_filtered_sql($pdo,$f,$params);$field=['date'=>'quote_date','quote_no'=>'quote_no','order_no'=>'order_no','quote_date'=>'quote_date','order_date'=>'order_date','shipment_date'=>'shipment_date','payment_date'=>'payment_date','quote_amount'=>'quote_amount','order_amount'=>'order_amount','paid_amount'=>'paid_amount','customer'=>'customer_name','owner'=>'owner'][$f['sort_field']]??'quote_date';$order=strtoupper($f['sort_order'])==='ASC'?'ASC':'DESC';$count=(int)(row($pdo,'SELECT COUNT(*) c FROM ('.$base.') z',$params)['c']??0);$offset=($f['page']-1)*$f['page_size'];$data=rows($pdo,$base." ORDER BY `$field` $order, quote_id DESC LIMIT ".(int)$f['page_size'].' OFFSET '.(int)$offset,$params);foreach($data as &$r){$r['status_label']=quote_summary_status($r);$r['receivable_amount']=max(0,(float)$r['order_amount']-(float)$r['paid_amount']);$r['quote_currency']=quote_summary_currency($r['quote_currency']);$r['order_currency']=quote_summary_currency($r['order_currency']);}unset($r);$data=quote_summary_attach_next_followup($pdo,$data);return ['list'=>$data,'total'=>$count,'page'=>$f['page'],'page_size'=>$f['page_size'],'pages'=>max(1,(int)ceil($count/$f['page_size']))];}
...
function quote_summary_export_excel(PDO $pdo,array $f){
  $params=[];$base=quote_summary_filtered_sql($pdo,$f,$params);$data=rows($pdo,$base.' ORDER BY quote_date DESC, quote_id DESC LIMIT 10000',$params);foreach($data as &$r){$r['status_label']=quote_summary_status($r);$r['receivable_amount']=max(0,(float)$r['order_amount']-(float)$r['paid_amount']);$r['quote_currency']=quote_summary_currency($r['quote_currency']);$r['order_currency']=quote_summary_currency($r['order_currency']);}unset($r);$data=quote_summary_attach_next_followup($pdo,$data);while(ob_get_level()>0)@ob_end_clean();header('Content-Type: application/vnd.ms-excel; charset=utf-8');header('Content-Disposition: attachment; filename="quotation-detail-'.date('Ymd-His').'.xls"');echo "\xEF\xBB\xBF";echo "报价号\t订单号\t客户\t国家/地区\t负责人\t报价日期\t下次跟进时间\t订单日期\t出货日期\t收款日期\t流程状态\t报价金额\t报价币种\t订单金额\t订单币种\t已收款\t未收款\t产品数量\n";foreach($data as $r){$line=[ $r['quote_no'],$r['order_no'],$r['customer_name'],$r['country'],$r['owner'],$r['quote_date'],$r['next_followup_at'],$r['order_date'],$r['shipment_date'],$r['payment_date'],$r['status_label'],number_format((float)$r['quote_amount'],2,'.',''),$r['quote_currency'],number_format((float)$r['order_amount'],2,'.',''),$r['order_currency'],number_format((float)$r['paid_amount'],2,'.',''),number_format((float)$r['receivable_amount'],2,'.',''),$r['quote_qty'] ];echo implode("\t",array_map(function($v){return str_replace(["\t","\r","\n"],[' ',' ',' '],(string)$v);},$line))."\n";}exit;
}
