...
{
    $viewOnly = ['view'];
    $basic = ['view','create','edit','result','stage','quote','material','dispatch','sync','send','reply','compose','attachment_download','link_customer','account_bind_own','signature_manage_own','task_create','execute','analytics','create_project','edit_project','create_group','edit_group','move_customer','create_personal','create_dispatch','create_multi','edit_own','edit_assigned','edit_cell','change_status','change_due_date','comment','urge','upload_image','upload_attachment','download_attachment','download','generate_pdf','generate_excel','package','naming_select','lead_capture','quote_draft','material_draft','confirm_own','reject_own','preferences_edit'];
    ...
    if ($profile === 'engineering') {
        return artdon_permission_keys_for_policy(
            ...
            ['view','create','edit','result','material','dispatch','create_personal','edit_assigned','edit_cell','change_status','change_due_date','comment','upload_image','upload_attachment','download_attachment','download','generate_pdf','generate_excel','package','naming_select','naming_link','confirm_own','preferences_edit'],
    ...
    if ($profile === 'purchase') {
        return artdon_permission_keys_for_policy(
            ...
            ['view','edit','create_personal','edit_assigned','change_status','comment','upload_attachment','download_attachment','download','confirm_own','preferences_edit'],
    ...
    if ($profile === 'production') {
        return artdon_permission_keys_for_policy(
            ...
            ['view','create_personal','edit_assigned','change_status','comment','upload_image','upload_attachment','download_attachment','download','preferences_edit'],
            ['logs.view_own'],
...
function admin_delete_user_account(int $userId): void
{
    ensure_user_profile_schema();
    if (!is_super_admin() && !has_permission('dangerous.permission_admin')) {
        throw new RuntimeException('没有删除账号权限。');
    }
    $current = current_user();
    if ($userId <= 0) throw new RuntimeException('缺少要删除的账号。');
    if ((int)($current['id'] ?? 0) === $userId) throw new RuntimeException('不能删除当前登录账号。');

    $stmt = db()->prepare('SELECT id, username, real_name, email, role_id, status FROM crm_users WHERE id = ? LIMIT 1');
    $stmt->execute([$userId]);
    $before = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$before) throw new RuntimeException('账号不存在。');

    if (!is_super_admin()) {
        $roleStmt = db()->prepare('SELECT role_key FROM crm_roles WHERE id = ? LIMIT 1');
        $roleStmt->execute([(int)($before['role_id'] ?? 0)]);
        if ((string)$roleStmt->fetchColumn() === 'super_admin') {
            throw new RuntimeException('super_admin 账号只能由超级管理员处理。');
        }
    }

    db()->prepare("UPDATE crm_users SET status = 'disabled', updated_at = NOW() WHERE id = ?")->execute([$userId]);
    db()->prepare('DELETE FROM crm_user_permissions WHERE user_id = ?')->execute([$userId]);
    if (db_table_exists('crm_user_scopes')) {
        db()->prepare('DELETE FROM crm_user_scopes WHERE user_id = ?')->execute([$userId]);
    }

    $after = $before;
    $after['status'] = 'disabled';
    if (function_exists('audit_log')) {
        audit_log('user_delete_account', 'users', 'user', $userId, $before, $after);
    }
    try {
        db()->prepare('INSERT INTO crm_user_approval_logs (user_id, action, operator_id, operator_name, reason, before_json, after_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())')
            ->execute([$userId, 'delete', $current['id'] ?? null, $current['username'] ?? '', '权限中心删除账号（停用并清理单独权限）', json_encode($before, JSON_UNESCAPED_UNICODE), json_encode($after, JSON_UNESCAPED_UNICODE)]);
    } catch (Throwable $e) {
        error_log('admin_delete_user_account approval log failed: ' . $e->getMessage());
    }
}
