(() => {
  ...
  const productMissing = (product) => Math.max(0, 10 - num(product.group_count));
  ...
  const renderProducts = () => {
    ...
    root.innerHTML = `
      <section class="mc-adaptation-products-page">
        ...
        <div class="mc-adaptation-table-wrap"><table class="mc-adaptation-table"><thead><tr><th>产品</th><th>类型 / 系列</th><th>当前阶段</th><th>完成度</th><th>技术范围缺失</th><th>核心物料缺失</th><th>冲突</th><th>待审批</th><th>版本</th><th>更新时间</th><th>操作</th></tr></thead><tbody>${visible.length ? visible.map((product) => {
          const stageText = stage(product); const coreMissing = productMissing(product);
          return `<tr><td><div class="mc-adaptation-product-cell"><span class="mc-adaptation-image">${image(product.image_url)}</span><span><b>${esc(product.product_code)}</b><small>${esc(productName(product))}</small></span></div></td><td>${esc(text(product.product_type))}<small>${esc(text(product.series_name))}</small></td><td><span class="mc-adaptation-stage">${esc(stageText)}</span></td><td>${num(product.group_count) ? '进行中' : '未开始'}</td><td>${num(product.group_count) ? '待确认' : '—'}</td><td>${coreMissing}</td><td>${num(product.conflict_count)}</td><td>${stageText === '待审批' ? 1 : 0}</td><td>草稿 ${num(product.group_count)} / 发布 ${text(product.approved_version, '—')}</td><td>${esc(text(product.updated_at))}</td><td><button type="button" class="mc-button mc-button--small ${product.configuration_state === 'unconfigured' ? 'mc-button--primary' : ''}" data-v3-open-product="${num(product.id)}">${product.configuration_state === 'unconfigured' ? '开始配置' : '继续配置'}</button></td></tr>`;
