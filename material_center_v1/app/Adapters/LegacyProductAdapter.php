...
final class LegacyProductAdapter
{
    ...
    {
        $sql="SELECT id,model_no,category,item_name,product_name,status,remark,dim_length,dim_width,dim_height FROM naming_models WHERE COALESCE(website_deleted,0)=0";$params=[];
        if($term!==''){$sql.=" AND (model_no LIKE ? OR item_name LIKE ? OR product_name LIKE ?)";$like='%'.$term.'%';$params=[$like,$like,$like];}
