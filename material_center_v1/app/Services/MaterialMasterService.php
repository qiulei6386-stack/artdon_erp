...
final class MaterialMasterService{
 ...
 private function cloneDraft(int$id,int$userId,string$mode):int{
  ...
  try{
   ...
   if($r['category_code']==='power_supply')$this->copyPowerSupplyDomain($id,$new,$userId);
   $logAction=$isRevision?'revision_draft_created':'copy';
   $this->log($new,$logAction,['source_material_id'=>$id,'source_material_code'=>$r['material_code'],'fields'=>array_keys($fields),'chip_variants_copied'=>$r['category_code']==='chip','lens_angle_compatibility_copied'=>$r['category_code']==='optical','power_domain_copied'=>$r['category_code']==='power_supply'],$userId);
 ...
 private function copyPowerSupplyDomain(int$from,int$to,int$userId):void{
  $fields=['installation_type','output_type','quality_grade','supplier_warranty_years','product_customer_warranty_years','nominal_power_w','min_output_power_w','max_output_power_w','power_band_id','input_voltage_min_v','input_voltage_max_v','input_frequency_min_hz','input_frequency_max_hz','output_current_ma','output_current_min_ma','output_current_max_ma','is_dip_switch','is_programmable','output_voltage_min_v','output_voltage_max_v','length_mm','width_mm','height_mm','shape','mounting_hole_distance_mm','wiring_type','ip_rating','power_factor','efficiency','thd','flicker_grade','standby_power','purchase_price','currency','moq','lead_time_days','certification','stock_status'];
  $select=$this->db->prepare('SELECT '.implode(',',$fields).' FROM mc_power_supply_specs WHERE material_id=? LIMIT 1');$select->execute([$from]);$src=$select->fetch(PDO::FETCH_ASSOC);if(!$src)return;
  $sets=[];$values=[];foreach($fields as$field){$sets[]="`$field`=?";$values[]=$src[$field]??null;}$sets[]="status='draft'";$sets[]='updated_at=NOW()';$values[]=$to;
  $this->db->prepare('UPDATE mc_power_supply_specs SET '.implode(',',$sets).' WHERE material_id=?')->execute($values);
  if(\mc_table_exists('mc_power_supply_current_options')){
   $this->db->prepare('DELETE FROM mc_power_supply_current_options WHERE material_id=?')->execute([$to]);
   $this->db->prepare("INSERT INTO mc_power_supply_current_options(material_id,current_ma,is_default,source) SELECT ?,current_ma,is_default,'revision' FROM mc_power_supply_current_options WHERE material_id=?")->execute([$to,$from]);
  }
  if(\mc_table_exists('mc_power_supply_dimming_modes')){
   $this->db->prepare('DELETE FROM mc_power_supply_dimming_modes WHERE material_id=?')->execute([$to]);
   $this->db->prepare('INSERT INTO mc_power_supply_dimming_modes(material_id,mode,is_primary) SELECT ?,mode,is_primary FROM mc_power_supply_dimming_modes WHERE material_id=?')->execute([$to,$from]);
  }
 }
