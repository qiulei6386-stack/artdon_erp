...
$pageTitle = '产品适配 V2';
$pageDescription = '第 2 阶段：基础数据模型、产品分类中心和配置组定义中心。';
...
?>
<style>
.mc-pa2-page{--pa2-teal:#0f9f9a;--pa2-blue:#2563eb;--pa2-border:#dbe7f3;--pa2-muted:#667085;display:grid;gap:18px}
.pa2-hero{display:flex;justify-content:space-between;gap:18px;align-items:flex-start;background:linear-gradient(135deg,#fff,#f3fbfb);border:1px solid var(--pa2-border);border-radius:18px;padding:22px}
...
.pa2-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px}.pa2-card{background:#fff;border:1px solid var(--pa2-border);border-radius:16px;padding:16px;box-shadow:0 8px 20px rgba(16,24,40,.04)}.pa2-card strong{display:block;color:#122033}.pa2-card b{font-size:26px}.pa2-card span,.pa2-card p{color:var(--pa2-muted)}
...
<section class="mc-page mc-pa2-page" data-adaptation-v2 data-phase="2" data-view="<?=mc_h($view)?>">
    ...
    <?php if ($view === 'home'): ?>
        <section class="pa2-grid">
            ...
            <article class="pa2-card"><strong>产品映射</strong><b><?=intval($summary['mapping_count'])?></b><p>具体产品可以映射到 V2 产品分类和系列。</p></article>
        </section>
        <section class="pa2-panel">
            <div class="pa2-panel__head"><div><h2>阶段路由和边界</h2><p>第 2 阶段只开放分类、配置组和产品映射；模板、规则、工作台和配置包仍按后续阶段开发。</p></div></div>
