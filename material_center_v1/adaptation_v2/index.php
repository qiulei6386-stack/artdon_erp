...
$workspaceListDisplay = (string)($_GET['display'] ?? 'cards');
if (!in_array($workspaceListDisplay, ['cards', 'list'], true)) $workspaceListDisplay = 'cards';
...
<section class="mc-page mc-pa2-page" data-adaptation-v2 data-phase="10" data-view="<?=mc_h($view)?>">
    ...
    <?php elseif ($view === 'workspace'): ?>
        <?php if ($workspaceProductId <= 0): ?>
            <section class="pa2-panel">
                <div class="pa2-panel__head">
                    <div><h2>选择产品进入工作台</h2><p>第 5 阶段工作台按产品打开；先从产品列表选择一个产品。</p></div>
                    <div class="pa2-view-switch">
                        <a class="<?=$workspaceListDisplay==='cards'?'is-active':''?>" href="<?=mc_h(pa2_view_url('workspace', ['display'=>'cards', 'q'=>(string)($_GET['q'] ?? '')]))?>">▦ 图标视图</a>
                        <a class="<?=$workspaceListDisplay==='list'?'is-active':''?>" href="<?=mc_h(pa2_view_url('workspace', ['display'=>'list', 'q'=>(string)($_GET['q'] ?? '')]))?>">☰ 列表视图</a>
                    </div>
                </div>
                <div class="pa2-panel__body pa2-section-gap">
                    <form class="pa2-form" method="get" action="<?=mc_h(mc_url('adaptation_v2/index.php'))?>">
                        <input type="hidden" name="view" value="workspace">
                        <input type="hidden" name="display" value="<?=mc_h($workspaceListDisplay)?>">
                    ...
                    <div class="pa2-product-toolbar">
                        <span class="pa2-muted">当前显示 <?=count($products)?> 个产品<?=trim((string)($_GET['q'] ?? '')) !== '' ? ' · 已按关键词筛选' : ''?>。</span>
                        <span class="pa2-muted">图标视图可直接看图片；列表视图适合快速扫型号。</span>
                    </div>
                    <?php if (!$products): ?>
                        <div class="pa2-placeholder">没有找到匹配产品。</div>
                    <?php elseif ($workspaceListDisplay === 'cards'): ?>
                        <div class="pa2-product-grid-select">
                            <?php foreach ($products as $p): ?>
                                <?php
                                    $productCode = (string)($p['product_code'] ?: ('#'.$p['id']));
                                    $productName = (string)($p['product_name'] ?? '');
                                    $imageUrl = trim((string)($p['image_url'] ?? ''));
                                    $categoryName = (string)($p['category_name'] ?: '未映射');
                                    $seriesText = (string)($p['series_code'] ?: $p['series_name'] ?: '—');
                                ?>
                                <a class="pa2-product-tile" href="<?=mc_h(pa2_view_url('workspace', ['product_id'=>(int)$p['id']]))?>" title="打开 <?=mc_h($productCode)?> 的配置工作台">
                                    <div class="pa2-product-thumb">
                                        <?php if ($imageUrl !== ''): ?><img src="<?=mc_h($imageUrl)?>" alt="<?=mc_h($productCode)?>" loading="lazy" onerror="this.closest('.pa2-product-thumb').classList.add('is-broken');this.remove();"><?php endif; ?>
                                        <span>▧</span>
                                    </div>
                                    <div class="pa2-product-info">
                                        <strong><?=mc_h($productCode)?></strong>
                                        <p><?=mc_h($productName ?: '未命名产品')?></p>
                                        <div class="pa2-product-meta">
                                            <span class="<?=$categoryName !== '未映射' ? 'is-mapped' : ''?>"><?=mc_h($categoryName)?></span>
                                            <span><?=mc_h($seriesText)?></span>
                                        </div>
                                    </div>
                                </a>
                            <?php endforeach; ?>
                    ...
                    <?php else: ?>
                        <table class="pa2-table">
                            ...
                            <?php foreach ($products as $p): ?>
                                ...
                                <tr>
                                    <td>
                                        <div class="pa2-table-product">
                                            <div class="pa2-product-thumb">
                                                <?php if ($imageUrl !== ''): ?><img src="<?=mc_h($imageUrl)?>" alt="<?=mc_h($productCode)?>" loading="lazy" onerror="this.closest('.pa2-product-thumb').classList.add('is-broken');this.remove();"><?php endif; ?>
                                                <span>▧</span>
                                            </div>
                                            <div><strong><?=mc_h($productCode)?></strong><span><?=mc_h($productName)?></span></div>
                                        </div>
                                    </td>
