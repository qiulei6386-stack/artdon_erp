...
try{
 ...
 $sourceRows=[];if(!$ids&&!empty($_GET['include_sources']))$sourceRows=(new SourceSyncService())->materialRows($category,5000);
 ...
 foreach($sourceRows as$row){$sourceStatus=$row['status']==='changed'?'异常':'待整理';fputcsv($out,array_map($safe,['BOM-'.$row['source_pk'],$row['legacy_category'],$row['material_name'],$row['brand'],$row['model'],$row['unit_name'],$sourceStatus,'旧 BOM（只读）',$row['spec']]));}
 fclose($out);
