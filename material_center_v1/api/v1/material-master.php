...
try{$user=(new LegacyAuthAdapter())->current();$p=new PermissionService();if($_SERVER['REQUEST_METHOD']!=='POST'||!function_exists('verify_csrf')||!verify_csrf((string)($_POST['csrf_token']??'')))throw new RuntimeException('安全令牌已过期。',419);$action=(string)($_POST['action']??'save');$s=new MaterialMasterService();
 ...
 elseif($action==='copy'){$p->requireAny($user,['material_center.material.copy','material_center.material.create']);$data=['id'=>$s->copy((int)$_POST['material_id'],$user->id)];$message='已复制为新草稿';}
 elseif($action==='revision_draft'){$p->requireAny($user,['material_center.material.copy','material_center.material.create']);$data=['id'=>$s->revisionDraft((int)$_POST['material_id'],$user->id)];$message='已生成修订草稿';}
 elseif($action==='delete_draft'){$p->requireMaterialTransition($user,'delete_draft');$s->deleteDraft((int)$_POST['material_id'],$user->id);$data=[];$message='草稿已删除';}
 elseif($action==='references'){$p->require($user,'material_center.view');$data=$s->references((int)$_POST['material_id']);$message='引用检查完成';}
 else{$p->requireMaterialTransition($user,$action);$s->transition((int)$_POST['material_id'],$action,$user->id,(string)($_POST['reason']??''));$data=[];$message='物料状态已更新';}
