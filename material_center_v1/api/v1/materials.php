...
try{$user=(new LegacyAuthAdapter())->current();$permission=new PermissionService();$action=(string)($_GET['action']??$_POST['action']??'fields');
 ...
 if($action==='lifecycle'){$permission->require($user,'material_center.material.lifecycle');(new MaterialLifecycleService())->transition((int)$_POST['material_id'],(string)$_POST['transition'],$user->id,(string)($_POST['reason']??''));$data=[];$message='物料状态已更新';}
