...
<?php else:?><section class="ui-card ui-table-panel"><div class="ui-table-wrap"><table class="ui-table" data-ui-table data-page-size="20"><thead><tr><th data-sort>物料编号</th><th data-sort>品牌</th><th data-sort>型号</th><th data-sort>名称</th><th data-sort>功率档</th><th data-sort>安装</th><th data-sort>输出</th><th data-sort>电流</th><th data-sort>调光</th><th data-sort>状态</th></tr></thead><tbody><?php foreach($rows as$r):?><tr><td><b><?=mc_h($r['material_code'])?></b></td><td><?=mc_h($r['brand'])?></td><td><?=mc_h($r['model'])?></td><td><?=mc_h($r['name'])?></td><td><?=mc_h($r['power_band'])?></td><td><?=mc_h($r['installation_type'])?></td><td><?=mc_h($r['output_type'])?></td><td><?=mc_h($r['output_current_ma'])?>mA</td><td><?=mc_h($r['dimming_modes']?:'待确认')?></td><td><span class="ui-badge"><?=$r['is_pilot']?'试点':'正式'?> / <?=mc_h($r['status'])?></span></td></tr><?php endforeach;?></tbody></table></div></section><?php endif;endif;?>
<?php mc_page_end();?>
