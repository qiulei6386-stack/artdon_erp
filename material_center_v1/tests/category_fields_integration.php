...
try{
 ...
 foreach($cases as$category=>$case){[$values,$table,$column]=$case;$value=reset($values);if(!$fields->definitions($category))throw new RuntimeException("$category field registry empty");$id=$master->save(['category_id'=>$categories[$category],'name'=>"$tag $category",'unit'=>'PCS','fields'=>$values],1);$ids[]=$id;$stmt=$db->prepare("SELECT `$column` FROM `$table` WHERE material_id=?");$stmt->execute([$id]);if((string)$stmt->fetchColumn()!==(string)$value)throw new RuntimeException("$category extension write failed");}
