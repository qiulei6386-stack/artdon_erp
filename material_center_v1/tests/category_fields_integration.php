...
try{
 ...
 $minimum=['power_supply'=>20,'chip'=>17,'optical'=>11,'profile'=>12,'connector'=>11,'accessory'=>9,'packaging'=>11];
 foreach($cases as$category=>$case){[$values,$table,$column]=$case;$value=reset($values);$definitions=$fields->definitions($category);if(count($definitions)<$minimum[$category])throw new RuntimeException("$category field coverage incomplete: ".count($definitions));$id=$master->save(['category_id'=>$categories[$category],'name'=>"$tag $category",'unit'=>'PCS','fields'=>$values],1);$ids[]=$id;$stmt=$db->prepare("SELECT `$column` FROM `$table` WHERE material_id=?");$stmt->execute([$id]);$actual=$stmt->fetchColumn();$same=is_numeric($value)?abs((float)$actual-(float)$value)<0.000001:(string)$actual===(string)$value;if(!$same)throw new RuntimeException("$category extension write failed");}
 ...
 $master->save(['id'=>$ids[1],'lock_version'=>1,'category_id'=>$categories['chip'],'name'=>"$tag chip updated",'brand'=>'A','unit'=>'PCS','fields'=>['chip.cri'=>91]],1);try{$master->save(['id'=>$ids[1],'lock_version'=>1,'category_id'=>$categories['chip'],'name'=>"$tag stale",'brand'=>'B','unit'=>'PCS'],1);throw new RuntimeException('stale concurrent edit accepted');}catch(RuntimeException$e){if($e->getMessage()==='stale concurrent edit accepted')throw$e;}
