...
foreach(array("'bom_quote_versions'=>'product_view'","'bom_quote_version'=>'product_view'",'qbv_validate_save($pdo,$d)') as $marker)qbvCheck(strpos($api,$marker)!==false,'API integration '.$marker);
echo "Quote BOM versions: exact model, snapshot classifier, zero, historical/current labels, bounded reads and save integration passed\n";
