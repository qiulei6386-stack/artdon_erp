...
(async()=>{
 ...
 try{for(const width of [390,768,1280]){
  ...
  fail=false;await page.click('[data-create]');await page.waitForURL('**/crm.php?quote_mail=*');assert.equal(requests[1].token,requests[2].token,'Retry uses same creation token');
  await page.close();
 }
