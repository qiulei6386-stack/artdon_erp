...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:process.env.CHROME_BIN});try{
 for(const width of [390,768,1280]){
  ...
  assert.equal(await page.locator('#orderList .order-card').count(),20);assert((await page.locator('.finance-strip').textContent()).includes('10700.00'));
  assert(await page.locator('[data-order-page="0"]').first().isDisabled());await page.locator('[data-order-page="2"]').first().click();await page.waitForFunction(()=>document.querySelector('#orderCount').textContent.includes('本页 20')&&document.querySelector('.order-page-controls')?.textContent.includes('第 2'));
  assert((await page.locator('.order-card').first().textContent()).includes('AT-87'));assert((await page.locator('.finance-strip').textContent()).includes('10700.00'));
  ...
  await page.fill('#orderSearch','');await page.waitForFunction(()=>document.querySelectorAll('.order-card').length===20);await page.locator('[data-order-size]').first().selectOption('50');await page.waitForFunction(()=>document.querySelectorAll('.order-card').length===50);
  await page.evaluate(()=>{failNext=true;return OrderCenter.load()});assert((await page.locator('#orderList').textContent()).includes('模拟断网'));await page.click('[data-order-retry]');await page.waitForFunction(()=>document.querySelectorAll('.order-card').length===50);
 ...
 console.log('Order page browser: 390/768/1280, page boundaries, totals, old-order search, 50 rows, retry and no overflow passed.');
