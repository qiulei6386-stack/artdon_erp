...
$sample=crm_delivery_signature_sample('<p>{user_name} / {mobile} / {position} / {email} / {contact_name}</p>', $account);
delivery_assert($sample['ready'] && strpos($sample['preview_html'],'Sender / 123 / Sales / sender@example.invalid')!==false,'Signature inspection uses actual sender and legacy aliases');
delivery_assert($sample['recipient_variables']===['{contact_name}'],'Never invent a recipient for signature inspection');
$sample=crm_delivery_signature_sample('{mail_user_position} {mail_user_mobile} {unknown}',array_merge($account,['user_position'=>'','user_phone'=>'']));
delivery_assert(!$sample['ready'] && count($sample['missing'])===3 && strpos($sample['preview_html'],'{mail_user_position}')!==false,'Missing values are visible, not silently removed');
delivery_throws(fn()=>crm_delivery_signature_html('personal',['signature_html'=>'<p>&nbsp;</p>']),'Empty signature must not pretend to work');
delivery_assert(crm_delivery_signature_html('none',[])==='','Explicit no signature');
$large='<p>{send_email}</p><img src="data:image/png;base64,'.str_repeat('A',1300000).'">';
$sample=crm_delivery_signature_sample($large,$account);
delivery_assert($sample['ready'] && substr_count($sample['preview_html'],'<img')===1 && strlen($sample['preview_html'])>1300000,'Large image signature is preserved independently of body size limit');
$packed=[];$item=crm_delivery_pack_content($packed,'<p>正文</p>',$large,$row,$account);
$expanded=crm_delivery_expand_item(['contents'=>$packed],$item);
delivery_assert(substr_count($expanded['body_html'],'data-promotion-signature')===1 && $expanded['signature_html']===$sample['preview_html'],'Inspection and frozen signature agree; appended exactly once');
