...
(function () {
  ...
  function render(){if(!root)return;
    ...
    root.querySelectorAll('.qb-box-fields').forEach((box,index)=>{const label=document.createElement('label');label.textContent='本组箱数（正整数）';const input=document.createElement('input');input.type='number';input.min='1';input.step='1';input.dataset.box=String(index);input.dataset.boxField='carton_count';input.value=String(state.cartons[index].carton_count||1);input.disabled=readonly;label.append(input);box.append(label);const note=document.createElement('p');note.className='qb-caption';note.textContent='可填写箱号范围，例如 1–20。产品数量、净重、毛重、体积均填写本组总计；拆出其中几箱时先分成两组。';box.after(note);if(state.cartons[index].weight_estimated){const label=document.createElement('label');label.innerHTML=`<input type="checkbox" data-weight-confirm="${index}" ${state.cartons[index].weight_confirmed?'checked':''} ${readonly?'disabled':''}> 已核对尾箱实际重量`;box.after(label);}});
  ...
  function packingChoices(){return '<p class="qb-caption">选择包装方案后，仅为尚未装箱的数量生成整箱和尾箱，不覆盖已有装箱。尾箱重量按比例估算，签发前须核对实际重量。</p>'+selectedItems().map(x=>`<div class="qb-pack-choice"><b>${H(x.product_code)} · ${H(x.customer_code||'')}</b><select data-pack-choice="${x.id}"><option value="">选择包装方案</option>${(x.packaging_options||[]).map(p=>`<option value="${Number(p.id)}">${{exact:'精确匹配',generic:'通用',alternative:'其他产品代码，须确认'}[p.match_kind]} · ${H(p.customer_code||'通用')} · ${Q(p.pcs_per_ctn)}件/箱 · ${H(p.carton_size)}</option>`).join('')}</select><button data-pack-generate="${x.id}">按此方案补装</button></div>`).join('');}
  function generatePacking(id){
    if(state.busy||state.status!=='planning')return;const item=state.selected.get(id);if(!item)return;
    const profile=(item.packaging_options||[]).find(p=>Number(p.id)===Number(root.querySelector(`[data-pack-choice="${id}"]`)?.value));
    if(!profile)return error('请选择包装方案；没有匹配资料时，可先维护包装资料或手工装箱。');
    const pcs=N(profile.pcs_per_ctn);if(pcs<=0||!profile.carton_size)return error('包装资料缺少每箱件数或箱规，请先补齐。');
    if(profile.match_kind==='alternative'&&!confirm('此包装属于 '+profile.customer_code+'，不是当前产品代码。确认箱规、每箱件数和重量适用于本产品？'))return;
    const left=Math.max(0,N(item.selectedQty)-N(allocations(state.cartons)[id]));if(left<=0)return error('此产品已全部装箱，未重复生成。');
    const full=Math.floor(left/pcs),tail=Math.round((left-full*pcs)*10000)/10000;
    for(const [count,qty,isTail] of [[full,full*pcs,false],[tail>0?1:0,tail,true]]){if(count<=0)continue;
      let number=1;for(const box of state.cartons){const match=String(box.carton_no).match(/^\s*(\d+)(?:\s*[-–—~]\s*(\d+))?\s*$/);if(match)number=Math.max(number,match[2]?Number(match[2])+1:Number(match[1])+(Number(box.carton_count)||1));}
      const dims=String(profile.carton_size).match(/\d+(?:\.\d+)?/g)||[],volume=N(profile.carton_cbm)||(dims.length===3?dims.reduce((a,b)=>a*N(b),1)/1000000:0);
      const ratio=isTail?qty/pcs:count;state.cartons.push({carton_no:count>1?number+'-'+(number+count-1):String(number),carton_count:count,carton_size:profile.carton_size,nw:Number((N(profile.carton_nw)>0?N(profile.carton_nw)*ratio:N(profile.unit_nw)*qty).toFixed(4)),gw:Number((N(profile.carton_gw)>0?N(profile.carton_gw)*ratio:N(profile.unit_gw)*qty).toFixed(4)),cbm:Number((volume*count).toFixed(4)),packing_profile_id:Number(profile.id),weight_estimated:isTail,weight_confirmed:false,note:isTail?'尾箱重量为估算，须核对实际重量':'按包装资料生成',items:[{order_item_id:id,qty}]});
    }touch();render();error('已生成未装数量的整箱/尾箱，请核对箱号和实际重量。');
  }
  function packingHtml(readonly){const items=selectedItems();
    return `<div class="qb-note">每箱按订单产品分配；重量、体积只按箱汇总一次。撤下整箱＝本次不出；拆箱重装＝保留计划数量。体积仅供参考，不代表一定装得下。</div>${readonly?'':packingChoices()+`<button data-action="add-box">手工新增箱 / 混箱</button>`}
  ...
  function choose(id,qty){const item=findItem(id);if(!item)return;const allocated=N(allocations(state.cartons)[id]);if(qty<allocated){error('该产品已经装箱 '+Q(allocated)+' 件，请先调整或撤下对应箱。');render();return;}
    ...
    touch();error('');}
  async function loadPlan(id){if(state.dirty&&!confirm('有未保存修改，确认重新加载已保存版本？'))return;const token=++epoch;try{const plan=await api('get',{id});if(token!==epoch)return;applyPlan(plan);render();for(const orderId of new Set([...state.selected.values()].map(x=>Number(x.order_id)))){const bundle=await api('items',{order_id:orderId,plan_id:state.id});if(token!==epoch)return;for(const item of bundle.items){const chosen=state.selected.get(Number(item.id));if(chosen)chosen.packaging_options=item.packaging_options||[];}}render();await candidates();}catch(e){if(token===epoch)error(e.message);}}
  ...
  async function preview(version=0){if(!state.id||state.dirty){error('请先保存计划后预览，确保看到服务器保存的同一份内容。');return;}window.open('quote_batch_document.php?id='+state.id+'&version='+version+'&type=pl','_blank','noopener');}
  ...
  document.addEventListener('click',event=>{const b=event.target.closest('[data-pack-generate]');if(root?.contains(b))generatePacking(Number(b.dataset.packGenerate));});
  document.addEventListener('change',event=>{const el=event.target;if(root?.contains(el)&&el.hasAttribute('data-weight-confirm')){state.cartons[Number(el.dataset.weightConfirm)].weight_confirmed=el.checked;touch();}});
  window.addEventListener('beforeunload',event=>{if(state?.dirty){event.preventDefault();event.returnValue='';}});
  openCombinedShipmentModal=open;
  openShipmentModal=open;
  // Download routing is provided by quote-shipment-links.js.
