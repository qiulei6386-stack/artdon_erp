  ...
  async function open(id) {
    ...
    try {
      ...
      dialog.querySelector('main').innerHTML=`
        ...
        <p role="status" aria-live="polite" data-status></p><button type="button" class="quote-mail-primary" data-create>准备正式邮件 → CRM 确认发送</button>
      ...
      dialog.querySelector('[data-create]').onclick=async()=>{
        ...
        try {const result=await api('create',{id,account_id:info.account_id,token,formats,email,mail_kind,body_text});location.assign(result.url);}
