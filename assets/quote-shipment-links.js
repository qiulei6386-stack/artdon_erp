...
window.QuoteShipmentLinks=(()=>{
  ...
  function bind(root){root.addEventListener('click',async e=>{
    const a=e.target.closest('a[href]');
    // A same-origin download must not be handed to an installed PWA as navigation.
    if(a&&/[?&]format=(pdf|xls)(?:&|$)/.test(a.search)){a.download='';a.removeAttribute('target');return;}
    const b=e.target.closest('[data-qsl-plan]');if(!b)return;dialog?.remove();dialog=null;await QuoteShipmentBatch.open(Number(b.dataset.qslBase),Number(b.dataset.qslPlan));
  });}
