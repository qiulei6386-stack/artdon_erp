...
(function (root) {
  ...
  function install(p, env) {
    ...
    function validation(step,d) {
      ...
      if (step === 2 && (d.channel_key === 'email'||d.channel_key === 'preference') && d.signature_key!=='personal') return '请选择实际发件账号签名；缺少签名的邮箱不能执行。';
    ...
    function renderBasics(d) {
      return panel('任务信息','必填项标有 *，其他内容可稍后补充。',
        ...
        select('channel_key','推广方式 *',d.channel_key,[['email','邮件 · 自动发送'],['phone','电话 · 人工执行'],['wechat','微信 · 人工执行'],['whatsapp','WhatsApp · 人工执行'],['linkedin','LinkedIn · 人工执行'],['wechat_group','微信群 · 人工执行'],['whatsapp_group','WhatsApp群 · 人工执行'],['offline','线下跟进 · 人工执行'],['preference','按资料唯一渠道 · 混合执行']],'联系人明确设置优先；与客户主档不同会在最终预览说明。多个渠道须明确选择，不会猜测。')+
    ...
    function renderContent(d) {
      ...
      var signature=email?panel('发件签名','每个邮箱必须有自己的签名，自动追加；缺失则阻止确认。',select('signature_key','签名方式',d.signature_key,[['personal','实际发件账号签名（逐邮箱核对）']])+'<div data-pc-signature-tools>'+signatureTools(d)+'</div>'):'';
