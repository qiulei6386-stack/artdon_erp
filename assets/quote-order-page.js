...
(()=>{
  ...
  const sizes=[5,10,20];
  let preferredSize=5;try{const saved=Number(localStorage.getItem('quote-order-page-size-v2'));if(sizes.includes(saved))preferredSize=saved;}catch(e){}
  const state={page:1,size:preferredSize,data:null,key:'',revision:0,pending:null,timer:null,overviewPending:null};
  ...
  function pager(){const d=state.data;if(!d)return '';return `<div class="order-page-controls" aria-label="订单分页">
    ...
    <label>每页 <select data-order-size aria-label="每页订单数">${sizes.map(size=>`<option value="${size}" ${d.size===size?'selected':''}>${size} 单</option>`).join('')}</select></label></div>`;}
  function paint(){
    ...
    el.innerHTML=pager()+
      (rows.length?rows.map(o=>{
        ...
        <small>${esc(o.currency||'USD')} ${money(o.amount)} ｜ ${Number(o.qty||0)} PCS ｜ ${esc(o.shipment_status)} ｜ 未收 ${money(o.balance_amount)}</small></div>
  ...
  document.addEventListener('change',e=>{if(e.target.matches('[data-order-size]')){const size=Number(e.target.value);if(!sizes.includes(size))return;state.size=size;try{localStorage.setItem('quote-order-page-size-v2',String(size));}catch(err){}state.page=1;load();}});
