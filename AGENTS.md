...
1. 每次工作结束、退出、暂停或准备交接前，都必须更新仓库根目录的 `WORK_CONTEXT.md`；即使本次没有代码修改，也不得省略上下文记录。
2. 至少记录：
   - 本次完成内容
   - 修改文件
   - 检查结果
...
4. 更新后的 `WORK_CONTEXT.md` 必须与本次修改一起提交并推送 GitHub，再将同一提交同步服务器；工作结束时本地、GitHub、服务器中的上下文记录必须一致。
