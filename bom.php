...
function toggleDashboardGroup(){dashboardGroup=!dashboardGroup;localStorage.setItem('bom_dashboard_group_v79',dashboardGroup?'1':'0');renderDashboard()}
...
function renderDashboard(){if(!$('dashboardTbody'))return;renderBaseOptions();updateDashboardViewButtons();lastDashboardRows=dashboardFilteredProjects();const totalCost=lastDashboardRows.reduce((s,p)=>s+totals(p).total,0),totalQuote=lastDashboardRows.reduce((s,p)=>s+totals(p).suggest,0);$('dashCount').textContent=lastDashboardRows.length+'/'+projects.length;$('dashTotalCost').textContent=money(totalCost);$('dashTotalQuote').textContent=money(totalQuote);$('dashAvgCost').textContent=money(lastDashboardRows.length?totalCost/lastDashboardRows.length:0);const label=dashboardRange==='month'?'本月':dashboardRange==='7'?'近 7 天':dashboardRange==='3'?'近 3 天':dashboardRange==='all'?'全部':'自定义时间';$('dashHint').textContent=`当前范围：${label} ｜ 时间字段：${$('dashTimeField').value==='createdAt'?'创建时间':'最后保存'} ｜ 共 ${lastDashboardRows.length} 个 BOM`;const iconBox=$('dashboardIconGrid');if(iconBox)iconBox.className=dashboardGroup?'dash-icon-groups':'dash-icon-grid';if(!lastDashboardRows.length){$('dashboardTbody').innerHTML=`<tr><td colspan="10"><div class="dash-empty">当前筛选没有 BOM。可以切换“全部”或清空筛选。</div></td></tr>`;if(iconBox)iconBox.innerHTML='<div class="dash-empty">当前筛选没有 BOM。可以切换“全部”或清空筛选。</div>';return}if(dashboardGroup){const groups=dashboardGroups(lastDashboardRows);$('dashboardTbody').innerHTML=groups.map(g=>`<tr class="dash-group-row"><td colspan="10"><div class="dash-group-title"><span>${esc(g.name)}</span><small>${g.rows.length} 个 BOM</small></div></td></tr>`+g.rows.map(dashboardTableRowHtml).join('')).join('');if(iconBox)iconBox.innerHTML=groups.map(g=>`<section class="dash-icon-group"><div class="dash-icon-group-head"><span>${esc(g.name)}</span><small>${g.rows.length} 个 BOM</small></div><div class="dash-icon-grid">${g.rows.map(dashboardCardHtml).join('')}</div></section>`).join('');return}$('dashboardTbody').innerHTML=lastDashboardRows.map(dashboardTableRowHtml).join('');if(iconBox)iconBox.innerHTML=lastDashboardRows.map(dashboardCardHtml).join('')}
