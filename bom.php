...
.wrap{width:100%;max-width:none;margin:0 auto;padding:14px 18px}.brand-cn{font-size:22px;font-weight:800}.brand-en{font-size:15px;color:#475569;margin-top:3px}.brand-sub{font-size:13px;color:#64748b;margin-top:5px}
...
async function loadAll(){
  try{
    ...
    projects=(r.projects||[]).map(p=>({project_uid:p.project_uid,id:p.project_uid,name:p.name,customer:p.customer,model:p.model,productType:p.product_type,currency:p.currency,productImage:bomProjectDisplayImage(p),productImageDb:p.product_image||'',labor:+p.labor,other:+p.other,profitRate:+p.profit_rate,quoteMode:p.quote_mode,exchange:+p.exchange_rate,note:p.note,rows:normalizeBomRows(p.rows,p.rows_json),createdAt:p.created_at,updatedAt:p.updated_at,createdBy:p.created_by,updatedBy:p.updated_by,linkedSystem:p.linked_system||'',linkedId:p.linked_id||'',linkedTitle:p.linked_title||'',linkedJson:p.linked_json||'',namingSnapshotJson:p.naming_snapshot_json||'',namingSync:p.naming_sync||null}));
    materials=r.materials||[]; lists=r.lists||{}; ['categories','brands','suppliers','productTypes'].forEach(k=>{if(!Array.isArray(lists[k]))lists[k]=[]});
...
function setDashboardRange(r){dashboardRange=r;['Month','7','3','All','Custom'].forEach(x=>{const el=$('dashRange'+x);if(el)el.classList.remove('active')});const map={month:'Month','7':'7','3':'3',all:'All',custom:'Custom'};const el=$('dashRange'+map[r]);if(el)el.classList.add('active');if(r==='month'){$('dashStart').value=dateInputValue(startOfMonth());$('dashEnd').value=dateInputValue(new Date())}else if(r==='7'){const d=startOfToday();d.setDate(d.getDate()-6);$('dashStart').value=dateInputValue(d);$('dashEnd').value=dateInputValue(new Date())}else if(r==='3'){const d=startOfToday();d.setDate(d.getDate()-2);$('dashStart').value=dateInputValue(d);$('dashEnd').value=dateInputValue(new Date())}else if(r==='all'){$('dashStart').value='';$('dashEnd').value=''}renderDashboard()}
function dashboardFilteredProjects(){const kw=($('dashKeyword')?.value||'').toLowerCase(),customer=$('dashCustomer')?.value||'',type=$('dashType')?.value||'',timeField=$('dashTimeField')?.value||'updatedAt';let arr=projects.filter(p=>{const dt=dateFromText(p[timeField]);const st=dashboardRangeStart(),ed=dashboardRangeEnd();if(st&&(!dt||dt<st))return false;if(ed&&(!dt||dt>ed))return false;if(customer&&p.customer!==customer)return false;if(type&&p.productType!==type)return false;if(kw&&![p.name,p.customer,p.model,p.productType,JSON.stringify(p.rows||[])].join(' ').toLowerCase().includes(kw))return false;return true});const sort=$('dashSort')?.value||'updatedDesc';arr.sort((a,b)=>sort==='costDesc'?totals(b).total-totals(a).total:sort==='costAsc'?totals(a).total-totals(b).total:sort==='createdDesc'?String(b.createdAt).localeCompare(String(a.createdAt)):sort==='customerAsc'?String(a.customer).localeCompare(String(b.customer)):sort==='modelAsc'?String(a.model).localeCompare(String(b.model)):String(b.updatedAt).localeCompare(String(a.updatedAt)));return arr}
function setDashboardView(v){dashboardView=v==='icon'?'icon':'table';localStorage.setItem('bom_dashboard_view_v79',dashboardView);renderDashboard()}
function updateDashboardViewButtons(){if($('dashViewTable'))$('dashViewTable').classList.toggle('active',dashboardView!=='icon');if($('dashViewIcon'))$('dashViewIcon').classList.toggle('active',dashboardView==='icon');if($('dashboardTableWrap'))$('dashboardTableWrap').hidden=dashboardView==='icon';if($('dashboardIconGrid'))$('dashboardIconGrid').hidden=dashboardView!=='icon'}
...
function dashboardCardHtml(p){
  ...
  return `<article class="dash-bom-card">
    ...
    <div class="dash-bom-body">
      ...
      <div class="dash-bom-meta"><span title="${esc(p.customer||'-')}">客户 ${esc(p.customer||'-')}</span><span>${esc(p.productType||'未分类')}</span><span>物料 ${rows.length}</span><span title="${esc(p.updatedAt||'')}">更新 ${esc(String(p.updatedAt||'').slice(0,16)||'-')}</span></div>
...
function renderDashboard(){if(!$('dashboardTbody'))return;renderBaseOptions();updateDashboardViewButtons();lastDashboardRows=dashboardFilteredProjects();const totalCost=lastDashboardRows.reduce((s,p)=>s+totals(p).total,0),totalQuote=lastDashboardRows.reduce((s,p)=>s+totals(p).suggest,0);$('dashCount').textContent=lastDashboardRows.length+'/'+projects.length;$('dashTotalCost').textContent=money(totalCost);$('dashTotalQuote').textContent=money(totalQuote);$('dashAvgCost').textContent=money(lastDashboardRows.length?totalCost/lastDashboardRows.length:0);const label=dashboardRange==='month'?'本月':dashboardRange==='7'?'近 7 天':dashboardRange==='3'?'近 3 天':dashboardRange==='all'?'全部':'自定义时间';$('dashHint').textContent=`当前范围：${label} ｜ 时间字段：${$('dashTimeField').value==='createdAt'?'创建时间':'最后保存'} ｜ 共 ${lastDashboardRows.length} 个 BOM`;if(!lastDashboardRows.length){$('dashboardTbody').innerHTML=`<tr><td colspan="10"><div class="dash-empty">当前筛选没有 BOM。可以切换“全部”或清空筛选。</div></td></tr>`;if($('dashboardIconGrid'))$('dashboardIconGrid').innerHTML='<div class="dash-empty">当前筛选没有 BOM。可以切换“全部”或清空筛选。</div>';return}$('dashboardTbody').innerHTML=lastDashboardRows.map(p=>{const t=totals(p);return `<tr><td class="bom-title-cell"><b>${esc(p.name||'未命名BOM')}</b><small>${esc((p.rows||[]).slice(0,2).map(r=>r.name).filter(Boolean).join(' / '))}</small></td><td>${esc(p.customer||'-')}</td><td>${esc(p.model||'-')}</td><td>${esc(p.productType||'未分类')}</td><td class="num">${(p.rows||[]).length}</td><td class="price num">${money(t.total)}</td><td class="num">${money(t.suggest)}</td><td>${esc(p.createdAt||'')}</td><td>${esc(p.updatedAt||'')}</td><td><button class="small ok" onclick="openProjectFromDashboard('${p.id}')">打开编辑</button> <button class="small ghost" onclick="quickDuplicateFromDashboard('${p.id}')">复制</button></td></tr>`}).join('');if($('dashboardIconGrid'))$('dashboardIconGrid').innerHTML=lastDashboardRows.map(dashboardCardHtml).join('')}
