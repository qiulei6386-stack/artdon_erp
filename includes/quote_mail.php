...
function qmail_create(array $input, array $account): array {
    ...
    try {
        ...
        if($email!==''&&!$chosen)throw new RuntimeException('所选联系人邮箱已变化，请重新选择。');
        ...
        $body='<p>Dear '.htmlspecialchars((string)($chosen['name']??'Customer'),ENT_QUOTES,'UTF-8').',</p><p>Please find our quotation '.htmlspecialchars($snap['quote_no'],ENT_QUOTES,'UTF-8').' attached for your review.</p><p>Please let us know if you have any questions.</p>';
        ...
        $draft=crm_mail_save_draft(['to_emails'=>$email,'subject'=>'Quotation '.$snap['quote_no'],'body_html'=>$body,'attachments_json'=>json_encode($queued),'customer_id'=>$contacts['customer_id'],'contact_id'=>$chosen['id']??0,'draft_meta_json'=>json_encode(['auto_signature'=>true,'quote_mail_token'=>$token,'quote_no'=>$snap['quote_no'],'quote_revision'=>substr($s['hash'],0,12),'quote_files'=>array_column($queued,'name')])]);
...
function qmail_send_guard(array $account, array $input, array $files): array {
    ...
    $input['quote_mail_token']=$token;$input['customer_id']=(int)$p['customer_id'];return $input;
}
function qmail_sent(array $account, array $input, int $mailId): void {
    ...
    crm_log_event('quote','quote_mail_sent','quote',(string)$p['quote_id'],null,['mail_id'=>$mailId,'detail'=>$detail]);
    if($p['customer_id'])crm_customer_timeline_add((int)$p['customer_id'],'quote_mail_sent','发送已审核报价',$detail,'mail',(string)$mailId);
}
