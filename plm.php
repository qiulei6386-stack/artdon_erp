...
.summary-sample-ref-card{border:1px solid #dbe4ef;background:#fff;border-radius:10px;padding:11px 12px;min-height:190px;display:flex;flex-direction:column;gap:7px;min-width:0;position:relative;cursor:pointer}
.summary-sample-ref-card:hover{border-color:#93c5fd;box-shadow:0 10px 24px rgba(37,99,235,.08)}
...
#tab-summary.summary-ref .summary-sample-ref-card .summary-sample-actions{position:relative!important;z-index:3!important}
#tab-summary.summary-ref .summary-sample-ref-card .summary-sample-actions button{position:relative!important;z-index:4!important;pointer-events:auto!important}
...
.test-project-layout{grid-template-columns:minmax(280px,.48fr) minmax(760px,1.52fr)!important;gap:14px!important;align-items:start!important;overflow:visible!important}
...
.test-card-grid{grid-template-columns:1fr!important;gap:8px!important}
.test-project-card{min-height:108px!important;height:auto!important;padding:9px!important;border-radius:10px!important;gap:5px!important;overflow:hidden!important}
...
.test-current-summary{display:grid!important;grid-template-columns:repeat(4,minmax(0,1fr))!important;border-bottom:1px solid #e2e8f0!important;background:#f8fafc!important;min-height:34px!important;overflow:hidden!important}
...
@media(max-width:1320px){.test-project-layout{grid-template-columns:minmax(260px,.55fr) minmax(0,1.45fr)!important}.test-detail-main-grid{grid-template-columns:1fr!important}.test-evidence-body{max-height:none!important}.test-card-grid{grid-template-columns:1fr!important}}
...
function renderTestTemplateField(f,t,custom){
  ...
  const compactBinds=['ambient_temp','led_temp','driver_temp','shell_temp','max_temp','sphere_lumen','sphere_power','sphere_efficiency','cct','cri','ies_angle','ies_lumen'];
  const isCompact=type==='number'||type==='date'||compactBinds.includes(bind)||/(环境温度|LED温度|电源温度|驱动温度|壳体温度|外壳温度|最高温度|测试时长|老化时长|测试数量|失效数量|功率|色温|光束角|峰值|光强|光通量|光效|功率因数|PF|CRI|R9|SDCM|角度|温度|时长)/i.test(labelText);
  const isMedium=!isCompact&&(bind==='test_standard'||/(判定标准|测试标准|测试机构|测试设备|配光类型|防护等级|目标等级|报告编号|测试项目)/.test(labelText));
  const isWide=!isMedium&&!isCompact&&(type==='textarea'||type==='file'||bind==='test_data'||bind==='test_conclusion'||/(测试条件|测试结论|异常说明|整改建议|备注|装配异常|测试图片|检查图片|配光报告|光谱报告|EMC报告)/.test(labelText));
...
function fileModelLabel(m){
  ...
  const name=m?.name||'';
  const ver=m?.sample_version||'';
  return [code||name||('型号'+m.id), name&&name!==code?name:'', ver].filter(Boolean).join(' · ');
}
...
function fileModelSelectHtml(id,val=0,includeAll=false){
  const ms=pModels(cur()?.id||selectedId);
  const valStr=String(val ?? '');
  let html='';
  if(includeAll) html+=`<option value="" ${valStr===''?'selected':''}>全部型号</option>`;
  html+=`<option value="0" ${(valStr!=='' && Number(val)===0)?'selected':''}>项目公共文件</option>`;
  html+=ms.map(m=>`<option value="${m.id}" ${(valStr!=='' && Number(val)===Number(m.id))?'selected':''}>${esc(fileModelLabel(m))}</option>`).join('');
  return `<select id="${id}">${html}</select>`
}
...
function openFileUploadDrawer(ctx={}){
  ...
  const host=document.createElement('div');
  host.innerHTML=`<div class="modal-mask file-upload-mask" onclick="if(event.target===this)this.remove()"><div class="modal file-upload-drawer"><div class="modal-head"><div><h3>上传文件</h3><div class="hint">当前项目：${esc(p.name||'未命名项目')}。支持一次选择多个文件。</div></div><button class="btn" onclick="this.closest('.modal-mask').remove()">关闭</button></div><div class="modal-body"><div class="file-upload-form"><section><h4>上传目标</h4><div class="grid2"><div class="field"><label>归属型号</label>${fileModelSelectHtml(uid+'_model',mid,false)}</div><div class="field"><label>来源模块</label><select id="${uid}_source"><option value="file" ${source==='file'?'selected':''}>文件中心上传</option><option value="project" ${source==='project'?'selected':''}>项目基础资料</option><option value="model" ${source==='model'?'selected':''}>样品管理</option><option value="flow" ${source==='flow'?'selected':''}>开发导航图</option><option value="test" ${source==='test'?'selected':''}>测试中心</option><option value="issue" ${source==='issue'?'selected':''}>问题闭环</option></select></div></div><div class="field"><label>关联开发步骤</label>${stepSelectHtml(uid+'_step',stepId,mid,true)}</div><div class="field"><label>关联测试项目</label>${testSelectHtml(uid+'_test',testId,mid,true)}</div><div class="field"><label>关联问题</label>${issueSelectHtml(uid+'_issue',issueId,mid,true)}</div></section><section><h4>文件资料</h4><div class="grid2"><div class="field"><label>文件分类</label><select id="${uid}_cat">${uniq([...FILE_CATS,...TEST_FILE_CATS]).map(c=>`<option>${esc(c)}</option>`).join('')}</select></div><div class="field"><label>文件标题</label><input id="${uid}_title" placeholder="可不填，默认用文件名"></div></div><input id="${uid}_file" type="file" multiple><textarea id="${uid}_note" placeholder="文件备注"></textarea></section><section><h4>权限</h4><label class="hint"><input id="${uid}_vis" type="checkbox" checked style="width:auto"> 可发客户</label><div class="hint">取消勾选即内部资料，不能进入客户资料包。</div></section><div id="${uid}_result" class="upload-result"></div></div></div><div class="modal-foot"><button class="btn" onclick="this.closest('.modal-mask').remove()">取消</button><button class="btn primary" onclick="uploadFilesFromDrawer('${uid}')">开始上传</button></div></div></div>`;
  document.body.appendChild(host.firstElementChild);
}
...
function renderSummary(p){
  ...
  const sampleCards=ms.map(m=>{
    ...
    const mid=Number(m.id), stepId=Number(curNext.current?.id||curNext.next?.id||0);
    return `<article class="summary-sample-ref-card ${mid===Number(summarySampleId)?'active':''}" role="button" tabindex="0" onclick="summaryJump('models',{model_id:${mid}})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();summaryJump('models',{model_id:${mid}})}"><div class="summary-sample-ref-head"><h4 title="${esc(modelLabel(m))}">${esc(modelLabel(m))}</h4><span class="summary-ver">${esc(m.sample_version||'V1.0')}</span></div><div class="summary-sample-model">型号：${esc(m.model||m.naming_model_no||m.sample_no||'-')}</div><div class="summary-sample-progress"><span>${esc(m.status||'开发中')}</span><i><em style="width:${mf.pct}%"></em></i><span>${mf.pct}%</span></div><div class="summary-sample-split"><div><b>当前步骤：${esc(curNext.current?curNext.current.title:'暂无进行中')}</b><b>下一步：${esc(curNext.next?curNext.next.title:(mf.total?'流程已完成':'暂无流程'))}</b></div><div><span>测试：${esc(testText)}</span><span>问题：${mIssues.length}</span><span>文件：${mFiles.length}</span><span>BOM：${esc(bomText)}</span></div></div><div class="summary-sample-actions"><button type="button" onclick="event.stopPropagation();summaryJump('models',{model_id:${mid}})">样品详情</button><button type="button" onclick="event.stopPropagation();summaryJump('flow',{model_id:${mid},step_id:${stepId}})">开发流程</button><button type="button" onclick="event.stopPropagation();summaryJump('tests',{model_id:${mid}})">测试</button><button type="button" onclick="event.stopPropagation();summaryJump('issues',{model_id:${mid}})">问题</button><button type="button" onclick="event.stopPropagation();summaryJump('files',{model_id:${mid}})">文件</button><button type="button" onclick="event.stopPropagation();createBomFromModel(${mid})">BOM</button></div></article>`;
