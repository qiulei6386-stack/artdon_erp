...
.test-project-layout{grid-template-columns:minmax(300px,.58fr) minmax(720px,1.42fr)!important;gap:16px!important;align-items:start!important;overflow:visible!important}
...
.test-statbar-lite{grid-template-columns:repeat(5,minmax(0,1fr))!important;min-height:42px!important}.test-statbar-lite span{font-size:11.5px!important}
...
@media(max-width:1320px){.test-project-layout{grid-template-columns:minmax(280px,.65fr) minmax(0,1.35fr)!important}.test-detail-main-grid{grid-template-columns:1fr!important}.test-evidence-body{max-height:none!important}.test-card-grid{grid-template-columns:1fr!important}}
...
function renderTests(p){
  ...
  $('tabbody').innerHTML=`<div class="test-workbench">
    <div class="test-topbar">
      ...
      <div class="test-top-actions"><button class="btn primary" onclick="openAddTestProjectPanel()">添加测试项目</button><button class="btn good" onclick="addCommonTests()">一键加入常用测试</button><details class="test-more-menu"><summary>更多</summary><div><button class="btn small" onclick="openTestTemplateDrawer()">测试模板</button><button class="btn small" onclick="exportCurrentProjectTestReport()">导出 CSV</button><button class="btn small" onclick="window.print()">打印</button><button class="btn small" onclick="openTestDashboardModal()">查看测试报表</button></div></details></div>
    </div>
    <div class="test-statbar test-statbar-lite"><span><b>${pct}%</b> 完成率</span><span><b>${total}</b> 测试</span><span class="good"><b>${done}</b> 通过</span><span class="bad"><b>${failed}</b> 不通过</span><span class="warn"><b>${retest}</b> 待复测</span></div>
    <div class="test-stat-progress"><i style="width:${Math.max(0,Math.min(100,pct))}%"></i></div>
    <div class="test-project-layout">
      <div class="test-project-board"><div class="test-node-head"><h3>测试项目</h3></div>
