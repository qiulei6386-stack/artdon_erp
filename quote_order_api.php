...
function qo_ok($data=[]){ if(($GLOBALS['action']??'')==='detail'){qo_write_json(['ok'=>true,'data'=>$data]);exit;} echo json_encode(['ok'=>true,'data'=>$data], JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES); exit; }
function qo_fail($msg){ throw new RuntimeException($msg); }
function qo_release_session_lock(){ if(session_status()===PHP_SESSION_ACTIVE) @session_write_close(); }
function qo_input($release=false){ static $j=null; if($release){$j=null;return [];} if($j!==null) return $j; $raw=file_get_contents('php://input'); $a=json_decode((string)$raw,true); $j=is_array($a)?$a:($_POST?:[]); return $j; }
...
function qo_carton_count_value($v){ $n=qo_num($v); return $n>0?$n:1; }
function qo_has_carton_detail(array $c): bool {
  foreach(['carton_no','carton_range','items_text','qty','carton_size','nw','gw','cbm','note'] as $k){
    if(trim((string)($c[$k]??''))!=='') return true;
  }
  return false;
}
function qo_carton_detail_totals(array $cartons): array {
  $tot=['qty'=>0,'cartons'=>0,'nw'=>0,'gw'=>0,'cbm'=>0];
  foreach($cartons as $c){
    if(!is_array($c) || !qo_has_carton_detail($c)) continue;
    $tot['qty']+=qo_num($c['qty']??0);
    $tot['cartons']+=qo_carton_count_value($c['carton_count']??1);
    $tot['nw']+=qo_num($c['nw']??0);
    $tot['gw']+=qo_num($c['gw']??0);
    $tot['cbm']+=qo_num($c['cbm']??0);
  }
  return $tot;
}
function qo_add_carton_detail_totals(array $tot,array $cartons): array {
  $extra=qo_carton_detail_totals($cartons);
  if($extra['qty']>qo_num($tot['qty']??0)+0.00001) throw new RuntimeException('拼箱内数量不能超过本次产品出货总数；产品行须包含拼箱内的产品数量。');
  // Carton contents allocate the product quantities; they are not extra goods.
  foreach(['cartons','nw','gw','cbm'] as $k) $tot[$k]=qo_num($tot[$k]??0)+qo_num($extra[$k]??0);
  return $tot;
}
function qo_carton_text_code(string $text): string {
  $text=trim($text);
  if($text==='' || !preg_match('/^([A-Za-z0-9][A-Za-z0-9._-]*)/u',$text,$m)) return '';
  return (string)$m[1];
}
function qo_carton_text_qty_parts(string $text,float $fallbackQty=0): array {
  $parts=[];
  if(preg_match('/((?:\d+(?:\.\d+)?\s*\+\s*)+\d+(?:\.\d+)?)\s*pcs\b/i',$text,$m)){
    foreach(preg_split('/\s*\+\s*/',$m[1]) as $p){ $n=qo_num($p); if($n>0) $parts[]=$n; }
  } elseif(preg_match('/(?:qty|数量)\s*[:：]?\s*(\d+(?:\.\d+)?)/i',$text,$m)) {
    $n=qo_num($m[1]); if($n>0) $parts[]=$n;
  }
  if(!$parts && $fallbackQty>0) $parts[]=$fallbackQty;
  return $parts;
}
function qo_tail_carton_allocations_for_shipment(PDO $pdo,int $shipmentId): array {
  $items=qo_rows($pdo,'SELECT id,order_item_id,product_code,customer_code FROM quote_shipment_items WHERE shipment_id=? ORDER BY item_index,id',[$shipmentId]);
  $cartons=qo_rows($pdo,'SELECT items_text,qty,items_json FROM quote_shipment_cartons WHERE shipment_id=? ORDER BY id',[$shipmentId]);
  $alloc=[];
  foreach($cartons as $c){
    $text=qo_s($c['items_text']??'',5000);
    if($text===''){
      $json=qo_json($c['items_json']??'',[]);
      if(isset($json[0]['text'])) $text=qo_s($json[0]['text'],5000);
    }
    $code=qo_carton_text_code($text);
    $parts=qo_carton_text_qty_parts($text,qo_num($c['qty']??0));
    if($code==='' || !$parts) continue;
    $matched=[];
    foreach($items as $it){
      if((string)($it['product_code']??'')===$code || (string)($it['customer_code']??'')===$code) $matched[]=$it;
    }
    if(!$matched) continue;
    if(count($parts)===count($matched)){
      foreach($matched as $i=>$it){ $oid=(int)($it['order_item_id']??0); if($oid>0) $alloc[$oid]=qo_num($alloc[$oid]??0)+qo_num($parts[$i]??0); }
    } elseif(count($matched)===1) {
      $oid=(int)($matched[0]['order_item_id']??0); if($oid>0) $alloc[$oid]=qo_num($alloc[$oid]??0)+array_sum($parts);
    }
  }
  return $alloc;
}
function qo_tail_carton_allocations_for_order(PDO $pdo,int $orderId): array {
  $alloc=[];
  foreach(qo_rows($pdo,"SELECT id FROM quote_shipments WHERE order_id=? AND COALESCE(status,'') NOT IN ('已作废','取消')",[$orderId]) as $sh){
    foreach(qo_tail_carton_allocations_for_shipment($pdo,(int)$sh['id']) as $oid=>$qty){
      $alloc[(int)$oid]=qo_num($alloc[(int)$oid]??0)+qo_num($qty);
    }
  }
  return $alloc;
}
...
function qo_ensure_schema(PDO $pdo){
  ...
  $pdo->exec("CREATE TABLE IF NOT EXISTS quote_shipment_orders (id INT AUTO_INCREMENT PRIMARY KEY) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
  $shoCols=['shipment_id'=>'INT NOT NULL DEFAULT 0','order_id'=>'INT NOT NULL DEFAULT 0','order_no'=>'VARCHAR(120) DEFAULT \'\'','quote_no'=>'VARCHAR(120) DEFAULT \'\'','customer_id'=>'VARCHAR(120) DEFAULT \'\'','customer_name'=>'VARCHAR(255) DEFAULT \'\'','sort_order'=>'INT DEFAULT 0','created_at'=>'DATETIME DEFAULT CURRENT_TIMESTAMP'];
  foreach($shoCols as $c=>$ddl) qo_ensure_col($pdo,'quote_shipment_orders',$c,$ddl);
  try{$pdo->exec('ALTER TABLE quote_shipment_orders ADD UNIQUE KEY uk_ship_order(shipment_id,order_id)');}catch(Throwable $e){}
  try{$pdo->exec('ALTER TABLE quote_shipment_orders ADD KEY idx_qso_order(order_id)');}catch(Throwable $e){}
  ...
  $pdo->exec("CREATE TABLE IF NOT EXISTS quote_order_statement_notes (
    id INT AUTO_INCREMENT PRIMARY KEY,
    customer_key VARCHAR(255) NOT NULL DEFAULT '',
    customer_name VARCHAR(255) DEFAULT '',
    statement_month CHAR(7) NOT NULL DEFAULT '',
    currency VARCHAR(20) NOT NULL DEFAULT '',
    note TEXT NULL,
    updated_by VARCHAR(120) DEFAULT '',
    updated_at DATETIME NULL,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uk_statement_note(customer_key,statement_month,currency)
  ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4");
  qo_ensure_col($pdo,'quote_order_statement_notes','customer_key',"VARCHAR(255) NOT NULL DEFAULT ''");
  qo_ensure_col($pdo,'quote_order_statement_notes','customer_name',"VARCHAR(255) DEFAULT ''");
  qo_ensure_col($pdo,'quote_order_statement_notes','statement_month',"CHAR(7) NOT NULL DEFAULT ''");
  qo_ensure_col($pdo,'quote_order_statement_notes','currency',"VARCHAR(20) NOT NULL DEFAULT ''");
  qo_ensure_col($pdo,'quote_order_statement_notes','note','TEXT NULL');
  qo_ensure_col($pdo,'quote_order_statement_notes','updated_by',"VARCHAR(120) DEFAULT ''");
  qo_ensure_col($pdo,'quote_order_statement_notes','updated_at','DATETIME NULL');
  qo_ensure_col($pdo,'quote_order_statement_notes','created_at','DATETIME DEFAULT CURRENT_TIMESTAMP');
  try{$pdo->exec('ALTER TABLE quote_order_statement_notes ADD UNIQUE KEY uk_statement_note(customer_key,statement_month,currency)');}catch(Throwable $e){}
...
function qo_item_qty_for_product_total($it): float {
  $it=qo_nested_item(is_array($it)?$it:[]);
  if(qo_is_virtual_item($it) && empty($it['count_in_qty'])) return 0.0;
  return qo_num($it['qty']??0);
}
...
function qo_item_row($it,$idx){
  ...
  $isVirtual=qo_is_virtual_item($it);
  ...
  if($isVirtual){ $customerCode=''; $color=''; $image=''; }
  return [
...
function qo_recalc_payment(PDO $pdo,$orderId){
  $order=qo_row($pdo,'SELECT id,amount,status,currency FROM quote_sales_orders WHERE id=? LIMIT 1',[$orderId]); if(!$order)return [];
...
function qo_update_item_shipped(PDO $pdo,$orderId){
  $items=qo_rows($pdo,'SELECT '.qr_item_columns($pdo).' FROM quote_sales_order_items WHERE order_id=?',[$orderId]);
  $totalQty=0; $totalShip=0;
  foreach($items as $it){
    if(qo_is_virtual_item($it)){
      $pdo->prepare('UPDATE quote_sales_order_items SET shipped_qty=0 WHERE id=?')->execute([$it['id']]);
      continue;
    }
...
function qo_pack_options(PDO $pdo,$productCode,$customerCode=''){
  $productCode=qo_s($productCode,160); $customerCode=qo_s($customerCode,120);
  if($productCode==='') return [];
  $rows=[];
  if($customerCode!==''){
    $rows=qo_rows($pdo,'SELECT * FROM quote_packaging_profiles WHERE product_code=? AND (customer_code=? OR customer_code=\'\') ORDER BY (customer_code=?) DESC,id DESC LIMIT 50',[$productCode,$customerCode,$customerCode]);
  }else{
    $rows=qo_rows($pdo,'SELECT * FROM quote_packaging_profiles WHERE product_code=? ORDER BY customer_code DESC,id DESC LIMIT 50',[$productCode]);
  }
  $seen=[]; $out=[];
  foreach($rows as $r){
    $key=implode('|',[qo_s($r['customer_code']??'',120),qo_s($r['carton_size']??'',160),qo_num($r['pcs_per_ctn']??0),qo_num($r['carton_nw']??0),qo_num($r['carton_gw']??0),qo_num($r['carton_cbm']??0)]);
...
function qo_next_doc_numbers(PDO $pdo,$orderId,$shipDate=''){
  $settings=qo_doc_settings($pdo); $order=qr_order($pdo,(int)$orderId);
...
function qo_prepare_items(PDO $pdo,$orderId){
  ...
  if(!$items || !qo_order_items_table_has_content($items)) qo_fail('订单明细缺失，请先核对该订单；未自动重建或改写历史数据');
  $shipped=[];
  foreach(qo_rows($pdo,'SELECT order_item_id,SUM(qty) qty FROM quote_shipment_items WHERE order_id=? GROUP BY order_item_id',[$orderId]) as $r) $shipped[(int)$r['order_item_id']]=(float)$r['qty'];
  foreach($items as &$it){
    $it['shipped_qty']=$shipped[(int)$it['id']]??0;
    if(qo_is_virtual_item($it)){ $it['remain_qty']=0; $it['packaging_profile']=new stdClass(); $it['packaging_options']=[]; continue; }
    $it['remain_qty']=max(0,qo_num($it['qty'])-qo_num($it['shipped_qty']));
    $it['packaging_options']=qo_pack_options($pdo,$it['product_code']??'',$it['customer_code']??'');
    $it['packaging_profile']=$it['packaging_options'][0] ?? (qo_pack_match($pdo,$it['product_code']??'',$it['customer_code']??'') ?: new stdClass());
  }
  unset($it);
  return $items;
}
...
function qo_customer_key(array $order): string {
  $stableId=qo_s($order['customer_id']??'',120);
  if($stableId!=='' && $stableId!=='0') return 'id:'.$stableId;
...
function qo_load_orders(PDO $pdo,array $orderIds): array {
  ...
  $rows=qo_rows($pdo,'SELECT '.qr_order_columns($pdo).' FROM quote_sales_orders WHERE id IN ('.$ph.')',$ids);
  $map=[]; foreach($rows as $r){ $r['order_no']=qo_order_ref($r); $map[(int)$r['id']]=$r; }
  $out=[]; foreach($ids as $id){ if(isset($map[$id])) $out[]=$map[$id]; }
  return $out;
}
function qo_validate_same_customer_orders(array $orders): void {
  if(!$orders) qo_fail('没有可合并的订单');
  $key=null; $currency=null;
  foreach($orders as $o){
    $k=qo_customer_key($o); if($key===null) $key=$k; elseif($k!==$key) qo_fail('只能合并同一客户的订单出货');
    $c=qo_s($o['currency']??'',20); if($currency===null) $currency=$c; elseif($c!=='' && $currency!=='' && $c!==$currency) qo_fail('合并出货的订单币种必须一致');
  }
}
function qo_shipment_order_ids(PDO $pdo,int $shipmentId,array $shipment=[]): array {
  $rows=qo_rows($pdo,'SELECT order_id FROM quote_shipment_orders WHERE shipment_id=? ORDER BY sort_order,id',[$shipmentId]);
  $ids=[]; foreach($rows as $r){ $id=(int)($r['order_id']??0); if($id>0) $ids[]=$id; }
  if(!$ids && !empty($shipment['order_id'])) $ids[]=(int)$shipment['order_id'];
  return array_values(array_unique($ids));
}
function qo_shipment_orders(PDO $pdo,int $shipmentId,array $shipment=[]): array {
  $ids=qo_shipment_order_ids($pdo,$shipmentId,$shipment);
  return qo_load_orders($pdo,$ids);
}
function qo_sync_shipment_orders(PDO $pdo,int $shipmentId,array $orders): void {
  $pdo->prepare('DELETE FROM quote_shipment_orders WHERE shipment_id=?')->execute([$shipmentId]);
  $ins=$pdo->prepare('INSERT INTO quote_shipment_orders(shipment_id,order_id,order_no,quote_no,customer_id,customer_name,sort_order,created_at) VALUES(?,?,?,?,?,?,?,NOW())');
  foreach(array_values($orders) as $i=>$o){
    $ins->execute([$shipmentId,(int)$o['id'],qo_order_ref($o),qo_s($o['quote_no']??'',120),qo_s($o['customer_id']??'',120),qo_s($o['customer_name']??'',255),$i+1]);
  }
}
function qo_prepare_context(PDO $pdo,int $orderId): array {
  $order=qr_order($pdo,$orderId);if(!$order) qo_fail('订单不存在');
  $order['order_no']=qo_order_ref($order);
  if(in_array($order['status']??'',['已作废','取消'],true)) qo_fail('取消或作废的订单不能出货');
  return ['order'=>$order,'payment_summary'=>qr_payment($pdo,$order)];
}
function qo_prepare_combined_shipment(PDO $pdo,$orderId,bool $summaryOnly=false){
  $detail=qo_prepare_context($pdo,(int)$orderId);
  $base=$detail['order']??[]; $baseCustomerName=qo_s($base['customer_name']??'',255);
  $columns='id,order_no,quote_no,customer_id,customer_name,currency,status,order_date,created_at';
  if(qo_s($base['customer_id']??'',120)!=='' && $baseCustomerName!==''){
    $orders=qo_rows($pdo,"SELECT {$columns} FROM quote_sales_orders WHERE (customer_id=? OR customer_name=?) AND currency=? AND COALESCE(status,'') NOT IN ('已作废','取消') ORDER BY COALESCE(order_date,created_at),id",[$base['customer_id']??'',$baseCustomerName,$base['currency']]);
  }elseif(qo_s($base['customer_id']??'',120)!==''){
    $orders=qo_rows($pdo,"SELECT {$columns} FROM quote_sales_orders WHERE customer_id=? AND currency=? AND COALESCE(status,'') NOT IN ('已作废','取消') ORDER BY COALESCE(order_date,created_at),id",[$base['customer_id']??'',$base['currency']]);
  }elseif($baseCustomerName!==''){
    $orders=qo_rows($pdo,"SELECT {$columns} FROM quote_sales_orders WHERE customer_name=? AND currency=? AND COALESCE(status,'') NOT IN ('已作废','取消') ORDER BY COALESCE(order_date,created_at),id",[$baseCustomerName,$base['currency']]);
  }else{
    $orders=[$base];
  }
  $orders=array_values(array_filter($orders,function($o) use($base){ return qo_s($o['currency']??'',20)===qo_s($base['currency']??'',20); }));
  qo_validate_same_customer_orders($orders);
  if($summaryOnly){$detail['combined_shipment']=1;$detail['orders']=$orders;$detail['items']=[];return $detail;}
  $items=[]; $shipOrderList=[];
  foreach($orders as $o){
    $oid=(int)$o['id']; $ref=qo_order_ref($o); $hasRemain=false;
    foreach(qo_prepare_items($pdo,$oid) as $it){
      if(qo_num($it['remain_qty']??0)<=0) continue;
      $it['order_id']=$oid; $it['source_order_id']=$oid; $it['order_no']=$ref; $it['source_order_no']=$ref; $it['quote_no']=$o['quote_no']??'';
      $items[]=$it; $hasRemain=true;
    }
    if($hasRemain || $oid===(int)$orderId){ $o['order_no']=$ref; $shipOrderList[]=$o; }
  }
  $detail['combined_shipment']=1;
  $detail['orders']=$shipOrderList;
  $detail['items']=$items;
  return $detail;
}
function qo_requested_order_ids(array $d,int $fallbackOrderId): array {
  $ids=is_array($d['order_ids']??null)?$d['order_ids']:[$fallbackOrderId];
  return array_values(array_unique(array_filter(array_map('intval',$ids),function($x){return $x>0;})));
}
function qo_recalc_orders(PDO $pdo,array $orderIds): void {
  foreach(array_values(array_unique(array_map('intval',$orderIds))) as $oid){ if($oid>0){ qo_update_item_shipped($pdo,$oid); qo_recalc_payment($pdo,$oid); } }
}
function qo_shipment_validate_multi_items(PDO $pdo,array $orderIds,$shipmentId,array $itemsIn): array {
  if(!$itemsIn) qo_fail('请至少保留一项出货产品');
  $orders=qo_load_orders($pdo,$orderIds); qo_validate_same_customer_orders($orders);
  if(count($orders)!==count(array_unique(array_map('intval',$orderIds)))) qo_fail('部分订单不存在，请重新加载');
  foreach($orders as $o)if(in_array($o['status']??'',['已作废','取消'],true))qo_fail('取消或作废的订单不能出货');
  $ids=array_map(function($o){return (int)$o['id'];},$orders); if(!$ids) qo_fail('缺少订单ID');
  $ph=implode(',',array_fill(0,count($ids),'?'));
  $orderItems=[];
  foreach(qo_rows($pdo,'SELECT '.qr_item_columns($pdo).' FROM quote_sales_order_items WHERE order_id IN ('.$ph.')',$ids) as $it){ $orderItems[(int)$it['id']]=$it; }
  $clean=[]; $qtyByItem=[]; $tot=['qty'=>0,'cartons'=>0,'nw'=>0,'gw'=>0,'cbm'=>0];
  foreach($itemsIn as $x){
    $oid=(int)($x['order_item_id']??0); $it=$orderItems[$oid]??null;
    if(!$it) qo_fail('出货产品不属于本次合并订单');
    if(qo_is_virtual_item($it)) qo_fail('费用项不能生成出货批次：'.($it['product_name']??'费用项'));
    $qty=qo_num($x['qty']??0); if($qty<=0) continue;
    $x['qty']=$qty; $x['order_id']=(int)$it['order_id']; $clean[]=['input'=>$x,'item'=>$it];
  ...
  foreach($qtyByItem as $oid=>$qty){
    $it=$orderItems[(int)$oid]??[];
    $other=(float)(qo_row($pdo,'SELECT COALESCE(SUM(qty),0) AS s FROM quote_shipment_items WHERE order_item_id=? AND shipment_id<>?',[(int)$oid,$shipmentId])['s']??0);
    $reserved=function_exists('qb_reserved')?qb_reserved($pdo,(int)$oid):0;
    $available=max(0,qo_num($it['qty']??0)-$other-$reserved);
    if($qty>$available+0.00001) qo_fail('出货数量超过订单剩余可出数量：'.($it['product_code']??'产品'));
  }
  $active=[]; foreach($clean as $row) $active[(int)$row['item']['order_id']]=true;
  $orders=array_values(array_filter($orders,function($order)use($active){return isset($active[(int)$order['id']]);}));
  return ['rows'=>$clean,'totals'=>$tot,'orders'=>$orders];
}
function qo_create_shipment(PDO $pdo,$d,$quick=false){
  return qr_with_order_locks($pdo,qo_requested_order_ids($d,(int)($d['order_id']??0)),static function()use($pdo,$d,$quick){return qo_create_shipment_locked($pdo,$d,$quick);});
}
function qo_create_shipment_locked(PDO $pdo,$d,$quick=false,bool $insideBatchTransaction=false){
  if(!$insideBatchTransaction){qo_commission_schema($pdo);qo_ensure_schema($pdo);}
  elseif(!$pdo->inTransaction())throw new RuntimeException('Missing batch transaction');
  $orderId=(int)($d['order_id']??0); if($orderId<=0) qo_fail('缺少订单ID');
  $orderIds=qo_requested_order_ids($d,$orderId); $orders=qo_load_orders($pdo,$orderIds); if(!$orders) qo_fail('订单不存在');
  qo_validate_same_customer_orders($orders); $order=$orders[0]; $orderId=(int)$order['id'];
  ...
  if(!$itemsIn && !array_key_exists('items',$d)){
    foreach($orders as $o){ foreach(qo_prepare_items($pdo,(int)$o['id']) as $it){ if(qo_num($it['remain_qty'])<=0) continue; $p=is_array($it['packaging_profile']??null)?$it['packaging_profile']:[]; $qty=qo_num($it['remain_qty']); $pcs=qo_num($p['pcs_per_ctn']??0); if($pcs<=0)$pcs=$qty>0?$qty:1; $ctns=$pcs>0?ceil($qty/$pcs):1; $size=(string)($p['carton_size']??''); $nw=qo_num($p['carton_nw']??0)>0?qo_num($p['carton_nw'])*$ctns:qo_num($p['unit_nw']??0)*$qty; $gw=qo_num($p['carton_gw']??0)>0?qo_num($p['carton_gw'])*$ctns:qo_num($p['unit_gw']??0)*$qty; $cbm=qo_num($p['carton_cbm']??0)>0?qo_num($p['carton_cbm'])*$ctns:qo_dim_cbm($size,$ctns); $itemsIn[]=['order_id'=>(int)$o['id'],'order_item_id'=>(int)$it['id'],'qty'=>$qty,'pcs_per_ctn'=>$pcs,'cartons'=>$ctns,'carton_size'=>$size,'nw'=>$nw,'gw'=>$gw,'cbm'=>$cbm,'note'=>'']; } }
  }
  ...
  $tot=$checked['totals']; $rows=$checked['rows']; $orders=$checked['orders'];
  $order=$orders[0];$orderId=(int)$order['id'];
  $balance=0;$currency=$order['currency']??'USD';
  foreach($orders as $o){$pay=qo_recalc_payment($pdo,(int)$o['id']);$balance+=qo_num($pay['balance_amount']??0);}
  if(!$quick && empty($d['force_shipment']) && $balance>0.01) qo_fail((count($orders)>1?'合并出货订单':'订单').'尾款未收齐，当前未收：'.$currency.' '.number_format($balance,2));
  $tot=qo_add_carton_detail_totals($tot,$cartons);
  if(!$insideBatchTransaction)$pdo->beginTransaction();
  try {
  ...
  qo_sync_shipment_orders($pdo,$shipmentId,$orders);
  ...
  foreach($rows as $row){ $x=$row['input']; $it=$row['item']; $oid=(int)$it['id']; $rowOrderId=(int)($it['order_id']??$orderId); $qty=qo_num($x['qty']); $unit=qo_num($it['unit_price']??0); $amount=round($qty*$unit,2); $ins->execute([$shipmentId,$rowOrderId,$oid,(int)($it['item_index']??0),$it['customer_code']??'', $it['product_code']??'', $it['product_name']??'', $it['specification']??'', $it['color']??'', $qty, qo_num($x['pcs_per_ctn']??0), qo_num($x['cartons']??0), qo_s($x['carton_size']??'',160), qo_num($x['nw']??0), qo_num($x['gw']??0), qo_num($x['cbm']??0), $unit, $amount, $it['image']??'', qo_s($x['note']??'',5000), $it['item_json']??'']); qr_copy_shipment_payload($pdo,(int)$pdo->lastInsertId(),(int)$it['id']); }
  ...
  qo_recalc_orders($pdo,array_map(function($o){return (int)$o['id'];},$orders));
  if(!$insideBatchTransaction)$pdo->commit();
  } catch(Throwable $e) {if(!$insideBatchTransaction&&$pdo->inTransaction())$pdo->rollBack();throw $e;}
  if($insideBatchTransaction)return ['shipment_id'=>$shipmentId,'shipment_no'=>$shipmentNo,'totals'=>$tot];
  $orderRefs=implode(', ',array_map('qo_order_ref',$orders));
  ...
  return ['shipment_id'=>$shipmentId,'shipment_no'=>$shipmentNo,'packing_list_no'=>$plNo,'commercial_invoice_no'=>$ciNo,'order_ids'=>array_map(function($o){return (int)$o['id'];},$orders),'combined_order_count'=>count($orders),'totals'=>$tot];
}
...
function qo_shipment_edit_data(PDO $pdo,$shipmentId){
  ...
  $orders=$detail['orders']??[]; if(!$orders) $orders=[($detail['order']??[])];
  $order=$detail['order']??[]; $orderIds=array_map(function($o){return (int)($o['id']??0);},$orders);
  $current=[];
  foreach(($detail['items']??[]) as $row){ $current[(int)($row['order_item_id']??0)][]=$row; }
  $items=[];
  foreach($orders as $o){
    $oid=(int)($o['id']??0); if($oid<=0) continue;
    foreach(qo_rows($pdo,'SELECT '.qr_item_columns($pdo).' FROM quote_sales_order_items WHERE order_id=? ORDER BY item_index,id',[$oid]) as $it){
      $it['order_no']=qo_order_ref($o); $it['quote_no']=$o['quote_no']??''; $items[]=$it;
    }
  }
  $expanded=[];
  foreach($items as &$item){
    if(qo_is_virtual_item($item)){ $item['remain_qty']=0; $item['shipment_item']=new stdClass(); $item['packaging_profile']=new stdClass(); $item['packaging_options']=[]; continue; }
    $itemId=(int)$item['id'];
    ...
    $item['shipment_item']=$item['shipment_items'][0]??new stdClass();
    $item['packaging_options']=qo_pack_options($pdo,$item['product_code']??'',$item['customer_code']??'');
    $item['packaging_profile']=$item['packaging_options'][0] ?? (qo_pack_match($pdo,$item['product_code']??'',$item['customer_code']??'') ?: new stdClass());
    if($item['shipment_items']){
      foreach($item['shipment_items'] as $shipItem){ $copy=$item; $copy['shipment_item']=$shipItem; $expanded[]=$copy; }
    } else {
      $expanded[]=$item;
    }
  }
  ...
  $detail['payment_summary']=qo_recalc_payment($pdo,(int)($order['id']??0));
  return $detail;
}
function qo_shipment_validate_items(PDO $pdo,$orderId,$shipmentId,array $itemsIn): array {
  return qo_shipment_validate_multi_items($pdo,[(int)$orderId],$shipmentId,$itemsIn);
}
function qo_update_shipment(PDO $pdo,$d){
  $shipment=qo_row($pdo,'SELECT order_id FROM quote_shipments WHERE id=?',[(int)($d['shipment_id']??0)]);
  $ids=qo_shipment_order_ids($pdo,(int)($d['shipment_id']??0),$shipment?:[]);
  return qr_with_order_locks($pdo,$ids,static function()use($pdo,$d){return qo_update_shipment_locked($pdo,$d);});
}
function qo_update_shipment_locked(PDO $pdo,$d){
  ...
  $orderId=(int)$shipment['order_id']; $order=qo_row($pdo,'SELECT '.qr_order_columns($pdo).' FROM quote_sales_orders WHERE id=? LIMIT 1',[$orderId]); if(!$order) qo_fail('关联订单不存在');
  $orderIds=qo_shipment_order_ids($pdo,$shipmentId,$shipment);
  $checked=qo_shipment_validate_multi_items($pdo,$orderIds,$shipmentId,is_array($d['items']??null)?$d['items']:[]);
  ...
  try {
    ...
    qo_sync_shipment_orders($pdo,$shipmentId,$checked['orders']??[]);
    $orderId=(int)$checked['orders'][0]['id'];
    $pdo->prepare('UPDATE quote_shipments SET order_id=? WHERE id=?')->execute([$orderId,$shipmentId]);
    $ins=$pdo->prepare('INSERT INTO quote_shipment_items(shipment_id,order_id,order_item_id,item_index,customer_code,product_code,product_name,specification,color,qty,pcs_per_ctn,cartons,carton_size,nw,gw,cbm,unit_price,amount,image,note,item_json) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)');
    foreach($rows as $row){ $x=$row['input']; $it=$row['item']; $rowOrderId=(int)($it['order_id']??$orderId); $unit=qo_num($it['unit_price']??0); $qty=qo_num($x['qty']); $ins->execute([$shipmentId,$rowOrderId,(int)$it['id'],(int)($it['item_index']??0),$it['customer_code']??'', $it['product_code']??'', $it['product_name']??'', $it['specification']??'', $it['color']??'', $qty,qo_num($x['pcs_per_ctn']??0),qo_num($x['cartons']??0),qo_s($x['carton_size']??'',160),qo_num($x['nw']??0),qo_num($x['gw']??0),qo_num($x['cbm']??0),$unit,round($qty*$unit,2),$it['image']??'',qo_s($x['note']??'',5000),$it['item_json']??'']); qr_copy_shipment_payload($pdo,(int)$pdo->lastInsertId(),(int)$it['id']); }
    if($cartons){ $ci=$pdo->prepare('INSERT INTO quote_shipment_cartons(shipment_id,order_id,carton_no,carton_range,items_text,items_json,qty,carton_size,nw,gw,cbm,mark,note,carton_count) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)'); foreach($cartons as $c){ $ci->execute([$shipmentId,$orderId,qo_s($c['carton_no']??'',120),qo_s($c['carton_range']??'',160),qo_s($c['items_text']??'',5000),json_encode($c['items']??[],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES),qo_num($c['qty']??0),qo_s($c['carton_size']??'',160),qo_num($c['nw']??0),qo_num($c['gw']??0),qo_num($c['cbm']??0),qo_s($c['mark']??'',255),qo_s($c['note']??'',5000),qo_num($c['carton_count']??1)]); } }
    qo_recalc_orders($pdo,$orderIds); $pdo->commit();
...
function qo_delete_shipment(PDO $pdo,$d){
  $shipment=qo_row($pdo,'SELECT order_id FROM quote_shipments WHERE id=?',[(int)($d['shipment_id']??0)]);
  $ids=qo_shipment_order_ids($pdo,(int)($d['shipment_id']??0),$shipment?:[]);
  return qr_with_order_locks($pdo,$ids,static function()use($pdo,$d){return qo_delete_shipment_locked($pdo,$d);});
}
function qo_delete_shipment_locked(PDO $pdo,$d){
  ...
  $orderId=(int)$shipment['order_id']; $orderIds=qo_shipment_order_ids($pdo,$shipmentId,$shipment); $shipmentNo=(string)($shipment['shipment_no']??'');
  $pdo->beginTransaction();
  try { $pdo->prepare('DELETE FROM quote_shipment_cartons WHERE shipment_id=?')->execute([$shipmentId]); $pdo->prepare('DELETE FROM quote_shipment_items WHERE shipment_id=?')->execute([$shipmentId]); $pdo->prepare('DELETE FROM quote_shipment_orders WHERE shipment_id=?')->execute([$shipmentId]); $pdo->prepare('DELETE FROM quote_shipments WHERE id=?')->execute([$shipmentId]); qo_recalc_orders($pdo,$orderIds); $pdo->commit(); }
  catch(Throwable $e){ if($pdo->inTransaction()) $pdo->rollBack(); throw $e; }
  return ['deleted'=>1,'shipment_no'=>$shipmentNo,'order_id'=>$orderId,'order_ids'=>$orderIds];
}
...
function qo_mark_document_active(PDO $pdo,int $shipmentId,string $type): void {
  ...
  if($type==='ci'){
    $pdo->prepare("UPDATE quote_shipments SET ci_generated_at=COALESCE(ci_generated_at,NOW()),ci_status='active',ci_voided_at=NULL,ci_voided_by='',ci_void_reason='',ci_deleted_at=NULL,ci_deleted_by='',ci_delete_reason='',status=CASE WHEN COALESCE(status,'') IN ('','草稿') THEN '已生效' ELSE status END,updated_at=NOW() WHERE id=?")->execute([$shipmentId]);
  }else{
    $pdo->prepare("UPDATE quote_shipments SET pl_generated_at=COALESCE(pl_generated_at,NOW()),pl_status='active',pl_voided_at=NULL,pl_voided_by='',pl_void_reason='',pl_deleted_at=NULL,pl_deleted_by='',pl_delete_reason='',status=CASE WHEN COALESCE(status,'') IN ('','草稿') THEN '已生效' ELSE status END,updated_at=NOW() WHERE id=?")->execute([$shipmentId]);
  }
}
function qo_manage_document(PDO $pdo,array $d){
  qo_ensure_schema($pdo);
  $shipmentId=(int)($d['shipment_id']??0); if($shipmentId<=0) qo_fail('缺少出货批次ID');
  if(qo_table_exists($pdo,'quote_shipment_plans')&&qo_row($pdo,'SELECT id FROM quote_shipment_plans WHERE shipment_id=?',[$shipmentId]))qo_fail('此单证属于新版出货批次，请在装运工作台查看修订版本；不能覆盖已签发记录');
  ...
  $order=qo_row($pdo,'SELECT '.qr_order_columns($pdo).' FROM quote_sales_orders WHERE id=? LIMIT 1',[(int)$before['order_id']]) ?: [];
...
function qo_shipment_items_have_content(PDO $pdo,$shipmentId){
  $rows=qo_rows($pdo,'SELECT '.qr_item_columns($pdo,'quote_shipment_items').' FROM quote_shipment_items WHERE shipment_id=? ORDER BY item_index,id',[$shipmentId]);
  foreach($rows as $r){ if(qo_item_row_has_content($r)) return true; }
  return false;
}
function qo_rebuild_shipment_items_from_order(PDO $pdo,$shipment){
  return qr_with_order_locks($pdo,[(int)($shipment['order_id']??0)],static function()use($pdo,$shipment){
    $pdo->beginTransaction();try{qo_rebuild_shipment_items_locked($pdo,$shipment);$pdo->commit();}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
  });
}
function qo_rebuild_shipment_items_locked(PDO $pdo,$shipment){
...
function qo_existing_or_quick_shipment(PDO $pdo,$orderId,$type='pl'){
  ...
  if($r){ if(!qo_shipment_items_have_content($pdo,(int)$r['id'])) qo_fail('历史出货明细缺失，请先核对该批次；未自动重建或改写历史数据'); return qo_row($pdo,'SELECT * FROM quote_shipments WHERE id=? LIMIT 1',[(int)$r['id']]) ?: $r; }
  qo_fail('请进入装运工作台选择产品并保存计划；预览单证不再自动创建出货记录');
}
function qo_shipment_read_totals(PDO $pdo,array $shipments): array {
  $ids=array_values(array_unique(array_filter(array_map(function($s){return (int)$s['id'];},$shipments))));
  if(!$ids)return $shipments;
  $rows=qo_rows($pdo,"SELECT si.shipment_id,SUM(si.qty) AS qty,COUNT(DISTINCT IF(si.qty>0,si.order_id,NULL)) AS order_count,GROUP_CONCAT(DISTINCT IF(si.qty>0,o.order_no,NULL) ORDER BY o.order_no SEPARATOR ', ') AS order_nos FROM quote_shipment_items si LEFT JOIN quote_sales_orders o ON o.id=si.order_id WHERE si.shipment_id IN (".implode(',',array_fill(0,count($ids),'?')).") GROUP BY si.shipment_id",$ids);
  $map=[];foreach($rows as $row)$map[(int)$row['shipment_id']]=$row;
  foreach($shipments as &$shipment){
    if(!isset($map[(int)$shipment['id']]))continue;
    $sum=$map[(int)$shipment['id']];$shipment['total_qty']=qo_num($sum['qty']);
    $shipment['combined_order_count']=(int)$sum['order_count'];$shipment['combined_order_nos']=$sum['order_nos']??'';
    if((int)$sum['order_count']===1)$shipment['order_no']=$sum['order_nos'];
  }unset($shipment);
  return $shipments; // Read projection only: never rewrite historical rows.
}
function qo_shipment_detail(PDO $pdo,$shipmentId){
  $shipment=qo_row($pdo,'SELECT * FROM quote_shipments WHERE id=? LIMIT 1',[$shipmentId]); if(!$shipment) qo_fail('出货批次不存在');
  $order=qo_row($pdo,'SELECT '.qr_order_columns($pdo).' FROM quote_sales_orders WHERE id=? LIMIT 1',[(int)$shipment['order_id']]);
  $orders=qo_shipment_orders($pdo,(int)$shipmentId,$shipment);
  if(!$orders && $order) $orders=[$order];
  $items=qo_rows($pdo,'SELECT '.qr_item_columns($pdo,'quote_shipment_items','si').',o.order_no,o.quote_no,o.customer_name FROM quote_shipment_items si LEFT JOIN quote_sales_orders o ON o.id=si.order_id WHERE si.shipment_id=? ORDER BY si.order_id,si.item_index,si.id',[$shipmentId]);
  $cartons=qo_rows($pdo,'SELECT * FROM quote_shipment_cartons WHERE shipment_id=? ORDER BY id',[$shipmentId]);
  $shipment=qo_shipment_read_totals($pdo,[$shipment])[0];
  if($items){
    $active=[];foreach($items as $it)if(qo_num($it['qty'])>0)$active[(int)$it['order_id']]=true;
    $orders=array_values(array_filter($orders,function($o)use($active){return isset($active[(int)$o['id']]);}));
    if($orders && !isset($active[(int)($order['id']??0)]))$order=$orders[0];
  }
  foreach($items as &$it){ $it['order_no']=qo_order_no_at($it['order_no']??'',$it['quote_no']??''); }
  unset($it);
  return ['shipment'=>$shipment,'order'=>$order,'orders'=>$orders,'combined_order_count'=>count($orders),'items'=>$items,'cartons'=>$cartons,'settings'=>qo_doc_settings($pdo)];
}
...
function qo_statement_month_value($v): string {
  $m=qo_s($v,20);
  if(preg_match('/^\d{4}-\d{2}$/',$m)) return $m;
  if(preg_match('/^\d{4}\/\d{2}$/',$m)) return str_replace('/','-',$m);
  return date('Y-m');
}
function qo_statement_customer_key(array $o): string {
  $id=qo_s($o['customer_id']??'',255);
  if($id!=='') return 'id:'.$id;
  $name=qo_s($o['customer_name']??'',255);
  if($name!=='') return 'name:'.(function_exists('mb_strtolower')?mb_strtolower($name,'UTF-8'):strtolower($name));
  return 'order_customer:unknown';
}
function qo_statement_status(float $amount,float $paid,float $deduct,float $writeoff): string {
  $reduced=$paid+$deduct+$writeoff;
  if($amount<=0.00001) return '无应收';
  if($reduced<=0.00001) return '未收款';
  return ($amount-$reduced)<=0.00001?'已收齐':'部分收款';
}
function qo_statement_ship_status(float $shippable,float $shipped): string {
  if($shippable<=0.00001) return '无实物';
  if($shipped<=0.00001) return '未出货';
  return $shipped+0.00001>=$shippable?'已出货':'部分出货';
}
function qo_statement_item_row(array $it): array {
  $n=qo_nested_item($it);
  $isVirtual=qo_is_virtual_item($n);
  $qty=qo_num($n['qty']??0);
  $shipped=$isVirtual?0:qo_num($n['shipped_qty']??0);
  $unit=qo_num($n['unit_price']??($n['price']??0));
  $amount=qo_num($n['amount']??($qty*$unit));
  return [
    'id'=>(int)($n['id']??0),
    'item_index'=>(int)($n['item_index']??0),
    'customer_code'=>qo_s($isVirtual?'':($n['customer_code']??''),120),
    'product_code'=>qo_s($n['product_code']??($n['model']??($n['code']??'')),160),
    'product_name'=>qo_s($n['product_name']??($n['name']??($n['title']??'')),255),
    'specification'=>qo_s($n['specification']??($n['description']??''),20000),
    'color'=>qo_s($isVirtual?'':($n['color']??''),120),
    'qty'=>round($qty,3),
    'unit_price'=>round($unit,4),
    'amount'=>round($amount,2),
    'shipped_qty'=>round($shipped,3),
    'remain_qty'=>round($isVirtual?0:max(0,$qty-$shipped),3),
    'is_virtual'=>$isVirtual?1:0
  ];
}
function qo_order_statement_summary(PDO $pdo,array $d): array {
  qo_ensure_schema($pdo);
  $month=qo_statement_month_value($d['month']??'');
  $kw=qo_s($d['kw']??'',160);
  $productKw=qo_s($d['product_kw']??($d['product']??''),160);
  $currency=qo_s($d['currency']??'',20);
  $paymentFilter=qo_s($d['payment_status']??'',50);
  $shipmentFilter=qo_s($d['shipment_status']??'',50);
  $start=$month.'-01';
  $next=date('Y-m-d',strtotime($start.' +1 month'));
  $where=["COALESCE(o.status,'') NOT IN ('已作废','取消')","DATE(COALESCE(o.order_date,o.quote_date,o.created_at))>=?","DATE(COALESCE(o.order_date,o.quote_date,o.created_at))<?"];
  $args=[$start,$next];
  if($kw!==''){
    $where[]='(o.order_no LIKE ? OR o.quote_no LIKE ? OR o.customer_name LIKE ? OR o.customer_id LIKE ? OR o.user_name LIKE ? OR o.note LIKE ?)';
    for($i=0;$i<6;$i++) $args[]='%'.$kw.'%';
  }
  if($productKw!==''){
    $where[]="(
      EXISTS (
        SELECT 1 FROM quote_sales_order_items pi
        WHERE pi.order_id=o.id AND (
          pi.product_code LIKE ? OR pi.product_name LIKE ? OR pi.specification LIKE ? OR pi.customer_code LIKE ? OR pi.color LIKE ? OR pi.item_json LIKE ?
        )
      ) OR o.items_json LIKE ? OR o.snapshot_json LIKE ?
    )";
    for($i=0;$i<8;$i++) $args[]='%'.$productKw.'%';
  }
  if($currency!==''){ $where[]='o.currency=?'; $args[]=$currency; }
  $virtualSql=qo_virtual_item_sql_expr('i');
  $sql="SELECT
      o.id,o.order_no,o.quote_no,o.customer_id,o.customer_name,o.amount,o.currency,o.qty,o.order_date,o.quote_date,o.created_at,o.user_name,o.status,o.note,
      COALESCE(pay.paid_amount,0) AS paid_calc,
      COALESCE(pay.commission_deduct_amount,0) AS commission_deduct_calc,
      COALESCE(pay.writeoff_amount,0) AS writeoff_calc,
      COALESCE(items.shippable_qty,0) AS shippable_qty,
      COALESCE(items.item_count,0) AS item_count,
      COALESCE(ship.shipped_qty,0) AS shipped_qty,
      COALESCE(ship.shipment_count,0) AS shipment_count,
      COALESCE(ship.cartons,0) AS cartons,
      COALESCE(ship.last_ship_date,'') AS last_ship_date,
      COALESCE(ship.shipment_nos,'') AS shipment_nos,
      COALESCE(ship.pl_nos,'') AS pl_nos,
      COALESCE(ship.ci_nos,'') AS ci_nos,
      o.items_json,o.snapshot_json
    FROM quote_sales_orders o
    LEFT JOIN (
      SELECT order_id,
        SUM(COALESCE(amount,0)) AS paid_amount,
        SUM(COALESCE(commission_deduct_amount,0)) AS commission_deduct_amount,
        SUM(COALESCE(writeoff_amount,0)) AS writeoff_amount
      FROM quote_order_payments GROUP BY order_id
    ) pay ON pay.order_id=o.id
    LEFT JOIN (
      SELECT i.order_id,
        SUM(CASE WHEN {$virtualSql} THEN 0 ELSE COALESCE(i.qty,0) END) AS shippable_qty,
        COUNT(*) AS item_count
      FROM quote_sales_order_items i GROUP BY i.order_id
    ) items ON items.order_id=o.id
    LEFT JOIN (
      SELECT si.order_id,
        SUM(COALESCE(si.qty,0)) AS shipped_qty,
        SUM(COALESCE(si.cartons,0)) AS cartons,
        COUNT(DISTINCT si.shipment_id) AS shipment_count,
        MAX(s.ship_date) AS last_ship_date,
        GROUP_CONCAT(DISTINCT NULLIF(s.shipment_no,'') ORDER BY s.ship_date,s.id SEPARATOR ', ') AS shipment_nos,
        GROUP_CONCAT(DISTINCT NULLIF(s.packing_list_no,'') ORDER BY s.ship_date,s.id SEPARATOR ', ') AS pl_nos,
        GROUP_CONCAT(DISTINCT NULLIF(s.commercial_invoice_no,'') ORDER BY s.ship_date,s.id SEPARATOR ', ') AS ci_nos
      FROM quote_shipment_items si
      JOIN quote_shipments s ON s.id=si.shipment_id
      WHERE COALESCE(s.status,'') NOT IN ('已作废','取消')
      GROUP BY si.order_id
    ) ship ON ship.order_id=o.id
    WHERE ".implode(' AND ',$where)."
    ORDER BY o.customer_name ASC,COALESCE(o.order_date,o.quote_date,o.created_at) DESC,o.id DESC";
  $rows=qo_rows($pdo,$sql,$args);
  $itemMap=[];
  $orderIds=array_values(array_unique(array_filter(array_map('intval',array_column($rows,'id')))));
  if($orderIds){
    $ph=implode(',',array_fill(0,count($orderIds),'?'));
    foreach(qo_rows($pdo,"SELECT ".qr_item_columns($pdo)." FROM quote_sales_order_items WHERE order_id IN ($ph) ORDER BY order_id,item_index,id",$orderIds) as $it){
      $itemMap[(int)$it['order_id']][]=qo_statement_item_row($it);
    }
  }
  $notes=qo_rows($pdo,'SELECT * FROM quote_order_statement_notes WHERE statement_month=?',[$month]);
  $noteMap=[];
  foreach($notes as $n){ $noteMap[($n['customer_key']??'').'|'.($n['statement_month']??'').'|'.($n['currency']??'')]=$n; }
  $groups=[];$tot=['order_count'=>0,'amount'=>0,'paid'=>0,'commission_deduct'=>0,'writeoff'=>0,'balance'=>0,'shippable_qty'=>0,'shipped_qty'=>0,'remain_qty'=>0,'shipment_count'=>0,'cartons'=>0];$totByCurrency=[];
  foreach($rows as $r){
    $amount=qo_num($r['amount']??0);
    $paid=qo_num($r['paid_calc']??0);
    $deduct=qo_num($r['commission_deduct_calc']??0);
    $writeoff=qo_num($r['writeoff_calc']??0);
    $balance=max(0,round($amount-$paid-$deduct-$writeoff,2));
    $shipQty=qo_num($r['shippable_qty']??0);
    $shipped=qo_num($r['shipped_qty']??0);
    $remain=max(0,round($shipQty-$shipped,3));
    $payStatus=qo_statement_status($amount,$paid,$deduct,$writeoff);
    $shipStatus=qo_statement_ship_status($shipQty,$shipped);
    if($paymentFilter!=='' && $paymentFilter!==$payStatus) continue;
    if($shipmentFilter!=='' && $shipmentFilter!==$shipStatus) continue;
    $cur=qo_s($r['currency']??'',20) ?: 'USD';
    $ck=qo_statement_customer_key($r);
    $gk=$ck.'|'.$cur;
    if(!isset($groups[$gk])){
      $note=$noteMap[$ck.'|'.$month.'|'.$cur]??[];
      $groups[$gk]=[
        'customer_key'=>$ck,'customer_id'=>qo_s($r['customer_id']??'',120),'customer_name'=>qo_s($r['customer_name']??'未命名客户',255),'currency'=>$cur,'month'=>$month,
        'order_count'=>0,'amount'=>0,'paid'=>0,'commission_deduct'=>0,'writeoff'=>0,'balance'=>0,'shippable_qty'=>0,'shipped_qty'=>0,'remain_qty'=>0,'shipment_count'=>0,'cartons'=>0,
        'payment_status'=>'','shipment_status'=>'','note'=>$note['note']??'','note_updated_at'=>$note['updated_at']??'','note_updated_by'=>$note['updated_by']??'','orders'=>[]
      ];
    }
    $orderItems=$itemMap[(int)$r['id']]??[];
    if(!$orderItems){
      foreach(qo_order_items_from_order_record($r) as $i=>$raw){
        $fallback=qo_item_row($raw,$i+1);
        if(qo_item_row_has_content($fallback)) $orderItems[]=qo_statement_item_row($fallback);
      }
    }
    $order=[
      'id'=>(int)$r['id'],'order_no'=>qo_order_no_at($r['order_no']??'',$r['quote_no']??''),'quote_no'=>qo_s($r['quote_no']??'',120),
      'order_date'=>qo_s($r['order_date']??($r['quote_date']??$r['created_at']??''),30),'user_name'=>qo_s($r['user_name']??'',120),'status'=>qo_s($r['status']??'',80),
      'amount'=>round($amount,2),'paid'=>round($paid,2),'commission_deduct'=>round($deduct,2),'writeoff'=>round($writeoff,2),'balance'=>round($balance,2),'payment_status'=>$payStatus,
      'shippable_qty'=>round($shipQty,3),'shipped_qty'=>round($shipped,3),'remain_qty'=>round($remain,3),'shipment_status'=>$shipStatus,'shipment_count'=>(int)($r['shipment_count']??0),'cartons'=>round(qo_num($r['cartons']??0),3),
      'last_ship_date'=>qo_s($r['last_ship_date']??'',30),'shipment_nos'=>qo_s($r['shipment_nos']??'',2000),'pl_nos'=>qo_s($r['pl_nos']??'',2000),'ci_nos'=>qo_s($r['ci_nos']??'',2000),'note'=>qo_s($r['note']??'',5000),'items'=>$orderItems
    ];
    $groups[$gk]['orders'][]=$order;
    foreach(['amount','paid','commission_deduct','writeoff','balance','shippable_qty','shipped_qty','remain_qty','cartons'] as $k) $groups[$gk][$k]+=$order[$k];
    $groups[$gk]['shipment_count']+=$order['shipment_count'];
    $groups[$gk]['order_count']++;
  }
  foreach($groups as &$g){
    $g['amount']=round($g['amount'],2);$g['paid']=round($g['paid'],2);$g['commission_deduct']=round($g['commission_deduct'],2);$g['writeoff']=round($g['writeoff'],2);$g['balance']=round($g['balance'],2);
    $g['shippable_qty']=round($g['shippable_qty'],3);$g['shipped_qty']=round($g['shipped_qty'],3);$g['remain_qty']=round($g['remain_qty'],3);$g['cartons']=round($g['cartons'],3);
    $g['payment_status']=qo_statement_status($g['amount'],$g['paid'],$g['commission_deduct'],$g['writeoff']);
    $g['shipment_status']=qo_statement_ship_status($g['shippable_qty'],$g['shipped_qty']);
    $tot['order_count']+=$g['order_count'];$tot['shipment_count']+=$g['shipment_count'];
    foreach(['amount','paid','commission_deduct','writeoff','balance','shippable_qty','shipped_qty','remain_qty','cartons'] as $k) $tot[$k]+=$g[$k];
    $cur=$g['currency']?:'USD';
    if(!isset($totByCurrency[$cur])) $totByCurrency[$cur]=['amount'=>0,'paid'=>0,'commission_deduct'=>0,'writeoff'=>0,'balance'=>0,'order_count'=>0];
    foreach(['amount','paid','commission_deduct','writeoff','balance'] as $k) $totByCurrency[$cur][$k]+=$g[$k];
    $totByCurrency[$cur]['order_count']+=$g['order_count'];
  }
  unset($g);
  foreach(['amount','paid','commission_deduct','writeoff','balance'] as $k) $tot[$k]=round($tot[$k],2);
  foreach(['shippable_qty','shipped_qty','remain_qty','cartons'] as $k) $tot[$k]=round($tot[$k],3);
  foreach($totByCurrency as &$tc){ foreach(['amount','paid','commission_deduct','writeoff','balance'] as $k) $tc[$k]=round($tc[$k],2); }
  unset($tc);
  $list=array_values($groups);
  usort($list,function($a,$b){ if(abs(($b['balance']??0)-($a['balance']??0))>0.00001) return ($b['balance']<=>$a['balance']); return strcmp($a['customer_name']??'',$b['customer_name']??''); });
  return ['month'=>$month,'groups'=>$list,'totals'=>$tot,'totals_by_currency'=>$totByCurrency,'generated_at'=>qo_now()];
}
function qo_statement_export_range(array $d,string $month): array {
  $range=qo_s($d['range']??'month',20);
  $from=qo_s($d['date_from']??'',20);
  $to=qo_s($d['date_to']??'',20);
  if($range==='all') return ['range'=>'all','label'=>'全部','from'=>'','to'=>'','to_next'=>''];
  if($range==='week'){
    $base=qo_s($d['base_date']??'',20);
    if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$base)) $base=date('Y-m-d');
    $ts=strtotime($base) ?: time();
    $dow=(int)date('N',$ts);
    $start=date('Y-m-d',strtotime(date('Y-m-d',$ts).' -'.($dow-1).' days'));
    $end=date('Y-m-d',strtotime($start.' +6 days'));
    return ['range'=>'week','label'=>'本周','from'=>$start,'to'=>$end,'to_next'=>date('Y-m-d',strtotime($end.' +1 day'))];
  }
  if($range==='custom'){
    if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$from)) $from='';
    if(!preg_match('/^\d{4}-\d{2}-\d{2}$/',$to)) $to='';
    return ['range'=>'custom','label'=>'自定义','from'=>$from,'to'=>$to,'to_next'=>$to!==''?date('Y-m-d',strtotime($to.' +1 day')):''];
  }
  $start=$month.'-01';
  $end=date('Y-m-d',strtotime($start.' +1 month -1 day'));
  return ['range'=>'month','label'=>'本月','from'=>$start,'to'=>$end,'to_next'=>date('Y-m-d',strtotime($end.' +1 day'))];
}
function qo_order_statement_export(PDO $pdo,array $d): array {
  qo_ensure_schema($pdo);
  $month=qo_statement_month_value($d['month']??'');
  $customerKey=qo_s($d['customer_key']??'',255);
  $customerName=qo_s($d['customer_name']??'',255);
  if($customerKey==='' && $customerName==='') throw new RuntimeException('缺少客户条件');
  $currency=qo_s($d['currency']??'',20);
  $productKw=qo_s($d['product_kw']??($d['product']??''),160);
  $range=qo_statement_export_range($d,$month);
  $dateExpr="DATE(COALESCE(o.order_date,o.quote_date,o.created_at))";
  $where=["COALESCE(o.status,'') NOT IN ('已作废','取消')"];
  $args=[];
  if($range['from']!==''){ $where[]="$dateExpr>=?"; $args[]=$range['from']; }
  if($range['to_next']!==''){ $where[]="$dateExpr<?"; $args[]=$range['to_next']; }
  if(substr($customerKey,0,3)==='id:'){
    $where[]='o.customer_id=?';
    $args[]=substr($customerKey,3);
  }elseif(substr($customerKey,0,5)==='name:'){
    $where[]='LOWER(o.customer_name)=?';
    $args[]=substr($customerKey,5);
  }elseif($customerName!==''){
  ...
  $orders=qo_rows($pdo,$sql,$args);
  $itemMap=[];
  $orderIds=array_values(array_unique(array_filter(array_map('intval',array_column($orders,'id')))));
  if($orderIds){
    $ph=implode(',',array_fill(0,count($orderIds),'?'));
    foreach(qo_rows($pdo,"SELECT ".qr_item_columns($pdo)." FROM quote_sales_order_items WHERE order_id IN ($ph) ORDER BY order_id,item_index,id",$orderIds) as $it){
      $itemMap[(int)$it['order_id']][]=qo_statement_item_row($it);
  ...
  $flat=[];
  $tot=['order_count'=>0,'item_count'=>0,'amount'=>0,'paid'=>0,'commission_deduct'=>0,'writeoff'=>0,'balance'=>0,'shippable_qty'=>0,'shipped_qty'=>0,'remain_qty'=>0,'shipment_count'=>0,'cartons'=>0];
  foreach($orders as $r){
    $amount=qo_num($r['amount']??0);
    $paid=qo_num($r['paid_calc']??0);
    $deduct=qo_num($r['commission_deduct_calc']??0);
    ...
    $payStatus=qo_statement_status($amount,$paid,$deduct,$writeoff);
    $shipStatus=qo_statement_ship_status($shipQty,$shipped);
    $cur=qo_s($r['currency']??'',20) ?: ($currency?:'USD');
    $orderNo=qo_order_no_at($r['order_no']??'',$r['quote_no']??'');
    $base=[
      'customer_id'=>qo_s($r['customer_id']??'',120),
      'customer_name'=>qo_s($r['customer_name']??'',255),
      'order_no'=>$orderNo,
      'quote_no'=>qo_s($r['quote_no']??'',120),
      'order_date'=>qo_s($r['order_date']??($r['quote_date']??$r['created_at']??''),30),
      'owner'=>qo_s($r['user_name']??'',120),
      'currency'=>$cur,
      'order_amount'=>round($amount,2),
      'paid'=>round($paid,2),
      'commission_deduct'=>round($deduct,2),
      'writeoff'=>round($writeoff,2),
      'balance'=>round($balance,2),
      'payment_status'=>$payStatus,
      'shipment_status'=>$shipStatus,
      'shipment_count'=>(int)($r['shipment_count']??0),
      'shipment_nos'=>qo_s($r['shipment_nos']??'',2000),
      'pl_nos'=>qo_s($r['pl_nos']??'',2000),
      'ci_nos'=>qo_s($r['ci_nos']??'',2000),
      'order_note'=>qo_s($r['note']??'',5000)
    ];
    $items=$itemMap[(int)$r['id']]??[];
    if(!$items){
      foreach(qo_order_items_from_order_record($r) as $i=>$raw){
        $fallback=qo_item_row($raw,$i+1);
        if(qo_item_row_has_content($fallback)) $items[]=qo_statement_item_row($fallback);
      }
    }
    if(!$items) $items=[['item_index'=>'','product_code'=>'','product_name'=>'','specification'=>'','customer_code'=>'','color'=>'','qty'=>'','unit_price'=>'','amount'=>'','shipped_qty'=>'','remain_qty'=>'','is_virtual'=>0]];
    foreach($items as $i=>$it){
      $flat[]=array_merge($base,[
        'item_index'=>$it['item_index']?:($i+1),
        'product_code'=>qo_s($it['product_code']??'',160),
        'product_name'=>qo_s($it['product_name']??'',255),
        'specification'=>qo_s($it['specification']??'',20000),
        'customer_code'=>qo_s($it['customer_code']??'',120),
        'color'=>qo_s($it['color']??'',120),
        'qty'=>$it['qty']??'',
        'unit_price'=>$it['unit_price']??'',
        'item_amount'=>$it['amount']??'',
        'shipped_qty'=>$it['shipped_qty']??'',
        'remain_qty'=>$it['remain_qty']??'',
        'item_type'=>!empty($it['is_virtual'])?'费用项 / 不出货':'实物'
      ]);
      $tot['item_count']++;
    }
    $tot['order_count']++;
    $tot['shipment_count']+=(int)($r['shipment_count']??0);
    foreach(['amount','paid','commission_deduct','writeoff','balance'] as $k){
      $src=['amount'=>$amount,'paid'=>$paid,'commission_deduct'=>$deduct,'writeoff'=>$writeoff,'balance'=>$balance][$k];
      $tot[$k]+=$src;
    }
    $tot['shippable_qty']+=$shipQty;
    $tot['shipped_qty']+=$shipped;
    $tot['remain_qty']+=$remain;
    $tot['cartons']+=qo_num($r['cartons']??0);
  }
  foreach(['amount','paid','commission_deduct','writeoff','balance'] as $k) $tot[$k]=round($tot[$k],2);
  foreach(['shippable_qty','shipped_qty','remain_qty','cartons'] as $k) $tot[$k]=round($tot[$k],3);
  return [
    'customer_key'=>$customerKey,
    'customer_name'=>$customerName ?: ($flat[0]['customer_name']??''),
    'currency'=>$currency ?: ($flat[0]['currency']??''),
    'range'=>$range,
    'product_kw'=>$productKw,
    'totals'=>$tot,
    'rows'=>$flat,
    'generated_at'=>qo_now()
  ];
}
function qo_save_statement_note(PDO $pdo,array $d): array {
  qo_ensure_schema($pdo);
  $month=qo_statement_month_value($d['month']??'');
  $customerKey=qo_s($d['customer_key']??'',255);
  if($customerKey==='') throw new RuntimeException('缺少客户标识');
  $currency=qo_s($d['currency']??'',20) ?: 'USD';
  $customerName=qo_s($d['customer_name']??'',255);
  $note=qo_s($d['note']??'',20000);
  $pdo->prepare('INSERT INTO quote_order_statement_notes(customer_key,customer_name,statement_month,currency,note,updated_by,updated_at,created_at) VALUES(?,?,?,?,?,?,NOW(),NOW()) ON DUPLICATE KEY UPDATE customer_name=VALUES(customer_name),note=VALUES(note),updated_by=VALUES(updated_by),updated_at=NOW()')
    ->execute([$customerKey,$customerName,$month,$currency,$note,qo_actor()]);
  qo_order_log($pdo,'statement_note_saved',['event'=>'保存订单对账备注','customer_id'=>$customerKey,'customer_name'=>$customerName,'summary'=>$customerName.' '.$month.' '.$currency.' 对账备注已保存']);
  return ['saved'=>1,'note'=>$note,'updated_by'=>qo_actor(),'updated_at'=>qo_now()];
}
function qo_save_order_note(PDO $pdo,array $d): array {
  qo_ensure_schema($pdo);
  $id=(int)($d['order_id']??$d['id']??0);
  if(!$id) throw new RuntimeException('缺少订单ID');
  $note=qo_s($d['note']??'',20000);
  $before=qo_row($pdo,'SELECT id,order_no,quote_no,customer_id,customer_name,note FROM quote_sales_orders WHERE id=? LIMIT 1',[$id]);
  if(!$before) throw new RuntimeException('订单不存在');
  $pdo->prepare('UPDATE quote_sales_orders SET note=?,updated_by=?,updated_at=NOW() WHERE id=?')->execute([$note,qo_actor(),$id]);
  $after=qo_row($pdo,'SELECT id,order_no,quote_no,customer_id,customer_name,note FROM quote_sales_orders WHERE id=? LIMIT 1',[$id]);
  qo_order_log($pdo,'order_note_saved',['event'=>'保存订单备注','order_id'=>$id,'order_no'=>$before['order_no']??'','quote_no'=>$before['quote_no']??'','customer_id'=>$before['customer_id']??'','customer_name'=>$before['customer_name']??'','before'=>$before,'after'=>$after,'summary'=>($before['order_no']??$id).' 订单备注已保存']);
  return ['saved'=>1,'order'=>$after];
}
...
function qo_freeze_commission(PDO $pdo,$orderId,$d,$items,$amount,$qty){
  qo_commission_schema($pdo);if(qo_row($pdo,"SELECT id FROM quote_commission_snapshots WHERE order_id=? AND COALESCE(settle_status,'')<>'cancelled' LIMIT 1",[$orderId]))return;
  $customerJson=qo_json($d['customer_json']??'',[]);
  $customerId=qo_s($d['customer_id']??($customerJson['crm_customer_id']??$customerJson['customer_id']??$customerJson['id']??''),120);
  $customerName=qo_s($d['customer_name']??($customerJson['name']??$customerJson['customer_name']??''),255);
  $customerCode=qo_s($customerJson['code']??$customerJson['customer_code']??'',120);
  $norm=function($v){$v=strtolower(trim((string)$v));return preg_replace('/^crm_/', '', $v);};
  $models=[];$cats=[];
  foreach($items as $it){$j=qo_json($it['item_json']??'',[]);$p=is_array($j['product']??null)?$j['product']:[];foreach([$it['product_code']??'',$it['customer_code']??'',$j['product_code']??'',$j['model']??'',$p['code']??'',$p['model']??''] as $m){$m=strtoupper(qo_s($m,120));if($m!=='')$models[$m]=true;}$cat=qo_s($p['category']??$j['category']??'',160);if($cat!=='')$cats[$cat]=true;}
  ...
  foreach($rules as $r){$score=0;$reasons=[];
    ...
    if($ruleProduct!==''){if(!isset($models[$ruleProduct]))continue;$score+=40;$reasons[]='产品型号命中';}
    if($ruleCategory!==''){if(!isset($cats[$ruleCategory]))continue;$score+=20;$reasons[]='产品分类命中';}
    if($score>$bestScore){$best=$r;$bestScore=$score;$bestReasons=$reasons;}
  }
  if(!$best||$bestScore<=0)return;$commission=qo_commission_amount($best['commission_mode'],$best['commission_value'],$amount,$qty);if($commission===null)return;
  $snap=['source'=>'auto_rule_match','rule'=>$best,'order'=>['id'=>$orderId,'order_no'=>$d['order_no']??'','quote_no'=>$d['quote_no']??'','customer_id'=>$customerId,'customer_code'=>$customerCode,'customer_name'=>$customerName,'amount'=>$amount,'qty'=>$qty,'currency'=>$d['currency']??'USD'],'match_score'=>$bestScore,'match_reasons'=>$bestReasons];
...
function qo_commission_order_list(PDO $pdo,$d){
  ...
  $join=" LEFT JOIN quote_commission_snapshots s ON s.id=(SELECT sx.id FROM quote_commission_snapshots sx WHERE sx.order_id=o.id AND COALESCE(sx.settle_status,'')<>'cancelled' ORDER BY (sx.rule_id=0) DESC,sx.id DESC LIMIT 1)";
...
function qo_commission_order_save(PDO $pdo,$d){
  qo_commission_schema($pdo);$orderId=(int)($d['order_id']??0);$o=qo_row($pdo,'SELECT '.qr_order_columns($pdo).' FROM quote_sales_orders WHERE id=?',[$orderId]);if(!$o)throw new RuntimeException('订单不存在');
...
function qo_commission_line_save(PDO $pdo,array $d): array {
  ...
  $it=qo_row($pdo,'SELECT '.qr_item_columns($pdo,'quote_sales_order_items','i').',o.order_no,o.quote_no,o.currency order_currency FROM quote_sales_order_items i JOIN quote_sales_orders o ON o.id=i.order_id WHERE i.id=? LIMIT 1',[$itemId]);
...
function qo_order_detail(PDO $pdo,$id,bool $light=false){
  qo_ensure_schema($pdo); if(!qo_row($pdo,'SELECT id FROM quote_sales_orders WHERE id=? LIMIT 1',[(int)$id])) qo_fail('订单不存在');
  if($light){$order=qr_order($pdo,(int)$id);$pay=qr_payment($pdo,$order);}else{qo_update_item_shipped($pdo,(int)$id);$pay=qo_recalc_payment($pdo,(int)$id);$order=qo_detail_order($pdo,(int)$id);}
  $items=$light?qo_rows($pdo,'SELECT '.qr_item_columns($pdo).' FROM quote_sales_order_items WHERE order_id=? ORDER BY item_index,id',[(int)$id]):qo_detail_items($pdo,(int)$id);
  if($light){
    $shipped=[];foreach(qo_rows($pdo,'SELECT order_item_id,SUM(qty) qty FROM quote_shipment_items WHERE order_id=? GROUP BY order_item_id',[(int)$id]) as $row)$shipped[(int)$row['order_item_id']]=(float)$row['qty'];
    foreach($items as &$item){$item['shipped_qty']=qo_is_virtual_item($item)?0:($shipped[(int)$item['id']]??0);$item['image']=!empty($item['has_image'])&&!qo_is_virtual_item($item)?'quote_order_api.php?action=item_image&id='.(int)$item['id']:'';}unset($item);
    $order['paid_amount']=$pay['paid_amount'];$order['balance_amount']=$pay['balance_amount'];$order['payment_status']=$pay['payment_status'];
  }
  $order['order_no']=qo_order_no_at($order['order_no']??'',$order['quote_no']??'');
  $shipments=qo_rows($pdo,'SELECT DISTINCT s.* FROM quote_shipments s LEFT JOIN quote_shipment_orders so ON so.shipment_id=s.id WHERE EXISTS(SELECT 1 FROM quote_shipment_items si WHERE si.shipment_id=s.id AND si.order_id=? AND si.qty>0) OR (NOT EXISTS(SELECT 1 FROM quote_shipment_items si WHERE si.shipment_id=s.id) AND (s.order_id=? OR so.order_id=?)) ORDER BY s.id DESC',[(int)$id,(int)$id,(int)$id]);
  $shipments=qo_shipment_read_totals($pdo,$shipments);
  qo_commission_schema($pdo);return ['order'=>$order,'items'=>$items,'shipments'=>$shipments,'payments'=>qo_rows($pdo,'SELECT * FROM quote_order_payments WHERE order_id=? ORDER BY payment_date DESC,id DESC',[(int)$id]),'payment_summary'=>$pay,'commission_snapshots'=>qo_rows($pdo,"SELECT * FROM quote_commission_snapshots WHERE order_id=? AND COALESCE(settle_status,'')<>'cancelled' ORDER BY id",[(int)$id]),'commission_lines'=>qo_rows($pdo,'SELECT * FROM quote_commission_lines WHERE order_id=? ORDER BY item_index,id',[(int)$id])];
}
...
try{
  if(strpos($action,'batch_')===0){
    ...
    if(function_exists('artdon_perm_require_action'))artdon_perm_require_action('quote',$op,$features,'view');
    $input=qo_input();
    if(!empty($input['force_shipment'])&&function_exists('artdon_perm_require_action'))artdon_perm_require_action('quote','force',[],'admin');
    if(isset($features[$op])){
    ...
    if($op==='get')qo_ok(qb_get($pdo,(int)($input['id']??0)));
    if($op==='document'){
      if(function_exists('artdon_perm_require_action'))artdon_perm_require_action('quote','document',[],'export');
      $plan=qb_get($pdo,(int)($input['id']??0));$version=(int)($input['version']??0);
      $document=$version?qo_row($pdo,'SELECT version,data_json,actor,created_at FROM quote_shipment_plan_documents WHERE plan_id=? AND version=?',[$plan['id'],$version]):null;
      if($version&&!$document)throw new RuntimeException('签发版本不存在');
      qo_ok(['id'=>$plan['id'],'state'=>$plan['state'],'current_version'=>$plan['version'],'version'=>$version?:$plan['version'],'issued'=>(bool)$document,'actor'=>$document['actor']??'','created_at'=>$document['created_at']??'','data'=>$document?qo_json($document['data_json']):$plan['data']]);
    }
    if(isset($features[$op]))qo_ok(qb_mutate($pdo,$op,$input));
    throw new RuntimeException('未知批次操作');
  }
  if($action==='item_image'){qo_release_session_lock();qr_send_item_image($pdo,(int)($_GET['id']??0));exit;}
  ...
  if($action==='statement_summary') qo_ok(qo_order_statement_summary($pdo,qo_input()));
  if($action==='statement_export') qo_ok(qo_order_statement_export($pdo,qo_input()));
  if($action==='save_statement_note') qo_ok(qo_save_statement_note($pdo,qo_input()));
  if($action==='save_order_note') qo_ok(qo_save_order_note($pdo,qo_input()));
  ...
  if($action==='detail'){ $d=qo_input(); qo_ok(qo_order_detail($pdo,(int)($d['id']??0),!empty($d['light']))); }
  if($action==='convert'){
    $d=qo_input(); qo_input(true);
    qo_ok(qo_convert_order($pdo,$d));
  }
  if($action==='update_status'){ $d=qo_input(); $id=(int)($d['id']??0); $status=qo_s($d['status']??'',80); if(!$id||$status==='') qo_fail('缺少订单ID或状态'); $pdo->prepare('UPDATE quote_sales_orders SET status=?,updated_by=?,updated_at=NOW() WHERE id=?')->execute([$status,qo_actor(),$id]);if($status==='已确认'){qo_commission_schema($pdo);$pdo->prepare("UPDATE quote_commission_snapshots SET settle_status='pending',updated_at=NOW() WHERE order_id=? AND settle_node='order_confirmed' AND settle_status='unsettled'")->execute([$id]);} qo_ok(['order'=>qo_row($pdo,'SELECT '.qr_order_columns($pdo).' FROM quote_sales_orders WHERE id=?',[$id])]); }
  if($action==='commission_snapshot_list'){ $d=qo_input();qo_commission_schema($pdo);qo_ok(['snapshots'=>qo_rows($pdo,"SELECT * FROM quote_commission_snapshots WHERE order_id=? AND COALESCE(settle_status,'')<>'cancelled' ORDER BY id",[(int)($d['order_id']??0)])]); }
  ...
  if($action==='prepare_shipment'){ $d=qo_input(); $id=(int)($d['order_id']??0); $detail=qo_prepare_context($pdo,$id); $items=[]; foreach(qo_prepare_items($pdo,$id) as $x){ if(qo_num($x['remain_qty']??0)<=0) continue; $x['order_id']=$id; $x['order_no']=$detail['order']['order_no']??''; $x['quote_no']=$detail['order']['quote_no']??''; $items[]=$x; } $detail['items']=$items; qo_ok($detail); }
  if($action==='prepare_combined_shipment'){ $d=qo_input(); qo_ok(qo_prepare_combined_shipment($pdo,(int)($d['order_id']??0),!empty($d['summary_only']))); }
  if($action==='create_shipment'){ qo_fail('出货已升级为待装计划，请刷新页面后进入装运工作台；旧页面提交未创建出货记录'); }
  ...
  if($action==='list_documents'){ $sql="SELECT s.*,o.order_no,o.quote_no,o.customer_name,o.currency,o.amount,sox.combined_order_nos,COALESCE(sox.combined_order_count,1) AS combined_order_count FROM quote_shipments s LEFT JOIN quote_sales_orders o ON o.id=s.order_id LEFT JOIN (SELECT shipment_id,GROUP_CONCAT(DISTINCT NULLIF(order_no,'') ORDER BY sort_order SEPARATOR ', ') AS combined_order_nos,COUNT(DISTINCT order_id) AS combined_order_count FROM quote_shipment_orders GROUP BY shipment_id) sox ON sox.shipment_id=s.id ORDER BY s.id DESC LIMIT 1000"; qo_ok(['documents'=>qo_shipment_read_totals($pdo,qo_rows($pdo,$sql))]); }
  if($action==='mark_document_generated'){ $d=qo_input(); $sid=(int)($d['shipment_id']??0); $type=($d['type']??'pl')==='ci'?'ci':'pl'; if($sid){ qo_mark_document_active($pdo,$sid,$type); qo_push_document_notification($pdo,$sid,$type); } qo_ok(['marked'=>1]); }
...
}catch(Throwable $e){ if($pdo->inTransaction())$pdo->rollBack();echo json_encode(['ok'=>false,'msg'=>'订单接口异常：'.$e->getMessage()],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES); }
