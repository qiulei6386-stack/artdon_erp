...
## 2026-09-08：日报明细内容与派工人数补齐（用户批准同步并测试）

- 发布完成：功能提交 `1df59640b1a0bb500bcb1a8ad5ace7c5e4073782` 已推送 main/发布分支并同提交部署广州。独立副本演练及正式8文件限定部署均通过，无关已跟踪/未跟踪文件摘要保持，原文件备份路径仅存私有运维记录。正式559 PHP语法/53后端复检、三个个人范围、全员合计、越权403与历史起点护栏通过；公网JS/CSS摘要与本地相同。正式只读全员后端约0.063秒，不代表浏览器端到端耗时。触发器、初始化起点及业务数据未由本批修改。
- 最后正式浏览器实点遇Mac锁屏，已请用户手动解锁，没有绕过；本批线上展开内容/名单的实际点击仍待解锁后核对，不将离线布局/交互验证冒充正式实点。刷新整个待办页后可加载新版，已有未保存内容应先保存。本条仅补发布记录，按同提交三方同步，产品代码不再变化。
- 用户反馈只有标题、缺少项目内容和清晰类型/人数。基线 `d0add0b`。本地补充项目/说明快照、创建人/派工人、本项负责人、类型与紧凑时间；长内容前120字，可展开且不重复摘要。分组按所选日日终快照统计实际去重人数/名单，周期按同一生成日期，不累计跨天人数；私人/删除项不参与名单，不输出其他人的项目或正文；转出等事件逐项校验名单资格。
- 产品仅日报helper/JS/CSS三文件，另三项测试、使用说明和本记录共8文件。没有改触发器定义或初始化起点，不需数据库管理员重新登录。
- 检查：64 JS/CJS语法/14套既有前端回归通过；最终离线Chrome10组360—1440布局及两组长内容/名单展开、XSS文字显示、空内容、只读和原交互通过，主代理查看最终桌面和手机截图。当前候选559 PHP语法/53套后端检查及独立MySQL内容、人数、隐私、转出、同日周期、历史保持回归通过；3000+合成任务约0.077秒、9.5KB响应、30MiB峰值，仅为隔离数据。测试数据库已停止，没有连接正式业务数据。
- 前轮传输被安全复核拒绝后未绕过；用户本轮明确批准这8文件同步指定仓库及广州并测试，候选上传成功。发布前只读核对正式基线一致，无关线上修改保留。下一步按GitHub双分支→同提交8文件限定部署→正式只读及页面验收执行，发布结果另补记录。不重新初始化数据库，不改真实任务、说明或历史事实。

## 2026-09-08：待办今日总结（个人/管理员）

- 正式页面验收完成：用户解锁后，用现有登录会话打开独立待办页，实际验证今日总结打开、本人/全员/他人切换、完成/应办未完/今日变更分类、空记录提示、第二页及末页禁用、他人补充只读、关闭后恢复原列表和重新打开回本人；查看正式截图确认桌面侧栏排版、固定头尾和主滚动正常。未创建测试任务、改状态或代填个人说明；普通账号越权/历史等后端及离线小屏检查沿用前述通过结果，不冒称更换身份后的全套正式实点。本轮基线 `3d6fc93`，产品代码无改动，仅此验收记录按同提交同步三方；日报初始化与本轮页面收尾均完成，启用前完整历史仍不补造。
- 初始化已完成：用户自行登录数据库管理员，主代理核对实例后经已授权管理页面只创建六个同版日报触发器，执行身份仍为原业务账号；没有扩权、重置密码或更改全库设置。原业务CLI逐项验证定义后建立初始记录，北京时间2026-09-08 10:11:30；再次运行起点保持不变。正式只读核对日报与实时任务/全员各人合计一致，三个个人范围、越权403和历史起点护栏通过；后端全员读取约0.054秒，仅后端测量。上线559 PHP/53契约复检及公网JS/CSS摘要核对通过。正式页面最后点击验收遇Mac锁屏，已请用户解锁，未冒称实点完成；离线布局/交互验证结果仍有效。本轮基线 `6f30e4e`，仅工作记录按同提交三方同步，产品代码没有追加改动。
- 收尾复核：用户询问是否需要登录数据库。本轮只读确认日报仍为三张空表、0触发器，未初始化护栏正确；未重试安装、重置密码、扩大权限或修改业务数据。代码基线 `1052b2e` 本地与广州一致，仅更新本记录并按既定流程同步。下一步由用户打开已有数据库管理员维护入口并完成登录，再检查可用授权会话后执行限定初始化；成功启用仍不等于能还原启用前的完整历史。
- 发布状态：功能提交 `715a6b3a6253ad149e6a23ab7dc11f6144cc3beb` 已同步 main/发布分支与广州，12文件限定部署及无关内容保留校验通过。正式部署代码559 PHP语法/53后端检查、最终离线Chrome 10组布局、隔离MySQL回归通过。但正式初始化因数据库维护权限条件未满足而停止，日报仍不可用，不宣称完成上线。只创建三张空日报表，0触发器/0记录，原业务任务未由本次初始化修改；接口正确提示尚未初始化，未登录页面302。需取得可用的数据库管理员维护入口后完成同版初始化和正式汇总核对，不能为此扩大业务账号权限或降低全库设置。具体环境信息仅存私有记录。
- 用户批准开始，要求排版美观。基线 `5b29ec0`；待办页面/API与线上相同，无关改动保留。本批顶部右侧今日总结、电脑侧栏/手机全屏、六类统计、分页任务与变更、管理员全员/人员切换、本人补充及历史日期查询。
- 按真实负责人和个人实例统计，多人不加算母任务；完成/应办未完/逾期/明日/其他进行中分区，重开回到未完成。新增独立任务前后状态记录与文字进度记录，写入和事实同事务，读消息/排序不算成果；历史按北京时间日终状态还原，首日变更起点明确，之前不补造。转派双方归属边界防止私人旧正文泄露。
- 后台普通账号强制自己、管理员可全员；本人说明仅自己当天可改，CSRF与版本校验、历史只读、失败保留输入。详情复用原权限入口；关闭详情保留日报位置。页面启动不查询日报，查询不做DDL；触发器通过同版CLI显式初始化，不改历史业务任务。
- 检查：559 PHP语法/53套后端检查，64 JS/CJS语法/14套既有前端检查；真实PHP8.0/MySQL隔离测试含事务回滚、权限、重开转派、基线幂等、历史还原、文字进度、补充冲突及3000+任务分页。规模样本约0.073秒、响应约7.4KB、峰值约29MB，仅为隔离数据，不冒充正式页面速度。离线Chrome 10组360—1440宽度布局及交互检查通过，主代理已查看桌面/手机/团队截图。最终复检和发布结果在后续补齐。
- 共12文件，含待办页面/API、新日报helper与JS/CSS、CLI初始化脚本、3项测试、PHP检查清单、说明和本记录。依既定授权先推GitHub main/发布分支，再同提交限定部署广州、初始化事实记录并复检；本条为发布前记录。不生成真实测试任务、不代填员工说明或发通知。附加/步骤及规则母板操作仍在原详情审计，未来未生成的周期任务不预测。

## 2026-09-08：推广签名检查与可操作性修复

- 发布完成：功能提交 `1d2b245322033950b23dd6360607cf59849529f4` 已推送 main/发布分支并同提交部署广州。独立副本先演练保留修改/未跟踪文件，正式九文件快进前逐项核对与备份，所有无关文件摘要保持。上线后 PHP 8.0 两产品文件语法及555 PHP/52套回归全部通过，公网向导JS摘要与本地相同，未登录CRM仍正常302。内部恢复路径留在私有运维记录。本条仅补发布记录并同提交同步三方。
- 用户反馈推广签名不能用，要求修复；基线 `01806cc`。只读确认线上与本地推广文件一致；有邮箱签名引用手机号/职位而所属人员资料为空，现有严格渲染会阻止最终预览；新向导只有签名选择，没有即时查看/检查入口。用户尚未提供具体故障截图，不能把这个已证实问题断言为所有“签名不能用”的唯一原因。
- 第三步新增按需签名预览与缺项检查，明确选择检查邮箱，不改变执行安排、不向正文重复插入。支持个人/公司/无签名、旧变量别名、收件人变量保留、最新资料重查、切换邮箱/方式/退出后的旧响应丢弃；空白签名不能误判为已设置。
- 检查和最终生成使用同一签名来源，个人签名始终取实际发件账号，公司签名按该账号替换。接口校验账号权限/有效状态；缺项错误包含账号与修复位置。没有放宽正式变量校验，没有填写/清空员工资料或覆盖已保存签名，没有真实客户邮件。
- 检查：62 JS/CJS 语法与14套前端检查、555 PHP语法与52套后端检查通过。正式同版 PHP 8.0.26 独立 MySQL 验证权限、空资料复现与修复后重查、预览/队列/模拟发送一致，连同原并发与3000对象回归通过，隔离实例已停止。大于1MB合成签名图保留；单项目正文限制未变化。离线 Chrome 25个布局及真实签名按钮、失败/旧响应与草稿保留检查通过，不代表正式账号收到邮件验收。
- 修改9文件：推广签名后端两文件、向导JS/CSS、三项既有测试、使用说明和本记录。当前为发布前记录；按已授权本地→GitHub main/发布分支→广州同提交→复检执行，保留所有无关线上改动。部署结果在发布后补记。仍需用户指出若存在另外的收件端签名问题；本批不声称修复未复现的问题。

## 2026-09-08：报价金额一致性整改（用户批准实施）

- 发布结果：功能提交 `08c591e0eb99772d75b0854d6a812560439ca84a` 已推送 GitHub main / 发布分支，并以同一提交部署广州。先在独立副本演练，再对正式 15 文件做原文件校验、备份和限定替换；其它已跟踪/未跟踪文件内容散列校验全部保持。内部恢复位置单独留存于私有运维记录。
- 上线复检：555 个 PHP 语法、52 套后端检查通过；本地 62 个 JS/CJS 语法、14 套前端检查通过。已部署代码在正式同版 PHP 8.0.26 的独立 MySQL 再次通过全部金额、来源版本、并发、回滚和大图测试；大图保存/审核峰值 114,352,128 字节，转单峰值 89,325,568 字节，测试实例已正常停止。报价未登录页面 302、API 401，门禁响应正常；未在真实业务单据上点击保存/审核/转单。
- GitHub 功能提交的 PHP 8.0 / Node 20 验证任务成功；通用自动部署任务因正式目录已有无关改动而按保护规则停止。本次实际上线采用上述已演练的限定部署，不删除脏文件来让自动任务变绿，也不把整个自动工作流描述为成功。此条仅补发布结果，随后同提交同步工作记录三方，产品代码不再变化。
- 用户批准开始修复。基线 `0d32ca5ee4b1f7df4a7515d51f1b201148ab8c90`；先核对并完整纳入 quotation.php / quote_api.php 原有线上差异，未覆盖其它模块改动。所有修改以当前工作副本为源，先检查、GitHub 双分支，再同提交部署。
- `includes/quote_money.php` 统一当前 price、币种、数量、单价精度、行金额和整单调整。保存/审核后台重算并拒绝与页面不符的总额；旧 unit_price / approved_price 不再反向覆盖当前价格，合法零值保留，已存明细打开/复制/预览不按当前成本自动重定价。
- 保存和审核采用打开时版本摘要、事务内行锁校验；审核绑定原始行身份/配置，不从后来刷新的缓存按索引拼接。审核预览显示提交→审核金额及差额，修改数量/单价/倍率/MOQ 时必须写原因并明确确认。状态、明细、金额、审核日志和快照原子写入，失败回滚；通知后置且不误报主业务失败，重复审核拒绝。
- 审核快照去掉递归嵌套旧快照；重提日志记录实际变化，金额审计剔除图片且保持有效 JSON。PDF/Excel 验证审核快照、保留实际单价小数和最终零金额，Excel 行公式按分四舍五入。转单核对来源已审核版本、行快照、数量及总额，保留用户原有 PI 编辑能力及大图分段写入/并发防重复保护。
- 隔离验证：真实 save_quote / approve_quote 路由体配合独立 MySQL，验证旧保存/旧审核拒绝、伪造总额拒绝、改价原因、日志和快照回滚、并发只审核一次、零金额。正式同版 PHP 8.0.26、128 MiB 下大图保存/审核通过；前轮最终产品候选峰值 114,352,128 字节。大图转单请求解析/转单峰值 89,325,568 字节，约 63.85 MB 详情输出通过；图片/快照摘要保持原样。测试数据库均为专用 socket-only 临时实例，不加载正式配置，测试通知故意失败用于验证提交结果不受影响。
- 前端实际函数及审核处理器验证别名换币 600→4020、零值、自动价不重算、精度、原始版本传递、改价原因和失败保留窗口；7 段内联 JS 语法通过。新增实际 Excel XML / PDF 总额回归；新测试纳入持续检查。最后全量复检与上线结果在发布后补记，不提前宣称上线。
- 全部现存 215 张历史报价只读核查：当前最终金额/单价别名冲突 0；150 张旧版主表小计为默认 0，但明细与最终金额一致；214 张审核快照通过，1 张早期单缺失快照。未重算或改写历史金额，缺快照单须由业务核对后按权限另存/重新审核。私有详细核查报告保存在 workspace outputs，不将完整客户/图像数据公开。
- 本批 15 文件（页面、报价接口、公共金额 helper、转单服务、PDF/Excel、4 个新增测试/1 个已有转单测试、2 个 CI 清单、新隔离运行脚本、本记录）。本条为发布前记录；没有替用户创建、审核、反审或转真实订单，没有真实邮件。正式账号浏览器端到端操作未执行，不把隔离验收冒充生产业务验收。

## 2026-09-08：做单/审核金额差异只读诊断

- 用户截图 AT-260904EX139 做单 RMB 4020、审核 RMB 600。只读查报价 ID 282：9 月 4 日两次提交 4020；9 月 5 日审核日志记载 4020→4355，当前主表/明细/审核快照均 4355。截图是历史流程，不把 600 误报为当前存储值，也不判断 4355 是否为业务正确价格。
- 使用当前线上实际 JS 函数合成复现：换币只更新 price/amount/currency，旧 unit_price 或 approved_price 保留 600；按 6.7 从 USD 600 变为 RMB 4020 后，审核优先取旧别名仍显示 600。两条别名路径均复现，对照组 4020 正确。该历史单当前汇率 6.5；旧完整请求日志被 60000 字节截断，无法百分之百还原当时具体触发字段/汇率，不把合成输入当历史事实。
- 还发现保存/审核总额缺少服务端唯一计算与一致性强校验、审核无修订号防并发错配、合法零值回退、重提日志固定空 changes，以及大图截断金额历史的问题。建议一次完整处理单价来源/币种汇率/快照版本/后端校验/审核差额/结构化日志和存量复核。
- 最近 200 张报价有限字段只读抽查未发现已保存别名冲突；只比较完整等长数组，不能替代全量历史及页面状态验收。未修改业务代码、报价、审批、历史订单或导出文件。
- 诊断报告：workspace `outputs/报价做单与审核金额差异诊断_2026-09-08.md`；私有复现与只读脚本在 `work/crm_audit/quote_amount_20260908/`。本轮仓库仅更新本记录，按既定 GitHub 双分支/广州同提交同步，基线 `35732cca7d5a1863f490803c668c638d9012eed4`；尚未获本轮金额整改实施授权，下一步先向用户说明原因和完整方案。

## 2026-09-07：报价转订单大图片内存修复与原子写入

- 发布完成：功能版本 `49aa4a8f331332ee66fae6d49491d1c97206b268` 已同步 GitHub main/发布分支及广州，本地干净。5 文件精确部署、其他线上文件摘要保护通过，恢复包 `/tmp/quote-conversion-release-uPD5chaf`。上线后 551 PHP 语法、50 套既有回归通过；已部署代码以正式 PHP 8.0.26 在独立数据库再次完成全部大数据/详情/回滚/并发/佣金测试，实例 `/tmp/crm-phase1-mysql-20260906-hFmE2rcF` 已停止。只读复核 `AT-260903EX007` 仍无订单记录，原报价大小不变；未代用户生成订单。本条为发布结果记录，随后仅同步本记录，不再改业务代码。
