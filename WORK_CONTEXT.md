...
- 生产只读预检：当前同步产品 243、适配组 25、适配选项 1、产品电源规则 0；新迁移尚未登记，`rule_json`、配件直径和厚度列均不存在，符合首次迁移条件。预检未修改数据库。
- 本地 / 候选检查：38 个 JavaScript 文件语法、3 个 JavaScript 静态 / 契约测试、350 个候选 PHP 文件语法、22 个无数据库 PHP 契约、Shell 语法和 `git diff --check` 全部通过；三份既有 / 新增适配契约均通过。真实数据库集成测试需在同一提交部署并应用迁移后执行。
...
- 待完成：提交并推送 GitHub，等待自动测试 / 部署成功，应用生产迁移 017，运行真实批量套用集成测试并清理测试数据，最后核对页面、公网守卫、数据库残留及本地 / GitHub / 服务器三方一致。
