...
## 本次：找回被覆盖的 AT-260730CN010 报价（只读提取，待决定恢复）
...
- 当前只生成独立恢复副本，没有插入、更新、删除或覆盖正式报价数据。后续若用户确认恢复，应以新数据库 ID 插入 `AT-260730CN010`，不得覆盖现有 ID 116/117，并需同时修复前端陈旧报价 ID 导致跨客户覆盖的问题。
