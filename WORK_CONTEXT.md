...
## 本次：任务中心稳定性与报价流程加载修复（待发布）

- 用户反馈报价/客户跟进可重复保存，历史中已出现同内容同时间的多条记录；本次不删除既有业务记录，只阻止后续重复写入。
...
- 规则：普通账号不能修改当天到期或已经逾期的派工截止时间；管理员不受锁定和次数限制。默认每条单人派工或每个多人派工组每天最多修改 2 次。
- 设置：入口已调整为“派工待办 → 更多设置 → 截止时间规则”；管理员可调整每日最多次数（0 表示不限）和“到期前禁改天数”（默认 0，即当天起锁定；例如设为 2，则到期前两天也锁定）。系统设置中心不再重复显示该业务规则。
- 强制执行：详情保存、表格单元格编辑、移动端编辑和多人派工的组级修改均由后端统一校验；次数记录独立保存到 `dispatch_next_due_change_events`，多人派工按组统一计数，不能仅靠改前端绕过。
- 使用体验：列表、详情和多人编辑弹窗会显示具体不可修改原因并禁用截止时间；当天次数汇总在单次请求中只查询一次，避免为每条待办重复查询而影响派工页面速度。
...
- 截止时间规则：后端不再排除 `personal` 类型；普通账号的个人待办、单人派工和多人派工均遵守当天/逾期锁定及每日修改次数，管理员仍是唯一例外。
