...
- 项目内容编辑框高度由 112px 扩大到 198px，利用原详细说明区域；项目仍按原有 `project` 字段独立保存，不再合并、迁移或清空历史 `description` 数据，不修改多人派工详情展示。
- 再次按用户反馈固定左侧网格为顶部对齐，防止删除详细说明行后剩余空间拉伸“任务标题”所在行；任务标题输入框继续使用原尺寸，只有项目内容框占用释放出的区域。
