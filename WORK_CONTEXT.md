...
- 检查：本机 `crm.js` Node 语法和 `git diff --check` 通过；服务器只读归档隔离应用候选补丁后，新专项契约 PHP 8.0 语法、专项契约和原 1–9 步流程契约通过。第一次专项契约因测试标记写得过严失败，仅调整测试断言后重跑成功，未修改生产代码或数据。
- 修改范围：`assets/crm/crm.js`、`assets/crm/crm.css`、新增专项契约、PHP CI 清单和本上下文；用户原有商务中心命令文件删除及未跟踪文档保持原样。
- Git / 部署 / 线上回归：待本轮后续完成并补记。
