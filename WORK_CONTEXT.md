...
- 检查：本机 JavaScriptCore 解析和 `git diff --check` 通过；服务器 `/tmp` 隔离候选副本使用 PHP 8.0 检查 379 个 PHP 文件语法、31 项无数据库契约全部通过，其中邮件预览专项契约和完整 1–9 步流程契约均通过。没有使用真实邮箱发送测试邮件，正式生产目录尚未在本条记录生成时改写。
...
- Git / 部署：本记录将与修复一起提交并推送 GitHub；GitHub 推送成功后，等待同一提交同步正式服务器，再执行生产复检和本地、GitHub、服务器三方提交号核对。
