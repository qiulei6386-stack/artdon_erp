...
- 视觉验收限制：本轮主动连接应用内浏览器时没有任何可用浏览器会话，无法访问用户登录态；没有再把源码和未登录 302 当作视觉通过。部署后用户强制刷新并重新打开第 9 步，应首先看到蓝框“邮件预览与测试发送”和版本标识；若仍不显示，需要用户提供当时截图以按真实 DOM 继续定位。
...
- Git / 部署：本记录将与修复一起提交并推送 GitHub；GitHub 检查通过后由自动流程同步同一提交到正式服务器，再执行正式复检和三方一致性核对。
