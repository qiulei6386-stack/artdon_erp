...
- 数据清理计划：保留 ID 7（已完成、负责人 suk ie 的有效拜访），软删除故障重复 ID 2–6、8–15 及其关联任务；ID 1 为 7 月 13 日另一客户记录，不处理。
- 修改文件：`crm_visit.php`、`crm_api.php`、`crm.php`、`assets/crm/crm.js`、`WORK_CONTEXT.md`。
- 检查：`git diff --check`、JavaScript 语法、三个 PHP 文件的服务器 PHP 8.0 标准输入语法检查均通过；最终提交、推送、服务器部署、重复数据软删除和三方一致性见本轮最终结果。
