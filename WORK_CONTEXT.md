...
- 安全与回归：新增产品适配持续显示与内联电源规则合同，保存后会将相关产品及电源组退回草稿并重算候选项，保留审批链路。功能提交 `67a06fab576393300d920b3411dd298238236b48` 已推送并快进正式服务器；服务器 PHP 语法通过，但新合同有一处仅测试文本的 PHP 引号错误（业务代码未受影响）。该测试已在本地修正，正按同一流程提交、部署并重新执行合同后补充最终三方核对。
